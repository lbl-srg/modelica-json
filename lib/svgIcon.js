...
function interfacePolygon (interfaceClass, interfaceClasses, minX, maxY, layerName) {
  ...
  if (iconTra.origin !== undefined) {
    topLeftX = Math.min(iconTra.extent[1].x1, iconTra.extent[0].x1) + iconTra.origin.x1 - minX
    topLeftY = maxY - (Math.max(iconTra.extent[0].x2, iconTra.extent[1].x2) + iconTra.origin.x2)
  } else {
    topLeftX = Math.min(iconTra.extent[1].x1, iconTra.extent[0].x1) - minX
    topLeftY = maxY - Math.max(iconTra.extent[0].x2, iconTra.extent[1].x2)
  }
  ...
  if (layer === 'icon') {
    ...
    if (transformation.rotation === undefined ||
      ...
      if (transformation.origin !== undefined) {
        minX = Math.min(transformation.extent[0].x1, transformation.extent[1].x1) +
               transformation.origin.x1
        minY = Math.min(transformation.extent[0].x2, transformation.extent[1].x2) +
    ...
    if (transformation.rotation === undefined ||
      ...
      if (transformation.origin !== undefined) {
        ...
        minY = transformation.extent[0].x2 + transformation.origin.x2
      } else {
        minX = transformation.extent[0].x1
        minY = transformation.extent[0].x2
      }
  ...
  if (boxWidth > 0) {
    if (boxWidth > stringLength) {
      if (horizontalAlign === 'TextAlignment.Left') {
        xStart = Math.min(obj.extent[0].x1, obj.extent[1].x1) - minX
        textAnchor = 'start'
      } else if (horizontalAlign === 'TextAlignment.Right') {
        xStart = Math.max(obj.extent[0].x1, obj.extent[1].x1) - minX
        textAnchor = 'end'
      } else {
        xStart = (obj.extent[0].x1 + obj.extent[1].x1) / 2 - minX
        textAnchor = 'middle'
      }
    } else {
      xStart = (obj.extent[0].x1 + obj.extent[1].x1) / 2 - minX
...
function interfaceSVG (obj, layerName) {
  ...
  if (layerName === 'diagram' && layer !== undefined &&
    ...
    checkText(layer)
    layerTexts.push(textSVG(layer.graphics.text[0], Math.min(layer.coordinateSystem.extent[1].x1, layer.coordinateSystem.extent[0].x1),
                            Math.max(layer.coordinateSystem.extent[1].x2, layer.coordinateSystem.extent[0].x2), layerName))
  }
