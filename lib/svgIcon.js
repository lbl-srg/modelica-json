...
{{#layerTexts.length}}
{{#layerTexts}}
<text font-family="{{font}}" font-size="{{fontSize}}" x="{{xStart}}" y="{{yStart}}" fill="{{fill}}" dominant-baseline="central" alignment-baseline="middle">
{{#tSpan.length}}
{{#tSpan}}
<tspan x="{{xSpan}}" {{#moreLines}} dy="1.0em" {{/moreLines}}>{{textString}}</tspan>
{{/tSpan}}
{{/tSpan.length}}
</text>
{{/layerTexts}}
...
// Create items of svg for interface class
...
// Create items of icon svg for regular class
function regularClassIcon (regClass, interfaceClasses) {
  ...
  var fromY = maxY - regClass.icon.coordinateSystem.extent[0].x2

  var toX = regClass.icon.coordinateSystem.extent[1].x1 - minX
  var toY = maxY - regClass.icon.coordinateSystem.extent[1].x2
  var coordinateExtent = []
  coordinateExtent.push(Object.assign({'x1': fromX, 'x2': fromY}))
  coordinateExtent.push(Object.assign({'x1': toX, 'x2': toY}))
  ...
  if (interfaces.length > 0) {
    for (var k = 0; k < interfaces.length; k++) {
      ...
      var tempValues = interfacePolygon(interfaces[k], interfaceClasses, 'icon')
  ...
  if (regClass.icon.graphics !== undefined &&
    ...
    for (var p = 0; p < regClass.icon.graphics.text.length; p++) {
      layerTexts.push(textSVG(regClass.icon.graphics.text[p], minX, maxY))
    }
  }
  // --------------------------------------------------------------
...
function interfacePolygon (interfaceClass, interfaceClasses, layerName) {
  ...
  instantiateWidth = Math.abs(iconTra.extent[1].x1 - iconTra.extent[0].x1)
  instantiateHeight = Math.abs(iconTra.extent[1].x2 - iconTra.extent[0].x2)
...
function instantiatePoints (pointsRatio, width, height, interfaceClass, layer) {
  ...
  if (layer === 'icon') {
    ...
    if (transformation.rotation === undefined ||
      ...
      y1 = transformation.extent[0].x2
      x2 = transformation.extent[1].x1
      y2 = transformation.extent[0].x2 +
           (transformation.extent[1].x2 - transformation.extent[0].x2) / 2
      x3 = transformation.extent[0].x1
      y3 = transformation.extent[1].x2
    ...
    if (transformation.rotation === undefined ||
      ...
      for (var j = 0; j < pointsRatio.x.length; j++) {
        x.push(pointsRatio.x[j] * width + transformation.extent[0].x1)
        y.push(pointsRatio.y[j] * height + transformation.extent[0].x2)
      }
    } else {
      // Rotate points
      angle = (transformation.rotation) * 3.1416 / 180.0
      ...
      y1 = startPoint.x2 + (endPoint.x2 - startPoint.x2) / 4
      x2 = endPoint.x1
      y2 = startPoint.x2 + (endPoint.x2 - startPoint.x2) / 2
      x3 = x1
      y3 = y1 + (endPoint.x2 - startPoint.x2) / 2
...
function rectangleSVG (obj, minX, maxY) {
  var x = Math.min(obj.extent[0].x1, obj.extent[1].x1) - minX
  var y = maxY - Math.max(obj.extent[0].x2, obj.extent[1].x2)
...
// Create rectangles SVG when the rectangle when raised border pattern
...
// Items for generating simple rectangel
...
function textSVG (obj, minX, maxY) {
  ...
  var fontSizes = sizeOfFont(font, textString, boxHeight, boxWidth)
  var fontSize = fontSizes.fontSize
  var stringLength = fontSizes.textWidth
  var xStart = Math.min(obj.extent[0].x1, obj.extent[1].x1) - minX + (boxWidth - stringLength) / 2
...
function stringfyPoints (obj, minX, maxY) {
  ...
  var points = sortPoints(x, y)
  return instantiatePointsString(points)
}

// Sorted points array so that x is in accending order
function sortPoints (x, y) {
  var sortedX = []
  x.forEach(ele => sortedX.push(ele))
  sortedX.sort(function (a, b) { return a - b })
  var sortedY = []
  for (var j = 0; j < sortedX.length; j++) {
  ...
  if (sortedX.length > 2) {
    for (var l = 1; l < sortedX.length; l++) {
      if (sortedX[l] === sortedX[l - 1]) {
        var temp
        // the first pair
        if (l === 1) {
          if (sortedY[l - 1] === sortedY[l + 1]) {
            temp = sortedY[l]
            sortedY[l] = sortedY[l - 1]
            sortedY[l - 1] = temp
          }
        } else {
          if (sortedY[l] === sortedY[l - 2]) {
            temp = sortedY[l]
            sortedY[l] = sortedY[l - 1]
            sortedY[l - 1] = temp
          }
  ...
  return Object.assign({'x': sortedX}, {'y': sortedY})
}
...
function sizeOfFont (font, textString, boxHeight, boxWidth) {
  var fontSize
  var textWidth
  var ratio = 0.5
  while (true) {
    fontSize = ratio * boxHeight / textString.length
    var eachStringLength = []
    for (var i = 0; i < textString.length; i++) {
      eachStringLength.push(textString[i].length)
    }
    var maxWidth = Math.max.apply(null, eachStringLength)
    textWidth = (fontSize * fontAspectRatio(font)) * (maxWidth)
...
function interfaceSVG (obj, layerName) {
  ...
  if (layer !== undefined && layer.coordinateSystem !== undefined) {
    oriWidth = layer.coordinateSystem.extent[1].x1 - layer.coordinateSystem.extent[0].x1
    oriHeight = layer.coordinateSystem.extent[1].x2 - layer.coordinateSystem.extent[0].x2
  } else {
    oriWidth = 200
    oriHeight = 200
  }
  ...
  for (var i = 0; i < layer.graphics.polygon[0].points.length; i++) {
    pointsOfEach.push((layer.graphics.polygon[0].points[i].x1 - layer.coordinateSystem.extent[0].x1) +
                      ',' +
                      (oriHeight - (layer.graphics.polygon[0].points[i].x2 - layer.coordinateSystem.extent[0].x2)))
  }
...
function minMaxSVG (obj, layerName) {
      ...
      if (layerName === 'icon' && dat.placement.iconTransformation !== undefined) {
        ...
        if (dat.placement.transformation.rotation !== undefined) {
          inputsExtentLow.push(dat.placement.transformation.origin.x2 -
                               Math.abs(dat.placement.transformation.extent[0].x1))
        }
      }
    })
  }
  ...
  var minX = Math.min(Math.min(layer.coordinateSystem.extent[0].x1,
                               layer.coordinateSystem.extent[1].x1),
                      Math.min(minInputsExtentFrom, minTextExtentFrom))
  var minY = Math.min(Math.min(layer.coordinateSystem.extent[0].x2,
  ...
  var maxY = Math.max(Math.max(layer.coordinateSystem.extent[0].x2,
                               layer.coordinateSystem.extent[1].x2),
