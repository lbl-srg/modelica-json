...
function interfacePolygon (interfaceClass, interfaceClasses, minX, maxY, layerName) {
  ...
  var topLeftX = Math.min(iconTra.extent[1].x1, iconTra.extent[0].x1) - minX
  var topLeftY = maxY - Math.max(iconTra.extent[0].x2, iconTra.extent[1].x2)
  ...
  if (layer === 'icon') {
    ...
    if (transformation.rotation === undefined ||
      ...
      for (var i = 0; i < pointsRatio.x.length; i++) {
        x.push(pointsRatio.x[i] * width + Math.min(transformation.extent[0].x1,
                                                   transformation.extent[1].x1))
        y.push(pointsRatio.y[i] * height + Math.min(transformation.extent[0].x2,
                                                    transformation.extent[1].x2))
      }
    } else {
      // Rotate points
