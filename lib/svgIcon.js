...
function newExtent (extent) {
  ...
  newcoord1['y1'] = extent[0]['x2']
  newextent.push(newcoord1)
  newcoord2 = {}
  newcoord2['x2'] = extent[1]['x1']
  newcoord2['y2'] = extent[1]['x2']
...
function textSVG (obj, minX, maxY, layer) {
  ...
  var yStart = maxY - (Math.max(obj.extent[0].x2, obj.extent[1].x2) -
                       (boxHeight - fontSize * textString.length) / 2) +
                       fontSize / 2
  // var yStart = maxY - (Math.min(obj.extent[0].x2, obj.extent[1].x2) +
  //                      (boxHeight - fontSize * textString.length) / 2)
...
function ellipseSVG (obj, minX, maxY) {
  ...
  var cx = (Math.max(obj.extent[0].x1, obj.extent[1].x2) -
           ...
           minX
  var cy = maxY -
           ((Math.max(obj.extent[0].y2, obj.extent[1].y1) -
             Math.min(obj.extent[0].y2, obj.extent[1].y1)) / 2 +
            Math.min(obj.extent[0].y2, obj.extent[1].y1))
