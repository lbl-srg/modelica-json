...
function newExtent (extent) {
  var newextent = []
  if (!('x2' in extent[1]) && (!('y2' in extent[1]))) {
    newcoord1 = {}
    newcoord1['x1'] = extent[0]['x1']
    newcoord1['y1'] = extent[0]['y1']
    newextent.push(newcoord1)
    newcoord2 = {}
    newcoord2['x2'] = extent[1]['x1']
    newcoord2['y2'] = extent[1]['y1']
    newextent.push(newcoord2)
    return newextent
  } else {
    return extent
  }
...
function regularClassIcon (regClass, interfaceClasses) {
  console.log('in regularClassIcon');
  ...
  /* ---------------------------------------------------------- */
  regClass.icon.coordinateSystem.extent = newExtent(regClass.icon.coordinateSystem.extent)
  var fromX = regClass.icon.coordinateSystem.extent[0].x1 - minX
  var fromY = maxY - regClass.icon.coordinateSystem.extent[0].y1

  var toX = regClass.icon.coordinateSystem.extent[1].x2 - minX
  var toY = maxY - regClass.icon.coordinateSystem.extent[1].y2
  var coordinateExtent = []
  coordinateExtent.push(Object.assign({ 'x1': fromX, 'y1': fromY }))
  coordinateExtent.push(Object.assign({ 'x2': toX, 'y2': toY }))
...
function interfacePolygon (interfaceClass, interfaceClasses, minX, maxY, layerName) {
  console.log('inside interfacePolygon');
  ...
  iconTra.extent = newExtent(iconTra.extent)
  instantiateWidth = Math.abs(iconTra.extent[1].x2 - iconTra.extent[0].x1)
  instantiateHeight = Math.abs(iconTra.extent[1].y2 - iconTra.extent[0].y1)
  ...
  if (iconTra.origin !== undefined) {
    topLeftX = Math.min(iconTra.extent[1].x2, iconTra.extent[0].x1) + iconTra.origin.x1 - minX
    topLeftY = maxY - (Math.max(iconTra.extent[0].y1, iconTra.extent[1].y2) + iconTra.origin.x2)
  } else {
    topLeftX = Math.min(iconTra.extent[1].x2, iconTra.extent[0].x1) - minX
    topLeftY = maxY - Math.max(iconTra.extent[0].y1, iconTra.extent[1].y2)
  }
  ...
  if (layer === 'icon') {
    ...
    if (transformation.rotation === undefined ||
        transformation.rotation === 0) {
          transformation.extent = newExtent(transformation.extent)
      if (transformation.origin !== undefined) {
        minX = Math.min(transformation.extent[0].x1, transformation.extent[1].x2) +
               transformation.origin.x1
        minY = Math.min(transformation.extent[0].y1, transformation.extent[1].y2) +
               transformation.origin.x2
      } else {
        minX = Math.min(transformation.extent[0].x1, transformation.extent[1].x2)
        minY = Math.min(transformation.extent[0].y1, transformation.extent[1].y2)
      }
      ...
      oriX = transformation.origin.x1
      oriY = transformation.origin.x2
      x1 = transformation.extent[0].x1
      y1 = transformation.extent[0].y1
      x2 = transformation.extent[1].x2
      y2 = transformation.extent[0].y1 +
           (transformation.extent[1].y2 - transformation.extent[0].y1) / 2
      x3 = transformation.extent[0].x1
      y3 = transformation.extent[1].y2
    ...
    if (transformation.rotation === undefined ||
        transformation.rotation === 0) {
      if (transformation.origin !== undefined) {
        minX = transformation.extent[0].x1 + transformation.origin.x1
        minY = transformation.extent[0].y1 + transformation.origin.x2
      } else {
        minX = transformation.extent[0].x1
        minY = transformation.extent[0].y1
      }
      ...
      x1 = startPoint.x1 + (endPoint.x1 - startPoint.x1) / 2
      y1 = startPoint.y1 + (endPoint.y1 - startPoint.y1) / 4
      x2 = endPoint.x1
      y2 = startPoint.y2 + (endPoint.y2 - startPoint.y2) / 2
      x3 = x1
      y3 = y1 + (endPoint.y2 - startPoint.y2) / 2
...
function instantiatePointsString (pointsValue) {
  console.log('in instantiatePointsString');
...
function lineSVG (obj, minX, maxY) {
  ...
  var stroke = stringfyColor(obj.color)
  var strokeDasharray = linePattern(obj.pattern)
  var strokeWidth = obj.thickness
...
function polygonSVG (obj, minX, maxY) {
  ...
  if (obj.fillPattern !== 'FillPattern.None') {
    fill = stringfyColor(obj.fillColor)
    hasFill = true
  }
  var stroke = stringfyColor(obj.lineColor)
  var strokeDasharray = linePattern(obj.pattern)
  var strokeWidth = obj.lineThickness
...
function rectangleSVG (obj, minX, maxY) {
  obj.extent = newExtent(obj.extent)
  var x = Math.min(obj.extent[0].x1, obj.extent[1].x2) - minX
  var y = maxY - Math.max(obj.extent[0].y1, obj.extent[1].y2)
  var width = Math.abs(obj.extent[1].x2 - obj.extent[0].x1)
  var height = Math.abs(obj.extent[1].y2 - obj.extent[0].y1)
  var fill = null
  var hasFill = false
  if (obj.fillPattern !== 'FillPattern.None') {
    fill = stringfyColor(obj.fillColor)
...
function textSVG (obj, minX, maxY, layer) {
  obj.extent = newExtent(obj.extent)
  var boxWidth = Math.abs(obj.extent[1].x2 - obj.extent[0].x1)
  var boxHeight = Math.abs(obj.extent[1].y2 - obj.extent[0].y1)
  var font = 'helvetica'
  var horizontalAlign = obj.horizontalAlignment !== undefined
    ? obj.horizontalAlignment
    : 'middle'
  var tempString = obj.textString.substring(1, obj.textString.length - 1)
  var textString = tempString.split('\n')
  var fontSizes = sizeOfFont(font, textString, boxHeight, boxWidth, obj, layer)
  var fontSize = fontSizes.fontSize
  ...
  if (boxWidth > 0) {
    if (boxWidth > stringLength) {
      if (horizontalAlign === 'TextAlignment.Left') {
        xStart = Math.min(obj.extent[0].x1, obj.extent[1].x2) - minX
        textAnchor = 'start'
      } else if (horizontalAlign === 'TextAlignment.Right') {
        xStart = Math.max(obj.extent[0].x1, obj.extent[1].x2) - minX
        textAnchor = 'end'
      } else {
        xStart = (obj.extent[0].x1 + obj.extent[1].x2) / 2 - minX
        textAnchor = 'middle'
      }
    } else {
      xStart = (obj.extent[0].x1 + obj.extent[1].x2) / 2 - minX
    ...
    xStart = obj.extent[0].x1 - minX - stringLength / 2
    textAnchor = 'middle'
  }
  var yStart = maxY - (Math.max(obj.extent[0].y1, obj.extent[1].y2) -
                       (boxHeight - fontSize * textString.length) / 2) +
                       fontSize / 2
  // var yStart = maxY - (Math.min(obj.extent[0].y1, obj.extent[1].y2) +
  //                      (boxHeight - fontSize * textString.length) / 2)
...
function ellipseSVG (obj, minX, maxY) {
  obj.extent = newExtent(obj.extent)
  var extentWidth = Math.abs(obj.extent[1].x2 - obj.extent[0].x1)
  var extentHeight = Math.abs(obj.extent[1].y2 - obj.extent[0].y1)
  var cx = (Math.max(obj.extent[0].x1, obj.extent[1].x2) -
            Math.min(obj.extent[0].x1, obj.extent[1].x2)) / 2 +
           Math.min(obj.extent[0].x1, obj.extent[1].x2) -
           minX
  var cy = maxY -
           ((Math.max(obj.extent[0].y1, obj.extent[1].y2) -
             Math.min(obj.extent[0].y1, obj.extent[1].y2)) / 2 +
            Math.min(obj.extent[0].y1, obj.extent[1].y2))
  ...
  if (obj.pattern !== 'LinePattern.None') {
    hasPattern = true
    stroke = stringfyColor(obj.lineColor)
    strokeDasharray = linePattern(obj.pattern)
    strokeWidth = obj.lineThickness
  }
  return Object.assign(
    { 'cx': cx },
    { 'cy': cy },
...
function stringfyPoints (obj, minX, maxY) {
  ...
  for (var i = 0; i < obj.length; i++) {
    ...
    y.push(maxY - obj[i].y1)
  }
  console.log('in stringfyPoints');
...
function interfaceSVG (obj, layerName) {
  ...
  layer.coordinateSystem.extent = newExtent(layer.coordinateSystem.extent)
  if (layer !== undefined && layer.coordinateSystem !== undefined) {
    oriWidth = layer.coordinateSystem.extent[1].x2 - layer.coordinateSystem.extent[0].x1
    oriHeight = layer.coordinateSystem.extent[1].y2 - layer.coordinateSystem.extent[0].y1
  } else {
    oriWidth = 200
    oriHeight = 200
  }
  ...
  for (var i = 0; i < layer.graphics.polygon[0].points.length; i++) {
    pointsOfEach.push((layer.graphics.polygon[0].points[i].x1 - layer.coordinateSystem.extent[0].x1) +
                      ',' +
                      (oriHeight - (layer.graphics.polygon[0].points[i].y1 - layer.coordinateSystem.extent[0].y1)))
  }
  ...
  if (layer !== undefined && layer.graphics.polygon[0].fillColor !== undefined) {
    var temp2 = 'rgb(' + layer.graphics.polygon[0].fillColor.r + ',' +
  ...
  if (layerName === 'diagram' && layer !== undefined &&
    ...
    checkText(layer)
    layerTexts.push(textSVG(layer.graphics.text[0], Math.min(layer.coordinateSystem.extent[1].x2, layer.coordinateSystem.extent[0].x1),
      Math.max(layer.coordinateSystem.extent[1].y2, layer.coordinateSystem.extent[0].y1), layerName))
  }
...
function minMaxSVG (obj, layerName) {
  ...
  if (obj.public !== undefined && obj.public.inputs !== undefined) {
    obj.public.inputs.forEach(function (dat) {
      if (layerName === 'icon' && dat.placement.iconTransformation !== undefined) {
        dat.placement.iconTransformation.extent = newExtent(dat.placement.iconTransformation.extent)
        inputsExtentFrom.push(Math.min(dat.placement.iconTransformation.extent[0].x1,
          dat.placement.iconTransformation.extent[1].x2))
        if (dat.placement.iconTransformation.rotation !== undefined) {
          inputsExtentLow.push(dat.placement.iconTransformation.origin.x2 -
                               Math.abs(dat.placement.iconTransformation.extent[0].x1))
        }
      } else {
        inputsExtentFrom.push(Math.min(dat.placement.transformation.extent[0].x1,
          dat.placement.transformation.extent[1].x2))
        if (dat.placement.transformation.rotation !== undefined) {
          inputsExtentLow.push(dat.placement.transformation.origin.x2 -
                               Math.abs(dat.placement.transformation.extent[0].y1))
        }
      }
    })
  }
    ...
    obj.public.outputs.forEach(function (dat) {
      if (layerName === 'icon' && dat.placement.iconTransformation !== undefined) {
        ...
        outputsExtentTo.push(Math.max(dat.placement.iconTransformation.extent[0].x1,
          dat.placement.iconTransformation.extent[1].x2))
        if (dat.placement.iconTransformation.rotation !== undefined) {
          outputsExtentUp.push(dat.placement.iconTransformation.origin.x2 +
                               Math.abs(dat.placement.iconTransformation.extent[0].x1))
        }
      } else {
        outputsExtentTo.push(Math.max(dat.placement.transformation.extent[0].x1,
          dat.placement.transformation.extent[1].x2))
        if (dat.placement.transformation.rotation !== undefined) {
          outputsExtentUp.push(dat.placement.transformation.origin.x2 +
                               Math.abs(dat.placement.transformation.extent[0].y1))
        }
      }
    })
  ...
  if (layer !== undefined &&
    ...
    layer.graphics.text.forEach(function (dat) {
      dat.extent = newExtent(dat.extent)
      textExtentFrom.push(Math.min(dat.extent[0].x1, dat.extent[1].x2))
      textExtentTo.push(Math.max(dat.extent[0].x1, dat.extent[1].x2))
      textExtentLow.push(Math.min(dat.extent[0].y1, dat.extent[1].y2))
      textExtentUp.push(Math.max(dat.extent[0].y1, dat.extent[1].y2))
    })
  }
  ...
  var minX = Math.min(Math.min(layer.coordinateSystem.extent[0].x1,
    layer.coordinateSystem.extent[1].x2),
  Math.min(minInputsExtentFrom, minTextExtentFrom))
  var minY = Math.min(Math.min(layer.coordinateSystem.extent[0].y1,
    layer.coordinateSystem.extent[1].y2),
  Math.min(minTextExtentLow, minInputsExtentLow))
  // Top right corner (in modelica model)
  var maxX = Math.max(Math.max(layer.coordinateSystem.extent[0].x1,
    layer.coordinateSystem.extent[1].x2),
  Math.max(maxOutputsExtentTo, maxTextExtentTo))
  var maxY = Math.max(Math.max(layer.coordinateSystem.extent[0].y1,
    layer.coordinateSystem.extent[1].y2),
...
module.exports.newExtent = newExtent
