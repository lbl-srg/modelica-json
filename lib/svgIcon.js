...
// Create icon svg for regular class
function regularClassIcon (regClass, interfaceClasses) {
  ...
  regClass.forEach(function (obj) {
    if (obj.icon !== undefined) {
      ...
      checkCoordinateSystem(obj.icon)
      checkLine(obj.icon)
    ...
    var minMaxValue = minMaxSVG(obj, 'icon')
    ...
    if (obj.icon.graphics !== undefined &&
    ...
    if (interfaces.length > 0) {
      for (var k = 0; k < interfaces.length; k++) {
        ...
        tempValues = interfacePolygon(interfaces[k], interfaceClasses, 'icon')
...
function interfacePolygon (interfaceClass, interfaceClasses, layerName) {
  ...
  var svg = (layerName === 'icon') ? objectiveClass.svg.icon
                                   : objectiveClass.svg.diagram // svg section in interface json
  ...
  var originWidth = Number(findNumberString(drawing, 'width=', 'mm'))
  var originHeight = Number(findNumberString(drawing, 'height=', 'mm'))
  var originPointsString = findNumberString(drawing, 'points=', '1')
...
function findNumberString (string, key, unit) {
  var beginIndex = string.indexOf('"', string.indexOf(key)) + 1
  var endIndex = string.indexOf('"', beginIndex)
  var tempWithUnit = string.substring(beginIndex, endIndex)
  var temp
  if (unit !== '1') {
    var numberEnd = tempWithUnit.indexOf(unit)
    temp = tempWithUnit.substring(0, numberEnd)
  } else {
    temp = tempWithUnit
  }
  return temp
...
function textSVG (obj, minX, maxY) {
  ...
  var tempString = textString.split('\n')
  var fontSize = 0.5 * boxHeight / (tempString.length)
  var eachStringLength = []
  for (var i = 0; i < tempString.length; i++) {
    eachStringLength.push(tempString[i].length)
  }
  var maxLength = Math.min.apply(null, eachStringLength)
  var stringLength = (fontSize * fontAspectRatio(font)) * (maxLength)
  temp.push('<text')
  ...
  var spanString = '<tspan x="0" dy="1.0em">'
  for (var j = 0; j < tempString.length; j++) {
    temp.push(spanString + tempString[j] + '</tspan> \n')
  }
  temp.push('</text>')
  return temp.join(' ')
}
...
module.exports.findNumberString = findNumberString
