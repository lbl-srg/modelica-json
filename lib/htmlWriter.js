...
<h3>Protected Blocks</h3>
  ...
  {{#hasDiagramSVG}}
   <a href = 'data:image/svg+xml;utf,{{diagramSVG}}'
   download={{svgName}}>Download and show sequence</a>
  {{/hasDiagramSVG}}
    `
...
function copyImages (fileNames) {
  ...
  fileNames.forEach(fileName => {
    const des = fileName.replace('file:///', '')
    var src = null
    var mustCopy = false
...
function write (fileName, data, parseMode) {
  var nonCDLData = data.filter(ele => !js.isElementaryCDL(ele.topClassName))
  // Copy the images
  const files = []
  ...
  // Write the file
  const page = getHtmlPage(fileName, nonCDLData, parseMode)
