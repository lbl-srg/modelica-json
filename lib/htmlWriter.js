...
<h3>Internal connections</h3>
    ...
    {{#connections}}
    <li>
    <code><a href="#{{0.instanceLinkTarget}}">{{0.instance}}</a>{{#0.connector}}.{{#0.portLinkTarget}}<a href="#{{0.portLinkTarget}}">{{/0.portLinkTarget}}{{0.connector}}
    {{#0.portLinkTarget}}</a>{{/0.portLinkTarget}}{{/0.connector}}</code>
    &rarr;
    <code><a href="#{{1.instanceLinkTarget}}">{{1.instance}}</a>{{#1.connector}}.{{#1.portLinkTarget}}<a href="#{{1.portLinkTarget}}">{{/1.portLinkTarget}}{{1.connector}}
  ...
  if (comCop.info) {
    // console.log('***** comCop = ', JSON.stringify(comCop, null, 2))
    comCop.info = addLinkToCode(comCop.info,
      saveConcat(
        'public.models', 'protected.models', comCop),
      saveConcat(
        'public.parameters', 'protected.parameters', comCop),
      saveConcat(
...
function addLinkToCode (
  ...
  if (allModels) allModels.forEach(obj => { ent[obj.name] = obj.html })
    // Iterate over the tabs and groups
  if (allParameters) {
    allParameters.tabs.forEach(tab => {
      tab.groups.forEach(gro => {
        gro.parameters.forEach(par => {
