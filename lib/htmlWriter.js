...
<h3>Protected Blocks</h3>
  ...
  if (comCop.public && comCop.public.parameters) {
    comCop.public.parameters = groupParameters(comCop.public.parameters)
  }
  if (comCop.protected && comCop.protected.parameters) {
    comCop.protected.parameters = groupParameters(comCop.protected.parameters)
  }
  ...
  if (comCop.info) {
    ...
    var parameters = []
    var prop = saveConcat('public.parameters', 'protected.parameters', comCop)

    if (prop) {
      parameters = prop.tabs.map(tab =>
        tab.groups.map(gro => gro.parameters)[0][0]
      )
    }
...
function addHtmlElements (component) {
  ...
  addHtmlAnchorAndTargets(['public', 'parameters'], comCop)
  addHtmlAnchorAndTargets(['protected', 'parameters'], comCop)
