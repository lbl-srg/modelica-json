...
/** Parse the json component description
  ...
  */
function toHtml (fileName, component, parseMode) {
  var template = `
...
<h3>Protected Blocks</h3>
  ...
  for (var i = 0; i < comCop.length; i++) {
    if (comCop[i].info) {
      ...
      comCop[i].info = updateImageSrc(comCop[i].info)
...
function addHtmlAnchorAndTargets (path, components, parseMode, fileName) {
  for (var i = 0; i < components.length; i++) {
    ...
    if (prop) {
      prop.forEach(mod => {
        mod['html'] = {
          'anchor': mod['name'] ? (components[i].topClassName + '.' + mod['name']) : (components[i].topClassName),
          'target': getTargetName(mod['className'], components[i].topClassName, components, parseMode, fileName)}
      })
    }
  }
}
...
function updateImageSrc (htmlBody) {
  ...
  $('img').each(function (index) {
    ...
    alt = $(this).attr('alt')
    newSrc = '<img alt="' + alt + '" src="' + path.join(__dirname, '../', srcOld) + '">'
...
function write (fileName, data, parseMode) {
  ...
  // Write the file
  const page = getHtmlPage(fileName, data, parseMode)
...
/** Get a text representation of the html page.
  ...
  */
var getHtmlPage = function (fileName, data, parseMode) {
  // Convert each element of data to html
  // const dataWithHtml = data.map(dat => toHtml(dat))
  if (parseMode === 'cdl') {
    const dataWithHtml = toHtml(fileName, data, parseMode)
    // Write the file
    ...
    for (var j = 0; j < fileName.length; j++) {
      ...
      for (var k = 0; k < newData.length; k++) {
        ...
        var packName
        if (newData[k].modelicaFile.endsWith('package.mo')) {
          packName = topCla
        } else {
          packName = topCla.slice(0, idx1)
        }
        if (packName === packNameBase) {
          dataGroupTemp.push(newData[k])
        }
      }
      dataGroup = movePack(dataGroupTemp)
      var withHtml = toHtml(fileName, dataGroup, parseMode)
