...
<h3>Parameters</h3>
    ...
    <tr class="tab ModelicaParameterTab"><td colspan="4">{{name}}</td></tr>
    {{#groups}}
    <tr class="group ModelicaParameterGroup"><td colspan="4">{{name}}</td></tr>
    {{#parameters}}
    <tr>
    ...
    <td class="type, text">{{comment}} {{#html.hasSpe}}[{{#html.hasUnit}}{{html.unit}} {{/html.hasUnit}}{{#html.hasMin}}Min = {{html.min}} {{/html.hasMin}}{{#html.hasMax}}Max = {{html.max}}{{/html.hasMax}}]{{/html.hasSpe}}</td>
    </tr>
    {{/parameters}}
    {{/groups}}
    {{/public.parameters.tabs}}
...
<h3>Inputs</h3>
    ...
    <tr>
      ...
      <td class="type, text"><span><code>{{name}}</code><a name="{{html.anchor}}"></a></span></td>
      <td class="type, text">{{comment}} {{#html.hasSpe}}[{{#html.hasUnit}}{{html.unit}} {{/html.hasUnit}}{{#html.hasMin}}Min = {{html.min}} {{/html.hasMin}}{{#html.hasMax}}Max = {{html.max}} {{/html.hasMax}}{{#html.hasEnable}}Enable = {{html.enable}}{{/html.hasEnable}}]{{/html.hasSpe}}</td>
      {{#conditionalInput}}
...
<h3>Protected Blocks</h3>
  ...
  {{#hasPointList}}
    ...
    <table>
    <tr>
    <th class="type">System/Equipment   </th>
    <th>Name  </th>
    <th>Type    </th>
    <th>Hardwired?   </th>
    <th>Trend [s] </th>
...
function getLibraryLink (className) {
  ...
  return 'https://simulationresearch.lbl.gov/modelica/releases/v9.1.0/help/' +
...
function addLinkToCode (
  ...
  if (allModels) {
    $('a').each(function (index) {
      ...
      var targetMod = allModels.filter(function (model) {
        return model.className === entry
      })
      if (targetMod[0]) {
        var name = targetMod[0].name
        if (name && ent[name]) {
          $(this).replaceWith('<code><a href="#' + ent[name].anchor + '">' + entry + '</a></code>')
        }
      } else {
  ...
  var pathExp = /^(\w+\.)+\w+$/
  // Add hyperlink in <code>...</code> to the parameter, input, output
  $('code').each(function (index) {
    const entry2 = $(this).text().trim()
    if (ent[entry2]) {
      // This is a name of parameter or input or output
      $(this).replaceWith('<code><a href="#' + ent[entry2].anchor + '">' + entry2 + '</a></code>')
    } else {
      if (!pathExp.test(entry2)) {
        $(this).replaceWith('<code>' + equationLink(entry2, ent) + '</code>')
      }
    }
  })
  // Add hyperlink in <pre>...</pre> to the parameter, input, output
  $('pre').each(function (index) {
    const entry3 = $(this).text().trim()
    if (ent[entry3]) {
      // This is a name of parameter or input or output
      $(this).replaceWith('<pre><a href="#' + ent[entry3].anchor + '">' + entry3 + '</a></pre>')
    } else {
      if (!pathExp.test(entry3)) {
        $(this).replaceWith('<pre>' + equationLink(entry3, ent) + '</pre>')
      }
    }
  })
...
/** Search in the equation and the add hyerlink to the parameters the equation
  */
function equationLink (entry, ent) {
  var regExp = /\s*[+*/()[\]{}=\-,]\s*/
  var elements = []
  var index = []
  var eleInd = splitOnRegexWithIndex(entry, regExp)
  for (var i = 0; i < eleInd.length; i++) {
    elements.push(eleInd[i][0])
    index.push(eleInd[i][1])
  }
  var newVal
  var str = entry
  for (var j = elements.length - 1; j >= 0; j--) {
    var ele = elements[j]
    if (ent[ele]) {
      newVal = '<a href="#' + ent[ele].anchor + '">' + ele + '</a>'
      str = str.substring(0, index[j]) + newVal + str.substring(index[j] + ele.length)
    }
  }
  return str
}

/** Return the split string array and the corresponding position index
  */
...
function write (fileName, data, parseMode, outputFormat) {
  var workData = []
  ...
  // Write the file
  const page = getHtmlPage(fileName, imgDir, workData, parseMode, outputFormat)
...
/** Get a text representation of the html page.
  ...
  */
var getHtmlPage = function (fileName, imgDir, data, parseMode, outputFormat) {
