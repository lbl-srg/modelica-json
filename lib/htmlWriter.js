...
/** Parse the json component description
  ...
  */
function toHtml (fileName, imgDir, component, parseMode, cdlDoc) {
  var template = `
...
<h3>Parameters</h3>
    ...
    {{#hasTab}}<tr class="tab ModelicaParameterTab"><td colspan="4">{{name}}</td></tr>{{/hasTab}}
    {{#groups}}
    {{#hasGroup}}<tr class="group ModelicaParameterGroup"><td colspan="4">{{name}}</td></tr>{{/hasGroup}}
    {{#parameters}}
    <tr>
    <td class="type, text"><span>{{{html.type}}}</span></td>
    <td class="type, text"><span><code>{{name}}</code><a name="{{html.anchor}}"></a></span></td>
    <td class="type, text"><span>{{html.value}}</span></td>
    <td class="type, text">{{comment}} {{#html.hasSpe}}[{{#html.hasUnit}}{{html.unit}}{{#html.hasMinMax}}; {{/html.hasMinMax}}{{/html.hasUnit}}{{#html.hasMin}}Min = {{html.min}}{{/html.hasMin}}{{#html.hasMax}}; Max = {{html.max}}{{/html.hasMax}}]{{/html.hasSpe}}</td>
    </tr>
    {{/parameters}}
    {{/groups}}
    {{/public.parameters.tabs}}
...
<h3>Inputs</h3>
    ...
    <tr>
      ...
      <td class="type, text"><span><code>{{name}}</code><a name="{{html.anchor}}"></a></span></td>
      <td class="type, text">{{comment}} {{#html.hasSpe}}[{{#html.hasUnit}}{{html.unit}}{{#html.hasMinMaxEna}}; {{/html.hasMinMaxEna}}{{/html.hasUnit}}{{#html.hasMin}}Min = {{html.min}}{{/html.hasMin}}{{#html.hasMaxEna}}; {{/html.hasMaxEna}}{{#html.hasMax}}Max = {{html.max}} {{/html.hasMax}}{{#html.hasEnable}}; Enable = {{html.enable}}{{/html.hasEnable}}]{{/html.hasSpe}}</td>
      {{#conditionalInput}}
      <td class="type, text"><span>{{enable}}</span></td>
      {{/conditionalInput}}
      </tr>
    {{/public.inputs}}
    </table>
...
<h3>Blocks</h3>
    ...
    {{#public.models}}
    <tr>
      <td class="type, text"><span><code><a href="{{html.target}}">{{className}}</a></code></span></td>
      <td><code>{{name}}</code><a name="{{html.anchor}}"></a></td>
      <td class="type, text"><span>{{comment}}</span></td>
      {{#pubConditionalBlock}}
      <td>{{enable}}</td>
      {{/pubConditionalBlock}}
...
<h3>Protected Blocks</h3>
  ...
  {{#hasDiagramSVG}}
  <p align="center">
  <img alt="Diagram layer" src="img/{{svgName}}" width="70%" height="auto">
  </p>
  {{/hasDiagramSVG}}
    `

  // Add html elements to simplify output generation
  var comCop = addHtmlElements(fileName, component, parseMode, cdlDoc)
  ...
  comCop.forEach(ele => {
    if (ele.info) {
      ...
      ele.info = cdlDoc ? removeLink(ele.info)
                 : addLinkToCode(ele.info,
                  saveConcat(
...
function groupParameters (listOfPars) {
  var tabs = []
  for (var i = 0; i < listOfPars.length; i++) {
    var tempTab = js.getProperty(['annotation', 'dialog', 'tab'], listOfPars[i])
    var tempGroup = js.getProperty(['annotation', 'dialog', 'group'], listOfPars[i])
    if (!listOfPars[i].annotation) {
      listOfPars[i]['annotation'] = {'dialog': {'tab': 'NoTab', 'group': 'NoGroup'}}
      tempTab = 'NoTab'
    } else if (!listOfPars[i].annotation.dialog) {
      listOfPars[i].annotation['dialog'] = {'tab': 'NoTab', 'group': 'NoGroup'}
      tempTab = 'NoTab'
    } else {
      if (tempTab === null) {
        listOfPars[i].annotation.dialog['tab'] = 'NoTab'
        tempTab = 'NoTab'
      }
      if (tempGroup === null) {
        listOfPars[i].annotation.dialog['group'] = 'NoGroup'
      }
    }
    tabs.push(tempTab)
  }
  ...
  const tabEntries = tabsUni.map(tab => {
    ...
    var parsInGroup = []
    groUni.forEach(gro => {
      parsInGroup.push({ 'name': gro, 'parameters': getParametersByTabAndGroup(listOfPars, tab, gro), 'hasGroup': gro !== 'NoGroup'})
    })
    // ensure the not grouped parameters before the grouped parameters
    var noGroupPar = []
    var groupPar = []
    for (var i = 0; i < parsInGroup.length; i++) {
      if (!parsInGroup[i].hasGroup) {
        noGroupPar.push(parsInGroup[i])
      } else {
        groupPar.push(parsInGroup[i])
      }
    }
    const hasTab = tab !== 'NoTab'
    return { 'name': tab, 'groups': [].concat(noGroupPar, groupPar), 'hasTab': hasTab}
  })
  var noTab = []
  var withTab = []
  for (var j = 0; j < tabEntries.length; j++) {
    if(!tabEntries[j].hasTab) {
      noTab.push(tabEntries[j])
    } else {
      withTab.push(tabEntries[j])
    }
  }
  var sortedEntries = [].concat(noTab, withTab)
  return { 'tabs': sortedEntries }
}
...
function getHTMLType (className, topCLass, components, parseMode, fileName) {
  ...
  packages.forEach(function (ele) {
    if (cla.startsWith('CDL.' + ele)) {
      ...
      cla = 'Buildings.' + cla
    } else if (cla.startsWith(ele)) {
      cla = 'Buildings.Controls.OBC.CDL.' + cla
    }
  })
    ...
    if (parseMode === 'cdl') {
      ...
      if (classPack === topClassPack) {
        ...
        if (className.includes('.')) {
          ...
          return topClassPack + '.html#' + className
        }
      }
    }
  }
...
function getLibraryLink (className) {
  ...
  return 'http://simulationresearch.lbl.gov/modelica/releases/v8.0.0/help/' +
  className.substring(0, lastInd).replace(/\./g, '_') + '.html#' +
  className
}

function addHtmlAnchorAndTargets (path, components, parseMode, fileName, cdlDoc) {
...
function setHtmlToParameterModification (parameterModification) {
  ...
  if (parameterModification.value !== null) {
    if (parameterModification.value !== undefined) {
      const defaultValueSet = parameterModification.value.split('.')
      const setLength = defaultValueSet.length
      parameterModification.value = (setLength > 3) ? defaultValueSet[setLength - 1]
                                                    : parameterModification.value
    }
    parameterModification.html.value = addAdjustable(parameterModification)
  }
  const hasUnit = !!parameterModification.unit
  parameterModification.html.hasUnit = hasUnit
  if (hasUnit) {
  ...
  const hasMin = !!parameterModification.min
  parameterModification.html.hasMin = hasMin
  if (hasMin) {
    parameterModification.html.min = addAdjustable(parameterModification.min)
  }
  const hasMax = !!parameterModification.max
  parameterModification.html.hasMax = hasMax
  if (hasMax) {
    parameterModification.html.max = addAdjustable(parameterModification.max)
  }
  parameterModification.html.hasSpe = hasUnit || hasMin || hasMax
  parameterModification.html.hasMinMax = hasMin || hasMax
...
function setConnectorHTMLAttributes (connector) {
  const hasUnit = !!connector.unit
  connector.html.hasUnit = hasUnit
  if (hasUnit) {
  ...
  const hasMin = !!connector.min
  connector.html.hasMin = hasMin
  if (hasMin) {
    connector.html.min = addAdjustable(connector.min)
  }
  const hasMax = !!connector.max
  connector.html.hasMax = hasMax
  if (hasMax) {
    connector.html.max = addAdjustable(connector.max)
  }
  const hasEnable = connector.enable
  connector.html.hasEnable = hasEnable
  if (hasEnable) {
    connector.html.enable = removeQuotes(connector.enable)
  }
  connector.html.hasSpe = hasUnit || hasMin || hasMax || hasEnable
  connector.html.hasMinMaxEna = hasMin || hasMax || hasEnable
  connector.html.hasMaxEna = hasMax || hasEnable
}
...
function setParameterHTMLAttributes (parameter) {
  const classNameSet = parameter.className.split('.')
  const setLength = classNameSet.length
  parameter.html.type = classNameSet[setLength - 1]
...
/** Add html elements to the component to simplify output rendering
  ...
  *@param cdlDoc Flag to check if it is to generate cdl documents in docx format
  *@return A copy of the component with the html elements added
  */
function addHtmlElements (fileName, components, parseMode, cdlDoc) {
  ...
  addHtmlAnchorAndTargets(['public', 'parameters'], comCop, parseMode, fileName, cdlDoc)
  addHtmlAnchorAndTargets(['protected', 'parameters'], comCop, parseMode, fileName, cdlDoc)

  addHtmlAnchorAndTargets(['public', 'models'], comCop, parseMode, fileName, cdlDoc)
  addHtmlAnchorAndTargets(['protected', 'models'], comCop, parseMode, fileName, cdlDoc)
  addHtmlAnchorAndTargets(['extends'], comCop, parseMode, fileName, cdlDoc)

  addHtmlAnchorAndTargets(['public', 'inputs'], comCop, parseMode, fileName, cdlDoc)
  addHtmlAnchorAndTargets(['protected', 'inputs'], comCop, parseMode, fileName, cdlDoc)

  addHtmlAnchorAndTargets(['public', 'outputs'], comCop, parseMode, fileName, cdlDoc)
  addHtmlAnchorAndTargets(['protected', 'outputs'], comCop, parseMode, fileName, cdlDoc)
...
/** remove hyperlink when generating cdl documents
  *
  * @param htmlBody html contents
  */
function removeLink (htmlBody) {
  var $ = cheerio.load('<html><head></head><body>' + htmlBody + '</body></html>')
  // When there is content like:
  //   <a href=\"Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.SetPoints.VAVSupplyFan\">
  //   \nBuildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.SetPoints.VAVSupplyFan</a>
  $('a').each(function (index) {
    const entry = $(this).text().trim()
    $(this).replaceWith('<code>' + entry + '</code>')
  })
  return $('body').html()
}
...
*/
function getPage (data, outputFormat) {
  var fileName = (outputFormat === 'html') ? path.join(__dirname, 'htmlPage.html')
                                           : path.join(__dirname, 'docxPage.html')
...
function write (fileName, data, parseMode, outputFormat, cdlDoc) {
  var workData = []
  if (parseMode === 'modelica') {
    ...
    // filter non cdl data
    workData = data.filter(ele => ele.topClassName !== undefined && !js.isElementaryCDL(ele.topClassName))
  }
  ...
  logger.debug('Compiling html for ' + workData.length + ' components.')
  // Write the file
  const page = getHtmlPage(fileName, imgDir, workData, parseMode, outputFormat, cdlDoc)
...
/** Get a text representation of the html page.
  ...
  *@param parseMode parsing mode, 'cdl' or 'modelica'
  *@param outputFormat output format, 'html' or 'docx'
  *@param cdlDoc Flag to check if it is to generate cdl documents in docx format
  *@return The text representation of the html page.
  */
var getHtmlPage = function (fileName, imgDir, data, parseMode, outputFormat, cdlDoc) {
  // Convert each element of data to html
  // const dataWithHtml = data.map(dat => toHtml(dat))
  if (parseMode === 'cdl') {
    const dataWithHtml = toHtml(fileName, imgDir, data, parseMode, cdlDoc)
    // Write the file
    return getPage(dataWithHtml.join(''), outputFormat)
  } else {
    var page = []
    fileName.forEach(name => {
      ...
      var withHtml = toHtml(fileName, imgDir, dataGroup, parseMode, cdlDoc)
      page.push(getPage(withHtml.join(''), outputFormat))
    })
    return page
  }
}
