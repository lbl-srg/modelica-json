...
function extractFromLongClassSpecifier(long_class_specifier) {
    ...
    if ('composition' in long_class_specifier && long_class_specifier.composition != undefined) {
        ...
        if ('annotation' in composition && composition.annotation!=undefined) {
            dict_identifier['annotation'] = composition.annotation
            dict_identifier['semantics'] = extractSemanticsFromAnnotations(composition.annotation, identifier=identifier)
        }
    }
    if (identifier != null) {
    ...
    for(let i=0; i<element_list.length; i++) {
        ...
        if ('component_clause' in element && element.component_clause != undefined) {
            ...
            for (let i=0; i<component_list.length; i++) {
                ...
                if ('description' in single_component_list && single_component_list.description != undefined) {
                    if ('annotation' in single_component_list.description && single_component_list.description.annotation != undefined) {
                        annotation = single_component_list.description.annotation
                    }
                }
                instances[identifier] = {
...
function extractSemanticsFromAnnotations(annotation, instance_identifier) {
    var semantics = {}
    annotation.forEach(single_annotation => {
        var annotation_name = single_annotation.element_modification_or_replaceable.element_modification.name
        if (annotation_name == "__cdl" || annotation_name == "__Buildings") {
            var class_modifications = single_annotation.element_modification_or_replaceable.element_modification.modification.class_modification
            class_modifications.forEach(class_modification => {
                var key_name = class_modification.element_modification_or_replaceable.element_modification.name
                if (key_name == "semantic") {
                    var semantic_class_modifications = class_modification.element_modification_or_replaceable.element_modification.modification.class_modification
                    semantic_class_modifications.forEach(semantic_class_modification => {
                        var semantic_language_key = semantic_class_modification.element_modification_or_replaceable.element_modification.name
                        var semantic_language = semantic_class_modification.element_modification_or_replaceable.element_modification.modification.expression.simple_expression
                        var semantic_language_content = semantic_class_modification.element_modification_or_replaceable.element_modification.description_string
                        semantic_language = semantic_language.split("\"")[1]
                        semantic_language_content = semantic_language_content.replaceAll("<cdl_instance_name>", instance_identifier)
                        if (semantic_language_key == "metadataLanguageDefinition" || semantic_language_key == "naturalLanguageDefinition") {
                            if (!(semantic_language in semantics)) {
                                semantics[semantic_language] = ""
                            }
                        } else if (semantic_language_key == "metadataLanguage" || semantic_language_key == "naturalLanguage") {
                            if (semantic_language in semantics) {
                                semantics[semantic_language] = semantics[semantic_language] + semantic_language_content + "\n"
                            } else {
                                semantics[semantic_language] = semantic_language_content + "\n"
                            }
                        }
                    })
                }
