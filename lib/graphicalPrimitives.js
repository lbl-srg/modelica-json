...
function graphicAnnotationObj (name, mod) {
  if (name === 'Line') {
    ...
    return rectangleObj(mod)
  } else if (name === 'Polygon') {
    return polygonObj(mod)
  } else if (name === 'Ellipse') {
    return ellipseObj(mod)
  } else if (name === 'Bitmap') {
    return bitmapObj(mod)
  } else if (name === 'Placement') {
    return placementObj(mod)
  } else if (name === 'coordinateSystem') {
    return coordinateSystemObj(mod)
  } else {
    return graphicsObj(mod)
  }
}
...
function coordinateSystemObj (mod) {
  ...
  for (var i = 0; i < claModArr.length; i++) {
    ...
    if (name === 'preserveAspectRatio') { preserveAspectRatio = Number(expression) }
...
function graphicsObj (mod) {
  ...
  allLocs = tempLocs.concat(linLoc ? linLoc : [])
                    .concat(texLoc ? texLoc : [])
                    .concat(recLoc ? recLoc : [])
                    .concat(polLoc ? polLoc : [])
                    .concat(ellLoc ? ellLoc : [])
                    .concat(bitLoc ? bitLoc : [])
...
function graphicAttributeObj (keyNam, valStr) {
  if (keyNam === 'Line') { return lineAttributeObj(valStr) }
  else if (keyNam === 'Text') { return textAttributeObj(valStr) }
  else if (keyNam === 'Rectangle') { return rectangleAttributeObj(valStr) }
  else if (keyNam === 'Polygon') { return polygonAttributeObj(valStr) }
  else if (keyNam === 'Ellipse') { return ellipseAttributeObj(valStr) }
  else {
    return bitmapAttributeObj(valStr) }
}
...
function textAttribute (names, values) {
  ...
  var graIteNams = [], graIteExps = []
  var filShaNams = [], filShaExps = []
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (name === 'textColor') { textColor = colorObj(expression)}
