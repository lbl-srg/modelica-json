...
function lineObj (mod) {
  ...
  for (let i = 0; i < claModArr.length; i++) {
    ...
    const expressionObj = getProperty(['element_modification_or_replaceable', 'element_modification', 'modification', 'expression', 'simple_expression'], ithEle)
    const expressionStr = expressionObj.logical_expression.logical_or[0].logical_and[0].arithmetic_expressions[0].name
    names.push(name)
    values.push(expressionStr)
  }
...
function coordinateSystemObj (mod) {
  ...
  for (let i = 0; i < claModArr.length; i++) {
    ...
    const expressionObj = getProperty(['element_modification_or_replaceable', 'element_modification', 'modification', 'expression', 'simple_expression'], ithEle)
    const expressionStr = expressionObj.logical_expression.logical_or[0].logical_and[0].arithmetic_expressions[0].name
    if (name === 'extent') { extent = this.pointsObj(expressionStr) }
    if (name === 'preserveAspectRatio') { preserveAspectRatio = expressionStr }
    if (name === 'initialScale') { initialScale = Number(expressionStr) }
  }
  return Object.assign(
    { extent },
    { preserveAspectRatio },
...
function graphicsObj (mod) {
  const graObj = mod.expression.simple_expression
  const graStr = graObj.logical_expression.logical_or[0].logical_and[0].arithmetic_expressions[0].name
...
function transformationObj (mod) {
  ...
  for (let i = 0; i < claModArr.length; i++) {
    ...
    const expressionStr = expressionObj.logical_expression.logical_or[0].logical_and[0].arithmetic_expressions[0].name
    if (name === 'extent') { extent = this.pointsObj(expressionStr) }
    if (name === 'origin') { origin = this.originObj(expressionStr) }
    if (name === 'rotation') { rotation = Number(expressionStr) }
  }
  return Object.assign(
    { origin },
    { extent },
