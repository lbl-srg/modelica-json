...
function pointsObj (expStr) {
  // TODO: handle DynamicSelect: points=DynamicSelect({{8,2},{-40,80}},{{8,2}, if u2 then {-40,80} else {-40,-80}})
  const subStr = expStr.substring(expStr.indexOf('{') + 1, expStr.lastIndexOf('}'))
  ...
  for (let i = 0; i < lefBraLocs.length; i++) {
    ...
    const x = Number(eleStrArr[0])
    const y = Number(eleStrArr[1])

    if (!(isNaN(x) || isNaN(y) || x === null || y === null)) {
      points.push(Object.assign({ x, y }))
    }
  }
  return points
}
...
function colorObj (expStr) {
  // TODO: handle DynamicSelect
  const subStr = expStr.substring(expStr.indexOf('{') + 1, expStr.indexOf('}'))
  const subStrArr = subStr.split(',')
  return Object.assign(
...
function graphicAttributeObj (keyNam, valStr) {
  if (keyNam === 'Line') {
    return this.lineAttributeObj(valStr)
  } else if (keyNam === 'Text') {
    return this.textAttributeObj(valStr)
  } else if (keyNam === 'Rectangle') {
    return this.rectangleAttributeObj(valStr)
  } else if (keyNam === 'Polygon') {
    return this.polygonAttributeObj(valStr)
  } else if (keyNam === 'Ellipse') {
    return this.ellipseAttributeObj(valStr)
  } else {
...
function polygonAttributeObj (valStr) {
  const nameValues = this.nameAttributePair(valStr)
  const names = nameValues.names
...
function nameAttributePair (valStr) {
  ...
  const allEquLocs = locations('=', valStr)
  const equLocsWithDouble = []
  equLocsWithDouble.push(allEquLocs[0])
  for (let k = 1; k < allEquLocs.length - 1; k++) {
    const equLoc = allEquLocs[k]
    if (valStr[equLoc + 1] === '=' || valStr[equLoc - 1] === '=' || valStr[equLoc + 1] === '%') {
      continue
    }
    equLocsWithDouble.push(equLoc)
  }
  const lasEquLoc = allEquLocs.at(-1)
  if (valStr[lasEquLoc - 1] !== '=' && valStr[lasEquLoc + 1] !== '%') {
    equLocsWithDouble.push(lasEquLoc)
  }
  const equLocs = []
  if (equLocsWithDouble.length > 0) {
    for (let m = 0; m < equLocsWithDouble.length; m++) {
      const equLocWithDouble = equLocsWithDouble[m]
      if (isValidEnclosed(equLocWithDouble, valStr)) {
        continue
      }
      equLocs.push(equLocWithDouble)
    }
  }
...
/**
 * Check if a character is enclosed in (), [], or {}
 *
 * @param idx Integer
 * @param str string
 */
function isValidEnclosed (idx, str) {
  const rightRoundBracket = str.lastIndexOf('(', idx)
  const leftRoundBracket = str.lastIndexOf(')', idx)
  if ((rightRoundBracket > 0 && leftRoundBracket === -1) || (rightRoundBracket > 0 && rightRoundBracket > leftRoundBracket)) {
    return true
  }
  const rightSquareBracket = str.lastIndexOf('[', idx)
  const leftSquareBracket = str.lastIndexOf(']', idx)
  if ((rightSquareBracket > 0 && leftSquareBracket === -1) || (rightSquareBracket > 0 && rightSquareBracket > leftSquareBracket)) {
    return true
  }
  const rightCurlyBracket = str.lastIndexOf('{', idx)
  const leftCurlyBracket = str.lastIndexOf('}', idx)
  if ((rightCurlyBracket > 0 && leftCurlyBracket === -1) || (rightCurlyBracket > 0 && rightCurlyBracket > leftCurlyBracket)) {
    return true
  }
}
