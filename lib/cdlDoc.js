...
function createSectionNum (documentation) {
  ...
  sortedDoc.forEach((el, idx) => {
    let section = el.instance?.cdlAnnotation?.Documentation?.section
      ...
      ?? 1 // Defaulting to 1 resolves later into sorting sections by instance order
    ...
    // We modify the section property to be relative to the parent instance
    ...
    if (unique != null &&
      ...
      newUniqueElements.push(uniqueElement)
    }
  })
...
function modifyInfo (docElement, evalContext, unitContext, unitData) {
  ...
  docElement.headingText =
    (docElement.instance?.cdlAnnotation?.Documentation?.heading
    ?? docElement.cdlAnnotation?.Documentation?.heading
