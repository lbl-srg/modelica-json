 ...
 * - the class annotation contains `__cdl(Documentation(include=true)`, ) AND (
 *
 * - the class annotation contains `__cdl(Documentation(info=...))`, OR
 * - the class annotation contains `Documentation(info=...)`. )
 *
...
function shallBeDocumented (docElement, evalContext) {
  ...
  const toBeDocumented = (isInstantiated !== false) &&
    (isInstanceCdlDocIncluded !== false) &&
    (isTypeEligible || isClassCdlDocIncluded === true) &&
 ...
 * @param {Array} documentation - An array of documentation objects, each containing
    ...
    if (href != null && /modelica:\/\//.test(href)) {
      ...
      for (const docElement of documentation) {
        ...
        if (docElement.fullClassName === href) {
          const anchorId = createAnchorId(docElement.headingNum, docElement.descriptionString)
          $(this)
            .attr('href', `#${anchorId}`)
 ...
 *
 * @param {Object} documentation  - The array of documentation objects to process.
 */
function createSectionNum (documentation) {
  // Sort by instance nesting depth (in a new array)
  ...
  sortedDoc.forEach(el => {
    let section = el.instance?.cdlAnnotation?.Documentation?.section
      ?? el.classCdlAnnotation?.Documentation?.section
    /* If the instance is not annotated with `unique=true`, the section number is
       relative to the section number of the parent instance. */
    if (`${el.instance?.cdlAnnotation?.Documentation?.unique}` !== 'true') {
      const parentInstance = el.instance?.name.split('.').slice(0, -1).join('.')
      const parent = documentation.find(doc => doc.instance?.name === parentInstance)
      section = (parent?.section == null ? '' : `${parent.section}.`) + section
    }
    el.section = section
  })
  documentation.forEach(el => {
    el.section = sortedDoc.find(_ => el.instance?.name === _.instance?.name).section
  })
}
...
function createHeadingNum (headingIdx, prevHeadingNum) {
  ...
  if (headingIdx === prevHeadingSplit.length) {
    return [...prevHeadingSplit.slice(0, prevHeadingSplit.length - 1),
    Number(prevHeadingSplit[prevHeadingSplit.length - 1]) + 1].join('.')
  } else if (headingIdx < prevHeadingSplit.length) {
    return [...prevHeadingSplit.slice(0, prevHeadingSplit.length - 2),
    Number(prevHeadingSplit[prevHeadingSplit.length - 2]) + 1].join('.')
...
function removeDuplicates (documentation) {
  return documentation.filter((el, idx) => {
    const unique = !!(el.instance?.cdlAnnotation?.Documentation?.unique
      ?? el.classCdlAnnotation?.Documentation?.unique)
    return !unique || unique &&
      documentation.slice(0, idx).findIndex(_ => _.fullClassName === el.fullClassName) < 0
  })
}
...
function buildDoc (classObj, jsons, unitData, outputDir, title = 'Sequence of Operation') {
  ...
  // Create temporary section numbers
  createSectionNum(documentation)
  // Sort the documentation sections
  documentation.sort(sortDocSections.bind(documentation))
  // Remove duplicate elements annotated with `unique=true`
  documentation = removeDuplicates(documentation)
