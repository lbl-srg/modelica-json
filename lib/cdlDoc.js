...
function shallBeDocumented (docElement, evalContext) {
  ...
  const isClassCdlDocInfo = /<html>[\s\S]*\w+[\s\S]*<\/html>/
    .test(docElement.cdlAnnotation?.Documentation?.info)
...
function createSectionNum (documentation) {
  ...
  sortedDoc.forEach(el => {
    ...
    /* If there is no `unique=true` annotation, the section number is
...
function removeDuplicates (documentation) {
  return documentation.filter((el, idx) => {
    ...
    return !unique || unique &&
      documentation.findIndex(_ => _.fullClassName === el.fullClassName) === idx
  })
}
