 ...
 * and modifies the paths inplace to point to the new image location.
 *
...
function createNomenclature (documentation) {
  ...
  documentation.forEach((el) => {
    el['headingIdx'] = ((el.instance?.cdlAnnotation?.section
      ?? el.classCdlAnnotation?.section
...
function modifyInfo (docElement, evalContext, unitContext, unitData) {
  ...
  $('body').prepend(
    createHeading(docElement.headingIdx, docElement.headingNum, docElement.descriptionString)
  );
...
function buildDoc (classObj, jsons, unitData, outputDir, title = 'Sequence of Operation') {
  ...
  // Sort the documentation and create a heading nomenclature
