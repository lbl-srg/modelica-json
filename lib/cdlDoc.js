...
function findFirstParentInDoc (instanceName, documentation) {
  if (!/\./.test(instanceName)) return null
  const parentName = instanceName.split('.').slice(0, -1).join('.')
  const parentInDoc = documentation.find(doc => doc.instance?.name === parentName)
  if (parentInDoc != null) return parentInDoc
  // Iterative call in case the immediate parent is not in doc but a grandparent is
  else return findFirstParentInDoc(parentName, documentation)
}
...
function createSectionNum (documentation) {
  ...
  sortedDoc.forEach((el, idx) => {
    ...
    const parentInDoc = findFirstParentInDoc(el.instance?.name, documentation)
