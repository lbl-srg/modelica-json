...
const createTable = function (data) {
  ...
  // Sort the data by dialogTab and then by dialogGroup if present,
  // using the declaration order of tabs and groups.
  if ('dialogTab' in data[0]) {
    dialogTabs = [...new Set(['General', ...data.map(d => d.dialogTab)])] // General first
    dialogGroups = [...new Set([undefined, ...data.map(d => d.dialogGroup)])] // undefined first
    data.sort((a, b) => {
      return dialogTabs.indexOf(a.dialogTab) - dialogTabs.indexOf(b.dialogTab) ||
        dialogGroups.indexOf(a.dialogGroup) - dialogGroups.indexOf(b.dialogGroup)
    })
  }
  ...
  table += `<table style='width:${tableWidth}px; ${borderStyle}'><thead><tr>` +
    `${keys.map(k => {
  ...
  if ('dialogTab' in data[0]) {
    for (const tab of dialogTabs) {
      const tabData = data.filter(d => d.dialogTab === tab)
      if (tabData.length === 0) continue
      table += `<tr><td colspan='${keys.length}' style='background-color: #c0c0c0; font-weight:bold; ${borderStyle}'>` +
        `${tab.replace(/^\\*"|\\*"$/g, '')}</td></tr>` // Remove surrounding double quotes
      const tabGroups = dialogGroups.filter(g => tabData.map(d => d.dialogGroup).includes(g))
      for (const group of tabGroups) {
        const groupData = tabData.filter(d => d.dialogGroup === group)
        table += group
          ? `<tr><td colspan='${keys.length}' style='background-color: #e0e0e0; ${borderStyle}'>` +
          `${group.replace(/^\\*"|\\*"$/g, '')}</td></tr>` // Remove surrounding double quotes
          : ''
        table += groupData.map(
          row => `<tr>${keys.map(k =>
            `<td style='${borderStyle}'>${row[k]}</td>`).join('')}</tr>`
        ).join('')
      }
    }
    table += '</tbody></table>'
 ...
 * - If the template attribute data-cdl-visible evaluates to false, the element is removed.
 *   Syntax: <template data-cdl-visible=...> ... </template>
 * - When using expressions to toggle the visibility, these must be passed as strings,
 *   not as literal expressions, e.g.: <template data-cdl-visible='param1 <> param2'>.
 *   Double quotes can be used but must be escaped when editing the HTML from Dymola.
 *   modelica-json will interpret the string as an expression and try to evaluate it in all cases.
 *
...
function processCdlToggle ($, evalContext, instanceName) {
  ...
  if ($template.length > 0) {
    $template.each(function () {
      const visibleExp = $(this).data('cdl-visible')
      if (expressionEvaluation.evalExpression(
        visibleExp, evalContext, instanceName) === false) {
        $(this).remove()
      } else {
        // Otherwise, replace the template element with its content (as template elements are hidden).
...
/**
 * Processes the unique elements in the provided documentation array.
 *
 * - This function modifies the documentation array inplace.
 * - It iterates over each element in the documentation array and checks if the element
 * is annotated as unique. If the element is unique and is not the new unique element created by
 * `createSectionNum()`, it replaces the info section with a URI pointing to the `fullClassName`.
 *
 * @param {Array} documentation - The array of documentation elements to process.
 */
...
/**
 * Creates an HTML table with block parameters, inputs, and outputs.
 *
 * @param {Object} variables - An object containing block variables categorized by full class names.
 * @returns {Object} An object containing block anchor IDs and the generated HTML block for variables.
 * @property {Object} blockAnchorIds - An object mapping full class names to their anchor IDs.
 * @property {string} htmlBlockVars - The generated HTML table for the block variables.
 */
function createSectionsBlockVars (variables) {
  ...
  blockNames.forEach(blockName => {
    if (variables[blockName].parameters.length +
      variables[blockName].parameters.length +
      variables[blockName].parameters.length === 0) return
...
/**
 * Generates a documentation string with a section for block variables.
 *
 * @param {Object} docElement - The document element containing the documentation info.
 * @param {Object} blockAnchorIds - An object mapping full class names to their corresponding anchor IDs.
 * @returns {string} - The modified HTML string with the added section for block variables.
 */
function getDocWithSectionBlockVars (docElement, blockAnchorIds) {
  const info = docElement.cdlAnnotation?.Documentation?.info ??
    docElement.documentationInfo
  // Return if no block variables are present
  if (!blockAnchorIds[docElement.fullClassName]) return info

  // Parse document: this will create boilerplate tags (<body>, <head>) if not present
  const $ = cheerio.load(
    info.replace(/^\\*"|\\*"$/g, '') // Remove enclosing double quotes: "simple_expression": "\"<html>...</html>\""
  )
  // Get heading indices
  const headings = []
...
function buildDoc (classObj, jsons, unitData, outputDir, includeVariables = false, title = 'Sequence of Operation') {
  ...
  // Keep only the elements that should be documented
  ...
  const classNamesIncludedInDoc = documentation.map(({ fullClassName }) => fullClassName)
  const variables = Object.fromEntries(
    Object.entries(paramAndDoc.variables).filter(([k, _]) => classNamesIncludedInDoc.includes(k))
  )

  // Create sections with tables describing the block variables
  const { blockAnchorIds, htmlBlockVars } = createSectionsBlockVars(variables)
  ...
  // Include sections with tables describing the block variables
