...
function simplifyModelicaJSON (model, parseMode) {
  const within = getProperty(['stored_definition', 'within'], model)
  const finalDef = getProperty(['stored_definition', 'final_class_definitions'], model)
  const claDefArr = []
  ...
  return Object.assign({'within': within}, {'class_definition': claDefArr})
}
...
function composition (com) {
  const eleLis = elementList(com.element_list)
  const eleSec = elementSections(com.element_sections)
  const extCom = com.external_composition
  const extComObj = extCom ? externalComposition(extCom) : null
  const ann = com.annotation
  const annObj = ann ? classModification(ann) : null
  return Object.assign(
    {'element_list': eleLis},
...
function elementList (eleLis) {
  ...
  for (var i = 0; i < ele.length; i++) {
    ...
    eleArr.push(Object.assign(
      {'import_clause': impCla ? importClause(impCla) : null},
      {'extends_clause': extCla ? extendsClause(extCla): null},
      {'redeclare': redeclare},
      {'final': final},
      {'innner': inner},
      {'outer': outer},
      {'replaceable': replaceable},
      {'constraining_clause': conCla ? constrainingClause(conCla) : null},
      {'class_definition': claDef ? classDefinition(claDef) : null},
      {'component_clause': comCla ? componentClause(comCla) : null},
      {'description': des ? description(des) : null}))
  }
  return eleArr
}
...
function importClause (impCla) {
  ...
  return Object.assign(
    ...
    {'name': name ? nameString(name) : null},
    {'dot_star': dotSta},
    {'import_list': impLis ? importList(impLis) : null},
    {'description': comment ? description(comment) : null}
  )
}
...
function description (des) {
  const strDes = des.string_comment
  const ann = des.annotation
  return Object.assign(
    {'description_string': strDes},
    {'annotation': ann ? classModification(ann) : null}
  )
}
...
function extendsClause (extCla) {
  ...
  const ann = extCla.annotation
  return Object.assign(
    {'name': name},
    {'class_modification': claMod ? classModification(claMod) : null},
    {'annotation': ann ? classModification(ann) : null}
  )
}
...
function componentClause (comCla) {
  ...
  return Object.assign(
    ...
    {'array_subscripts': arrSub ? arraySubscripts(arrSub) : null},
    {'component_list': comLis ? componentList(comLis) : null}
  )
}
...
function arraySubscripts (arrSub) {
  ...
  arrSub.subscripts.forEach (function (obj) {
    ...
    eleArr.push(Object.assign(
      {'colon_op': colOp},
      {'expression': exp ? expression(exp) : null}
    ))
  })
  return eleArr
}
...
function expression (expObj) {
  ...
  return Object.assign(
    {'simple_expression': simExp ? simpleExpression(simExp) : null},
    {'if_expression': ifExp ? ifExpression(ifExp) : null}
  )
}
...
function ifExpression (ifExp) {
  ...
  ifEls.forEach(function (obj) {
    ...
    ifElsArr.push(Object.assign(
      {'condition': expression(con)},
      {'then': expression(then)}
    ))
  })
  return Object.assign(
    {'if_elseif': ifElsArr},
    {'else_expression': expression(elsExp)}
  )
}
...
function componentList (comLis) {
  ...
  compList.forEach(function (obj) {
    ...
    decLisArr.push(Object.assign(
      {'declaration': dec ? declaration(dec) : null},
      {'condition_attribute': conAtt ? expression(conAtt.expression) : null},
      {'description': com ? description(com) : null}
    ))
  })
  return decLisArr
}
...
function declaration (dec) {
  ...
  return Object.assign(
    {'identifier': ident},
    {'array_subscripts': arrSub ? arraySubscripts(arrSub) : null},
    {'modification': mod ? modification(mod) : null}
  )
}
  ...
  if (argLis) {
    ...
    args.forEach(function (ele) {
      ...
      claModArr.push(Object.assign(
        {'element_modification_or_replaceable': eleModRep ? elementModificationReplaceable(eleModRep) : null},
        {'element_redeclaration': eleRed ? elementRedeclaration(eleRed) : null}
      ))
    })
    return claModArr
  } else {
...
function elementModificationReplaceable (eleModRep) {
  ...
  return Object.assign(
    {'each': each},
    {'final': final},
    {'element_modification': eleMod ? elementModification(eleMod) : null},
    {'element_replaceable': eleRep ? elementReplaceable(eleRep) : null}
  )
}
...
function elementModification (eleMod) {
  const name = nameString(eleMod.name)
  const mod = eleMod.modification
  const desStr = eleMod.string_comment
  return Object.assign(
    {'name': name},
    {'modification': mod ? modification(mod) : null},
    {'description_string': desStr}
...
function elementReplaceable (eleRep) {
  ...
  return Object.assign(
    {'short_class_definition': shoClaDef ? shortClassDefinition(shoClaDef) : null},
    {'component_clause1': comCla1 ? componentClause1(comCla1) : null},
    {'constraining_clause': conCla ? constrainingClause(conCla) : null}
  )
}
...
function elementSections (eleSec) {
  ...
  eleSec.forEach(function (obj) {
    ...
    secArr.push(Object.assign(
      {'public_element_list': pubEle ? elementList(pubEle) : null},
      {'protected_element_list': proEle ? elementList(proEle) : null},
      {'equation_section': equSec ? equationSection(equSec) : null},
      {'algorithm_section': algSec ? algorithmSection(algSec) : null}
    ))
  })
  return secArr
}
...
function equation (equ) {
  const assEqu = equ.assigment_equation
  ...
  return Object.assign(
    {'assigment_equation': assEqu ? assigmentEquation(assEqu) : null},
    {'if_equation': ifEqu ? ifEquation(ifEqu) : null},
    {'for_equation': forEqu ? forEquation(forEqu) : null},
    {'connect_clause': conCla ? connectClause(conCla) : null},
    {'when_equation': wheEqu ? whenEquation(wheEqu) : null},
    {'function_call_equation': funCalEqu ? functionCallEquation(funCalEqu) : null},
    {'description': des ? description(des) : null}
  )
}
...
/** Get the simplified json representation for the assigment_equation object
  ...
  */
function assigmentEquation (assEqu) {
  const simExp = assEqu.lhs
  const exp = assEqu.rhs
  return Object.assign(
    {'lhs': simExp ? simpleExpression(simExp) : null},
    {'rhs': exp ? expression(exp) : null}
  )
}
...
function ifEquation (ifEqu) {
  ...
  ifEls.forEach(function (obj) {
    ...
    ifElsArr.push(Object.assign(
      {'condition': expression(obj.condition)},
      {'then': theEquArr}
    ))
  })
  const elsArr = []
...
function functionCallEquation (funCalEqu) {
  ...
  return Object.assign(
    {'function_name': nameString(name)},
    {'function_call_args': funCalArg ? functionCallArgs(funCalArg) : null}
  )
}
...
function functionCallArgs (funCalArg) {
  ...
  return funArgs ? functionArguments(funArgs) : null
}
...
function functionArguments (funArgs) {
  ...
  return Object.assign(
    {'named_arguments': namArg ? namedArguments(namArg) : null},
    {'function_argument': funArg ? functionArgument(funArg) : null},
    {'for_indices': forInd ? forIndices(forInd) : null},
    {'function_arguments': intFunArgs ? functionArguments(intFunArgs) : null}
  )
}
...
function namedArguments (namArg) {
  const namArgArr = []
  namArg.forEach(function (obj) {
    const ident = obj.identifier
    const val = obj.value
    namArgArr.push(Object.assign(
      {'identifier': ident},
      {'value': val ? functionArgument(val) : null}
    ))
  })
  return namArgArr
}
...
function statement (sta) {
  ...
  const assWithFunCalSta = sta.b ? sta.b.assignment_with_function_call_statement : null
  ...
  return Object.assign(
    {'assignment_statement': assSta ? assignmentStatement(assSta) : null},
    {'Function_call_statement': funCalSta ? functionCallStatement(funCalSta) : null},
    {'assignment_with_function_call_statement': assWithFunCalSta ? assignmentWithFunctionCallStatement(assWithFunCalSta) : null},
    {'break': sta.is_break},
    {'return': sta.is_return},
    {'if_statement': ifSta ? ifStatement(ifSta) : null},
    {'for_statement': forSta ? forStatement(forSta) : null},
    {'while_statement': whiSta ? whileStatement(whiSta) : null},
    {'when_statement': wheSta ? whenStatement(wheSta) : null},
    {'description': des ? description(des) : null}
  )
}
...
function assignmentStatement (assSta) {
  ...
  return Object.assign(
    {'identifier': ident ? componentReference(ident) : null},
    {'value': val ? expression(val) : null}
  )
}
...
function functionCallStatement (funCalSta) {
  ...
  return Object.assign(
    {'function_name': name ? componentReference(name) : null},
    {'function_call_args': funCalArg ? functionCallArgs(funCalArg) : null}
  )
}
...
function assignmentWithFunctionCallStatement (assWithFunCalSta) {
  ...
  outExpLis.forEach(function (obj) {
    outExpArr.push(expression(obj.expression))
  })
...
/** Get the simplified json representation for the if_statement object
 ...
 function ifStatement (ifSta) {
  ...
  const elsSta = ifEqu.else_statement
  const ifElsArr = []
  ifEls.forEach(function (obj) {
    const theStaArr = []
...
function whileStatement (whiSta) {
  ...
  const staArr = []
  looSta.forEach(function (obj) {
    staArr.push(statement(obj.statement))
  })
  return Object.assign(
    {'condition': expression(con)},
...
function externalComposition (extCom) {
  ...
  const extAnn = extCom.external_annotation
  return Object.assign(
    {'language_specification': lanSpe},
    {'external_function_call': extFunCal ? externalFunctionCall(extFunCal) : null},
    {'external_annotation': extAnn ? classModification(extAnn) : null}
  )
}
...
function externalFunctionCall (extFunCal) {
  ...
  const exps = expLis ? expLis.expressions : null
  const expArr = []
  if (exps) {
  ...
  return Object.assign(
    {'component_reference': comRef ? componentReference(comRef) : null},
    {'identifier': ident},
    {'expression_list': exps ? expArr : null}
  )
}
...
function shortClassSpecifier (shoClaSpe) {
  ...
  return Object.assign(
    {'identifier': ident},
    {'short_class_specifier_value': val ? shortClassSpecifierValue(val) : null}
  )
}
...
function shortClassSpecifierValue (val) {
  ...
  const pre = basPre ? basPre.type_prefix : null
  const name = val.name
  ...
  if (enuLis) {
    enuLis.forEach(function (obj) {
      ...
      enuArr.push(Object.assign(
        {'identifier': ident},
        {'description': desCri ? description(desCri) : null}
      )) 
    })
  }
  return Object.assign(
...
function derClassSpecifier (derClaSpe) {
  ...
  if (val) {
    ...
    typDes = val.comment ? description(val.comment) : null
  ...
  return Object.assign(
    {'identifier': ident},
    {'der_class_specifier_value': val ? valObj : null}
  )
}
...
function comRefString (comRef) {
  ...
  for (var i = 0; i < comRefPar.length; i++) {
    ...
    comString += (dot_op + ident + arrSubStr)
  }
  return comString
}
...
function funArgsString (funArgObj) {
  ...
  if (namedArg) {
    return namedArgString(namedArg)
  } else {
    if (funArgs) {
...
function namedArgsString (namArgs) {
  if (namArgs.length > 1) {
    var namArr = []
    for (var i = 0; i < namArgs.length; i++) {
      var ident = namArgs[i].identifier
      var value = funArgString(namArgs[i].value)
      namArr.push(ident + '=' + value)
    }
    return namArr.join(',')
  } else {
    return (namArgs[0].identifier + '=' + funArgString(namArgs[0].value))
  }
}
...
function ifExpString (ifExp) {
  ...
  const elsExp = ifExp.else_expression
  const ifExpString = 'if ' + expressionString(ifElse[0].condition) + ' then ' + expressionString(ifElse[0].then)
  if (ifElse.length > 1) {
