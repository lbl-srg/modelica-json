...
function evalInContext (str, context, prefix, separator = '$') {
  /* Tentative parameter evaluation:
     - The instance name is prefixed by `prefix` if it is not null.
     - The instance separator in composite instance names is replaced by `separator`. */
  let strInContext = prefix ? `${prefix}${separator}${str}` : `${str}`
  strInContext = strInContext.replace(/\./g, separator)
  ...
  if (Array.isArray(exprToEvaluate)) {
    ...
    for (const [idx, term] of Object.entries(exprToEvaluate)) {
      ...
      if (Array.isArray(termEvalSplit) && termEvalSplit.some(
        x => Object.keys(context).includes(x.replace(/\./g, separator)))) {
          exprToEvaluate[idx] = evalExpression(
            exprToEvaluate[idx], context, undefined, separator, true)
      }
