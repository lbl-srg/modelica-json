...
function getClassDefinition (jsons, type_specifier, classObject) {
  ...
  if (!/\./.test(type_specifier)) { // Short class definition looked up in the class object
    const classDefinition = jsonQuery.findNestedObjects(classObject, 'class_definition')
      .filter(d => getClassIdentifier(d.class_specifier) === type_specifier)[0]
 ...
 * using the JSONs of all instantiated classes.
 *
 * Not supported:
  ...
  for (const component of components) {
    ...
    if (['parameter', 'constant'].some(el => component.type_prefix?.includes(el))) {
      ...
      fullClassName = lookupClassName(
        component.type_specifier, classObject.within, classObject.fullMoFilePath
      )
...
const mathFunctions = [
  'abs', 'cos', 'cosh', 'sin', 'sinh', 'tan', 'tanh', 'max', 'min', 'sqrt',
  'mod', 'ceil', 'floor', 'sign',
]
...
function evalExpression (expression, context, prefix, _recursive = false) {
  if (!['boolean', 'number', 'string'].includes(typeof expression) || expression == null) {
    return expression
  }
