 ...
 * @param {string} typeSpecifier - The type specifier to match against.
 ...
 */
function getClassDefinition (jsons, typeSpecifier, classObject) {
  if (typeSpecifier == null) { return null }

  if (!/\./.test(typeSpecifier)) { // Test for short class definition: to be looked up in the class object
    const classDefinition = jsonQuery.findNestedObjects(classObject, 'class_definition')
      .filter(d => getClassIdentifier(d.class_specifier) === typeSpecifier)[0]
  ...
  let within = typeSpecifier.replace(/^(.+)\.(.+)$/, '$1')
  const identifier = typeSpecifier.replace(/^(.+)\.(.+)$/, '$2')
  ...
  logger.debug(`Class definition not found for: ${typeSpecifier}`)
  return null
}
...
function getParametersAndBindings (classObject, jsons, fetchDoc = false, _instance, _bindings) {
  ...
  function handleSimpleAssignment (toReturn, component) {
    ...
    let unit = component.unit?.function_call?.name === 'fill' // In case of array parameters
      ? component.unit.function_call.arguments[0].name
      : component.unit
    unit = unit == null ? null : `${unit}`.replace(/"/g, '') // Remove double quotes
    let displayUnit = component.displayUnit?.function_call?.name === 'fill' // In case of array parameters
      ? component.displayUnit.function_call.arguments[0].name
      : component.displayUnit
    displayUnit = displayUnit == null ? null : `${displayUnit}`.replace(/"/g, '') // Remove double quotes
  ...
  if (fetchDoc) {
    ...
    let classCdlAnnotation = jsonQuery.findNestedObjects(
...
function evalInContext (str, context, prefix) {
  ...
  try { return eval?.(`"use strict"; ${str}`) } catch (error) { return str }
}
