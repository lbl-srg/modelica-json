...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName) {
  ...
  var getData = function (topLevelWithin, cla, declaringModelicaFileName) {
  ...
  var newJsonData = allClasses.map(cla => {
    ...
    return getData(topLevelWithin, cla, declaringModelicaFileName)
  }).filter(ele => ele !== null)
...
function getJSON (modelicaFile, outputFormat) {
  ...
  if (outputFormat === 'json-simplified' || outputFormat === 'html') {
    ...
    while (true) {
      ...
      newData = getNewData(topLevelWithin, newData, data, modelicaFile)
      if (newData.length !== 0) {
