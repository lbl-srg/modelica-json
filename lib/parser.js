...
/** Run the parser for the top-level `moFiles`,
  ...
  */
function getJsons (moFiles, parseMode, outputFormat, directory) {
  ...
  if (outputFormat === 'raw-json') {
    moFiles.forEach (function (obj) {
      ...
      // let rawJson = addChecksum(rawJsonWithoutChecksum, obj)
      var outputFileName = ut.getOutputFile(rawJson, 'raw-json', outDir)
      var out = JSON.stringify(rawJson, null, 2)
    ...
    for (var i = 0; i < moFiles.length; i++) {
      var fileList = getSimpleJson(moFiles[i], parseMode, tempJsonDir, parsedFile)
...
function getRawJson (moFile, outputFormat) {
  ...
  const rawJson = mj.toJSON(moFile, MODELICAPATH)
  if (outputFormat === 'raw-json') {
...
function addChecksum (dat, moFile) {
  // find checksum of the modelica file
  let resChecksum = getMoChecksum(moFile)
  return Object.assign(dat, { 'checksum': resChecksum})
}

/**
 *  Get checksum of the modelica file
 *
 * @param moFile modelica file full path
 */
// async function getMoChecksum(moFile) {
//   //source: https://wellingguzman.com/notes/node-checksum
//   return new Promise(function (resolve, reject) {
//     const input = fs.createReadStream(moFile)
 ...
 */
function getSimpleJson (moFile, parseMode, tempDir, parsedFile) {
  if (parsedFile.includes(moFile)) {
    // if the moFile has already been parsed, then no need to go further
    return null
  }
  ...
  // var rawJson = addChecksum(rawJsonWithoutChecksum, moFile)
  ...
  var tempJsonPath = ut.getOutputFile(da, 'json', tempDir)
  // write the simplified json output to file
  var out = JSON.stringify(da, null, 2)
  // var out = JSON.stringify(da)
...
function searchInstances (eleLis) {
  ...
  // iteratively remove duplicate elements
  // impEle = (impEle.length > 0) ? impEle.filter(function (item, pos) { return impEle.indexOf(item) === pos }) : []
  // extEle = (extEle.length > 0) ? extEle.filter(function (item, pos) { return extEle.indexOf(item) === pos }) : []
  // conEle = (conEle.length > 0) ? conEle.filter(function (item, pos) { return conEle.indexOf(item) === pos }) : []
  // comEle = (comEle.length > 0) ? comEle.filter(function (item, pos) { return comEle.indexOf(item) === pos }) : []
  // claModEle = (claModEle.length > 0) ? claModEle.filter(function (item, pos) { return claModEle.indexOf(item) === pos }) : []
