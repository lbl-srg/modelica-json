...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName, parseMode, loop) {
  ...
  if (parseMode === 'modelica') {
    extendsClasses.forEach(function(extendsClass) {
      if(extendsClass.className.startsWith("Buildings")) {
        console.log("extending "+extendsClass.className)
        allClasses.push(extendsClass)
      }
    })
  }
  else {
    allClasses = publicClasses.concat(protectedClasses).concat(extendsClasses)
  }
...
function getJsons (moFile, parseMode, outputFormat) {
  ...
  const extModels = jq.getModels(jsonContent, 'extends')
  ...
  // for (var extendedMoFile in extendedMoFiles) {
    // const extendedRawJsonContent = mj.toJSON(extendedMoFile, parseMode, MODELICAPATH)
    // data.push(extendedRawJsonContent) 
  // }
  ...
  if (outputFormat === 'json' || outputFormat === 'html' || outputFormat === 'docx') {
    ...
    // if (parseMode === 'cdl') {
    if (parseMode === 'cdl' || parseMode === 'modelica') {
