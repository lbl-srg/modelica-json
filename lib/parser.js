 ...
 * @param {boolean} [forceCDL=false] - The flag indicating output json in cdl mode, no matter the file is in or not in the OBC package.
 * @returns {Array<Object>} - An array of JSON objects representing the Modelica files and all used classes.
 */
function getJsons (moFiles, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false, mode = 'cdl', forceCDL = false) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    moFiles.forEach((moFile) => {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFile.split(path.sep).includes('Validation')) ||
        ...
        const simJson = getSimpleJson(moFile, tempJsonDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, forceCDL)
        if (simJson !== null) {
          (
            jsons.push(...simJson)
          )
 ...
 * @param {string} mode - Indicates whether to parse a 'cdl' (default) or 'modelica' file with outputFormat='cxf'.
 * @param {boolean} forceCDL - Indicates force output json in cdl mode, no matter the file is in or not in the OBC package.
 * @returns {Array<Object>} - An array of JSON objects representing the Modelica file and all used classes.
 */
function getSimpleJson (moFile, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, forceCDL) {
  ...
  if ((isElementaryBlockCDL || (!moFile.includes(OBCPath) && !forceCDL) || moFile.includes('UsersGuide')) && outputFormat === 'json' && mode === 'cdl') {
    return null
  }
  ...
  let newInstantiatedClasses = instantiatedClasses.filter(function (val) {
  ...
  if (mode === 'modelica') {
    newInstantiatedClasses = newInstantiatedClasses.filter(function (val) {
  ...
  if (newInstantiatedClasses !== undefined && newInstantiatedClasses !== null && newInstantiatedClasses.length > 0) {
    newInstantiatedClasses.forEach((element) => {
      const simJson = getSimpleJson(element, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, forceCDL)
      if (simJson !== null) {
        jsons.push(...simJson)
      }
    })
  ...
  if (outputFormat === 'semantic' || outputFormat === 'cxf') {
    const allObjectsJson = generateAllObjectsJson(jsons[0], moFile, tempDir, prettyPrint, mode)
    if (outputFormat === 'cxf' && mode === 'cdl') {
      generateCxf(allObjectsJson, moFile, tempDir, prettyPrint, generateElementary, generateCxfCore, mode)
    }
  }
...
function generateAllObjectsJson (jsonOutput, moFile, tempDir, prettyPrint, mode) {
  // instances and connections extraction
  const allObjectsJson = oe.extractAllObjects(jsonOutput, null, mode)
...
function generateCxf (jsonOutput, moFile, tempDir, prettyPrint, generateElementary, generateCxfCore, mode) {
