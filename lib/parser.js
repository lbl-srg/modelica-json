...
function getJsons (moFiles, parseMode, outputFormat, directory) {
  ...
  if (outputFormat === 'raw-json') {
    moFiles.forEach (function (obj) {
      ...
      var outputFileName = ut.getOutputFile(rawJson, 'raw-json', directory)
      ut.writeFile(outputFileName, rawJson)
    })
  } else {
...
function getRawJson (moFile, outputFormat) {
  ...
  return (outputFormat === 'raw-json') ? (delete rawJson.fullMoFilePath) : rawJson
}
...
function getInstantiatedFile (data) {
  ...
  claDef.forEach(function (obj) {
    insCla.push(instantiatedClass(within, obj))
  })
  return insCla
}
...
function instantiatedClass (within, claDef) {
  ...
  if (longCla) {
    ...
    const pubInstancesFile = instanceFilePath(pubInstances, within)
    const proInstancesFile = instanceFilePath(proInstances, within)
    const instantitatedFiles = pubInstancesFile.concat(proInstancesFile)
...
function searchInstances (eleLis) {
  ...
  // iteratively remove duplicate elements
  impEle = impEle.filter(function (item, pos) { return impEle.indexOf(item) === pos })
  extEle = extEle.filter(function (item, pos) { return extEle.indexOf(item) === pos })
  conEle = conEle.filter(function (item, pos) { return conEle.indexOf(item) === pos })
  comEle = comEle.filter(function (item, pos) { return comEle.indexOf(item) === pos })
  claModEle = claModEle.filter(function (item, pos) { return claModEle.indexOf(item) === pos })
...
function elementReplaceable (eleRep) {
  const eleLis = []
  const comCla1 = eleModRep.element_replaceable.component_clause1
  const conCla = eleModRep.element_replaceable.constraining_clause
