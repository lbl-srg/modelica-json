...
function getNewData (jsonData, data, declaringModelicaFileName) {
  ...
  var getData = function (cla, declaringModelicaFileName) {
    // console.log('**** checking file name for ' + JSON.stringify(cla))
    ...
    if (fileName) {
      return mj.toJSON(fileName)
        .then(jsonContent => {
          const jSimple = jq.simplifyModelicaJSON(jsonContent)
          console.log('$$$$$$$', jSimple)
          return Promise.resolve(jSimple)
        })
        .catch(err => {
          logger.error(err)
          throw err
