...
function getJsons (moFiles, parseMode, outputFormat, directory, prettyPrint) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    for (var i = 0; i < moFiles.length; i++) {
      var fileList = getSimpleJson(moFiles[i], parseMode, tempJsonDir, parsedFile, outDir, prettyPrint, outputFormat)
 ...
 * @param outputFormat output format
 */
function getSimpleJson (moFile, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat) {
  ...
  if (!needParsed || parsedFile.includes(moFile)) {
    if (outputFormat === 'semantic') {
      var jsonOp = JSON.parse(fs.readFileSync(outputFileName, 'utf8'))
...
function generateAllObjectsJson (jsonOutput, moFile, tempDir, prettyPrint) {
  ...
  var allObjectsJson = oe.extractAllObjects(jsonOutput)
  var allObjectsJsonPath = ut.getOutputFile(moFile, 'objects', tempDir)
  var allObjectsJsonOut = (prettyPrint === 'false') ? JSON.stringify(allObjectsJson) : JSON.stringify(allObjectsJson, null, 2)
...
function getInstantiatedFile (data, moFile) {
  ...
  claDef.forEach(function (obj) {
    var claLis = instantiatedClass(within, obj, moFile)
    if (claLis) {
