...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName, parseType) {
  ...
  var getData = function (topLevelWithin, cla, declaringModelicaFileName, parseType) {
    ...
    if (fileName) {
      ...
      return jq.simplifyModelicaJSON(jsonContent, parseType)
  ...
  var newJsonData = allClasses.map(cla => {
    ...
    return getData(topLevelWithin, cla, declaringModelicaFileName, parseType)
  }).filter(ele => ele !== null)
...
/** Run the parser for the top-level `moFiles`,
  ...
  */
function getJSON (moFiles, parseType, outputFormat) {
  var jsonData = []
  if (parseType === 'modelica') {
    for (var i = 0; i < moFiles.length; i++) {
      jsonData.push(getJsons(moFiles[i], parseType, outputFormat))
    }
  } else {
    jsonData = getJsons(moFiles[0], parseType, outputFormat)
  }
  return jsonData
}

/** Run the parser for each `moFile`,
  * if `parseType` is `modelica`, returns one single JSON representation of the `moFile`
  * if 'parseType' is `cdl`, returns multiple JSON represetations of giving 'moFile' and
  * the instantiated classes
  */
function getJsons (moFile, parseType, outputFormat) {
  ...
  if (outputFormat === 'json-simplified' || outputFormat === 'html') {
    const da = jq.simplifyModelicaJSON(jsonContent, parseType)
    data.push(da)
    ...
    while (parseType === 'cdl') {
      ...
      newData = getNewData(topLevelWithin, newData, data, moFile, parseType)
      if (newData.length !== 0) {
...
/** Export the JSON data and write to a file using
  ...
  */
function exportJSON (data, outputFileName, outputFormat, parseType) {
  if (outputFormat === 'html') {
    hw.write(outputFileName, data, parseType)
    return data
