...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName, obcOnly) {
  ...
  var getData = function (topLevelWithin, cla, declaringModelicaFileName, obcOnly) {
    ...
    if (fileName) {
      ...
      return jq.simplifyModelicaJSON(jsonContent, obcOnly)
  ...
  var newJsonData = allClasses.map(cla => {
    ...
    return getData(topLevelWithin, cla, declaringModelicaFileName, obcOnly)
  }).filter(ele => ele !== null)
...
/** Run the parser for the top-level `modelicaFile`,
  ...
  */
function getJSON (modelicaFile, obcOnly, outputFormat) {
  ...
  if (outputFormat === 'json-simplified' || outputFormat === 'html') {
    const da = jq.simplifyModelicaJSON(jsonContent, obcOnly)
    data.push(da)
    ...
    while (obcOnly === 'true') {
      ...
      newData = getNewData(topLevelWithin, newData, data, modelicaFile, obcOnly)
      if (newData.length !== 0) {
