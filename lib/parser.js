...
const ce = require('./cxfExtractor.js')
 ...
 * @param {string} parseMode - The parse mode.
 ...
 */
function getJsons (moFiles, parseMode, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    moFiles.forEach((moFile) => {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFile.split(path.sep).includes('Validation')) ||
        ...
        jsons.push(
          ...getSimpleJson(moFile, parseMode, tempJsonDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore)
        )
      }
    })
    if (outputFormat === 'cxf' && generateCxfCore && generateElementary) {
      ce.getCxfCore(moFiles, tempJsonDir, prettyPrint)
    }
 ...
 * @param {string} parseMode - The parse mode for the JSON output.
 ...
 */
function getSimpleJson (moFile, parseMode, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore) {
  // find the modelica file checksum
  ...
  if (!needParsed || parsedFiles.includes(moFile)) {
    jsons.push(JSON.parse(fs.readFileSync(outputFileName, 'utf8')))
  } else {
    ...
    jsons.push(jq.simplifyModelicaJSON(rawJson, parseMode))
    // write the simplified json output to file
    const out = (prettyPrint === 'false') ? JSON.stringify(jsons[0]) : JSON.stringify(jsons[0], null, 2)
    ut.writeFile(outputFileName, out)
    // store the path of the parsed file
    parsedFiles.push(jsons[0].fullMoFilePath)
  }

  // recursively process all instantiated classes
  ...
  if (newInstantiatedClasses) {
    newInstantiatedClasses.forEach((element) => {
      jsons.push(
        ...getSimpleJson(element, parseMode, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore)
      )
    })
  }
