...
const em = require('../lib/evaluateMath.js')
...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName, parseMode, loop) {
  ...
  // Delete classes that is CDL elementary
  // if (parseMode === 'cdl') {
 ...
 */
function getJSON (moFiles, parseMode, outputFormat, evaProPar, evaExp) {
  var jsonData = []
  if (parseMode === 'modelica') {
    for (var i = 0; i < moFiles.length; i++) {
      jsonData.push(getJsons(moFiles[i], parseMode, outputFormat, false, false))
    }
  } else {
    jsonData = getJsons(moFiles[0], parseMode, outputFormat, evaProPar, evaExp)
  }
  return jsonData
}
 ...
 */
function getJsons (moFile, parseMode, outputFormat, evaProPar, evaExp) {
  ...
  if (outputFormat === 'json' || outputFormat === 'html' || outputFormat === 'docx' || outputFormat === 'svg') {
    const da = jq.simplifyModelicaJSON(jsonContent, parseMode, evaProPar, evaExp)
    data.push(da)
    if (parseMode === 'cdl') {
...
function exportJSON (data, outputFileName, outputFormat, parseMode, directory) {
  if (outputFormat === 'html' || outputFormat === 'docx') {
    hw.write(outputFileName, data, parseMode, outputFormat)
      ...
      ut.converttobase64(FullimgPathList, directory).then(function (valuesdict) {
        ...
        outputFileName.forEach(function (element) {
          ...
          fs.readFile(input, 'utf-8', function (err, html) {
            ...
            // Write the docx file
            // fileSaver.saveAs(docx, docxoutput)
            // fs.writeFileAsync(docxoutput, docx)
            fs.writeFileAsync(docxoutput, docx, function(err) {
              if (err) throw err
            })
