...
function getNewData (topLevelWithin, jsonData, data, declaringModelicaFileName, parseMode, cdlDoc) {
  ...
  var getData = function (topLevelWithin, cla, declaringModelicaFileName, parseMode) {
    ...
    if (fileName) {
      ...
      return jq.simplifyModelicaJSON(jsonContent, parseMode, cdlDoc)
...
/** Run the parser for the top-level `moFiles`,
  ...
  */
function getJSON (moFiles, parseMode, outputFormat, cdlDoc) {
  var jsonData = []
  if (parseMode === 'modelica') {
    for (var i = 0; i < moFiles.length; i++) {
      jsonData.push(getJsons(moFiles[i], parseMode, outputFormat, cdlDoc))
    }
  } else {
    jsonData = getJsons(moFiles[0], parseMode, outputFormat, cdlDoc)
  }
  return jsonData
}
...
/** Run the parser for each `moFile`,
  ...
  */
function getJsons (moFile, parseMode, outputFormat, cdlDoc) {
  ...
  if (outputFormat === 'json' || outputFormat === 'html' || outputFormat === 'docx' || outputFormat === 'svg') {
    const da = jq.simplifyModelicaJSON(jsonContent, parseMode, cdlDoc)
    data.push(da)
    if (parseMode === 'cdl') {
      ...
      while (true) {
        ...
        newData = getNewData(topLevelWithin, newData, data, moFile, parseMode, cdlDoc)
        if (newData.length !== 0) {
...
/** Export the JSON data and write to a file using
  ...
  *@param cdlDoc Flag to check if it is to generate cdl documents in docx format
  */
function exportJSON (data, outputFileName, outputFormat, parseMode, directory, cdlDoc) {
  if (outputFormat === 'html' || outputFormat === 'docx') {
    hw.write(outputFileName, data, parseMode, outputFormat, cdlDoc)
    if (outputFormat === 'html') {
      ...
      outputFileName.forEach(function (element) {
        ...
        fs.readFile(input, 'utf-8', function (err, html) {
          ...
          var args = ['-f','html','-t','docx','-o', docxoutput]
          ...
          console.log(html)
          // pandoc(html, args)
          pandoc('Buildings.Controls.OBC.CDL.Continuous.html', '-f html -t docx -o test.docx')
          // Write the docx file
      ...
      //       var repSingleQuote = $.html().replace(/&apos;/g, "'")
      //       var docx = HtmlDocx.asBlob(repSingleQuote)
      //       // Write the docx file
      //       fs.writeFileAsync(docxoutput, docx)
      //       // Delete the original html
      //       fs.unlink(element, (err) => {
