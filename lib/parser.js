 ...
 */
function getJsons (moFiles, parseMode, outputFormat, directory, prettyPrint, generateCxfCore=false) {
...
function getSimpleJson (moFile, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat, generateCxfCore) {
  ...
  if (!needParsed || parsedFile.includes(moFile)) {
    ...
    if (outputFormat === 'cxf') {
      ...
      generateCxf(allObjectsJson, moFile, tempDir, prettyPrint)
    }
    return null
  }
  ...
  console.log(moFile, tempJsonPath)
  // write the simplified json output to file
  ...
  if (outputFormat === 'cxf') {
    ...
    generateCxf(allObjectsJson2, moFile, tempDir, prettyPrint)
  }
  ...
  if (instantiateClass.length > 0) {
    instantiateClass.forEach(function (obj) {
      if (!parsedFile.includes(obj)) {
        parsedFile.push(getSimpleJson(obj, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat))
      }
    })
  }
  return parsedPath
...
function generateCxf (jsonOutput, moFile, tempDir, prettyPrint, generateCxfCore=false) {
  fileNameTokens = moFile.split('.mo')[0].split(path.sep)
  if (fileNameTokens.slice(fileNameTokens.length-3)[0] === "CDL") {
    console.log("elementary Block")
    var cxfGraphJsonLd
    if (generateCxfCore) {
      ...
      cxfGraphJsonLd = Object.assign({}, cxfGraphJsonLd, oe.getCxfGraph(instances, requiredReferences, blockName, true))
    }
    if (cxfGraphJsonLd !== undefined && cxfGraphJsonLd !== null) {
      ...
      const cxfCorePath = ut.getOutputFile('CXF-core.jsonld', 'cxf', tempDir)
    ...
    const requiredReferences = jsonOutput.requiredReferences
    const blockName = moFile.split(path.sep)[moFile.split(path.sep).length - 1].split('.mo')[0]
    const cxfGraphJsonLd = oe.getCxfGraph(instances, requiredReferences, blockName)
    if (cxfGraphJsonLd !== undefined && cxfGraphJsonLd !== null) {
