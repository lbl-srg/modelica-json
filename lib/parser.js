 ...
 * @param {string} parseMode - The parsing mode.
 * @returns {Array<Object>} - An array of JSON objects representing the Modelica files and all used classes.
 */
function getJsons (moFiles, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false, parseMode = 'modelica') {
  ...
  if (outputFormat === 'raw-json') {
    ...
    moFiles.forEach((moFile) => {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFile.split(path.sep).includes('Validation')) ||
        ...
        const simJson = getSimpleJson(moFile, tempJsonDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, parseMode)
        if (simJson !== null) (
          jsons.push(simJson)
        )
      }
 ...
 */
function getSimpleJson (moFile, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, parseMode) {
  // check if it is in the cdl model, output simplified json, and the file is a cdl elementary block.
  const isElementaryBlockCDL = ut.checkIfCdlElementaryBlockOrPackage(moFile, true) && outputFormat === 'json' && parseMode === 'cdl'
  // check if the modelica file is within OBC package.
  const OBCPath = ['Buildings', 'Controls', 'OBC'].join(path.sep)
  if (isElementaryBlockCDL || !moFile.includes(OBCPath) || moFile.includes('UsersGuide')) {
    return null
  }
  ...
  const needParsed = ut.checkIfParse(ut.getOutputFile(moFile, 'json', outDir), moChecksum, moFile) && !isElementaryBlockCDL
  ...
  if (!needParsed || parsedFiles.includes(moFile)) {
    ...
    if (outputFormat === 'json' && parseMode === 'cdl') {
  ...
  if (newInstantiatedClasses !== undefined && newInstantiatedClasses !== null && newInstantiatedClasses.length > 0) {
    newInstantiatedClasses.forEach((element) => {
      const simJson = getSimpleJson(element, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, parseMode)
      if (simJson !== null) {
        jsons.push(simJson)
      }
    })
