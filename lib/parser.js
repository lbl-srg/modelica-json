 ...
 */
function getJsons (moFiles, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false, mode = 'cdl') {
  ...
  if (outputFormat === 'raw-json') {
    ...
    moFiles.forEach((moFile) => {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFile.split(path.sep).includes('Validation')) ||
        ...
        const simJson = getSimpleJson(moFile, tempJsonDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode)
        if (simJson !== null) {
          (
            jsons.push(...simJson)
          )
 ...
 */
function getSimpleJson (moFile, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode) {
  ...
  if ((isElementaryBlockCDL || moFile.includes('UsersGuide')) && outputFormat === 'json' && mode === 'cdl') {
    return null
  }
  ...
  if (newInstantiatedClasses !== undefined && newInstantiatedClasses !== null && newInstantiatedClasses.length > 0) {
    newInstantiatedClasses.forEach((element) => {
      const simJson = getSimpleJson(element, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode)
      if (simJson !== null) {
        jsons.push(...simJson)
      }
    })
