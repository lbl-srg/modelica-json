...
function getJsons (moFiles, parseMode, outputFormat, directory, prettyPrint) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    for (let i = 0; i < moFiles.length; i++) {
      const fileList = getSimpleJson(moFiles[i], parseMode, tempJsonDir, parsedFile, outDir, prettyPrint, outputFormat)
...
function getSimpleJson (moFile, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat) {
  ...
  if (!needParsed || parsedFile.includes(moFile)) {
    ...
    if (outputFormat === 'cxf') {
      const jsonOp = JSON.parse(fs.readFileSync(outputFileName, 'utf8'))
      var allObjectsJson = generateAllObjectsJson(jsonOp, moFile, tempDir, prettyPrint)
...
function generateCxf(jsonOutput, moFile, tempDir, prettyPrint) {
  var instances = jsonOutput.instances
  var blockName = moFile.split(path.sep)[moFile.split(path.sep).length-1].split('.mo')[0]
  var cxfGraphJsonLd = oe.getCxfGraph(instances, blockName)
