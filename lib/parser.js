...
function getJsons (moFiles, parseMode, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    for (let i = 0; i < moFiles.length; i++) {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFiles[i].split(path.sep).includes('Validation')) || (outputFormat === 'cxf' && !generateCxfCore) || (outputFormat !== 'cxf')) {
        const fileList = getSimpleJson(moFiles[i], parseMode, tempJsonDir, parsedFile, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore)
 ...
 * @param generateCxfCore Flag to indicate whether or not to generate CXF-core.jsonld
 */
function getSimpleJson (moFile, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore) {
  let outputFileName = ut.getOutputFile(moFile, 'json', outDir)
  // find the modelica file checksum
  ...
  if (!needParsed || parsedFile.includes(moFile)) {
    if (parsedFile.includes(moFile)) {
      outputFileName = path.join(tempDir, outputFileName.split(outDir)[1])
    }
    let allObjectsJson
    if (outputFormat === 'semantic' || outputFormat === 'cxf') {
      const jsonOp = JSON.parse(fs.readFileSync(outputFileName, 'utf8'))
      allObjectsJson = generateAllObjectsJson(jsonOp, moFile, tempDir, prettyPrint)
    }
    if (outputFormat === 'cxf') {
  ...
  if (outputFormat === 'cxf') {
    generateCxf(allObjectsJson2, moFile, tempDir, prettyPrint, generateElementary, generateCxfCore)
  }
  ...
  if (instantiateClass.length > 0) {
    instantiateClass.forEach(function (obj) {
      if (!parsedFile.includes(obj)) {
        parsedFile.push(getSimpleJson(obj, parseMode, tempDir, parsedFile, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore))
      }
    })
  }
  return parsedPath
