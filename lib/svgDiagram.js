...
function classInstance (classIcon, insClass, minX, maxY) {
  ...
  if (iconItems.layerTexts !== undefined) {
    for (var m = 0; m < iconItems.layerTexts.length; m++) {
      classTexts.push(updateText(iconItems.layerTexts[m], insClass, iconDim, insDim, insTopleft))
    }
  }
  // ---------------------------------------------------------------------------
...
function updateText (iconText, insClass, iconDim, insDim, insTopleft) {
  ...
  for (var i = 0; i < oldTSpan.length; i++) {
    ...
    if (oldTSpan[i].textString.includes('%')) {
      newText = updateTextString(oldTSpan[i].textString, insClass)
...
function connectionLine (obj, minX, maxY) {
  var output = []
  var line = si.lineSVG(obj, minX, maxY)
...
function updateTextString (textString, insClass) {
  var output
  if (textString === '%name') {
    ...
    for (var l = 0; l < parameters.length; l++) {
      insClass.modifications.forEach(function (ele) {
        if (ele.name === parameters[l]) {
    ...
    for (var m = 0; m < values.length; m++) {
      tempString = tempString.replace(values[m].origin, values[m].value)
    }
    output = tempString
  }
  return output
