...
function updateText (iconText, insClass, iconDim, insDim, insTopleft) {
  ...
  for (var i = 0; i < oldTSpan.length; i++) {
    var temp = oldTSpan[i].xSpan
    var temp2 = (temp / iconDim.width) * insDim.width + insTopleft.x
    var textString = (oldTSpan[i].textString.includes('%')) ?
                      updateTextString(oldTSpan[i].textString, insClass)
                      : oldTSpan[i].textString
