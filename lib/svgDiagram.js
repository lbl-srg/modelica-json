...
function regularClassDiagram (indClass, interfaceClasses, regClasses, jsonDataSet) {
  console.log('in regularClassDiagram');
...
function classInstance (classIcon, insClass, minX, maxY, jsonDataSet) {
  ...
  var iconCoordinate = si.newExtent(classIcon.coordinate)
  ...
  var classExtent = si.newExtent(insClass.placement.transformation.extent)
  // Find out decreased ratio of the icon's dimension when it is instantiated
  var widthRatio = Math.abs(classExtent[1].x2 - classExtent[0].x1) /
                   Math.abs(iconCoordinate[1].x2 - iconCoordinate[0].x1)
  var heightRatio = Math.abs(classExtent[1].y2 - classExtent[0].y1) /
                    Math.abs(iconCoordinate[1].y2 - iconCoordinate[0].y1)
  ...
  var topLeftX = (Math.min(classExtent[0].x1, classExtent[1].x2) -
                  widthRatio * Math.min(iconCoordinate[1].x2, iconCoordinate[0].x1)) -
                 minX
  var topLeftY = maxY -
                 (Math.max(classExtent[0].y1, classExtent[1].y2) +
                  Math.min(iconCoordinate[0].y1, iconCoordinate[1].y2) * heightRatio)
    ...
    for (var j = 1; j < linePoints.length; j++) {
      for (var k = 0; k < j; k++) {
        ...
        if (sameOrder1) {
          ...
          dots.push(findDots(linePoints[j], linePoints[k], l, l, false))
        }
        if (sameOrder2) {
          ...
          dots.push(findDots(linePoints[j], linePoints[k], pointsNumberJ - 1 - m, pointsNumberK - 1 - m, true))
        }
        if (reverseOrder1) {
          ...
          dots.push(findDots(linePoints[j], linePoints[k], n, pointsNumberK - 1 - n, false))
        }
        if (reverseOrder2) {
          ...
          dots.push(findDots(linePoints[k], linePoints[j], p, pointsNumberJ - 1 - p, false))
        }
      }
    }
  }
