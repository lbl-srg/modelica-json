...
function updateText (iconText, insClass, iconDim, insDim, insTopleft, jsonDataSet) {
  ...
  for (var i = 0; i < oldTSpan.length; i++) {
    var oldxSpan = oldTSpan[i].xSpan
    var oldText = oldTSpan[i].textString
    var scaledxSpan = (oldxSpan / iconDim.width) * insDim.width + insTopleft.x
    var newText = oldText
    var newTextPos = scaledxSpan
    if (oldTSpan[i].textString.includes('%')) {
      newText = updateTextString(oldTSpan[i].textString, insClass, jsonDataSet)
      newTextPos = updateTextPos(scaledxSpan, oldText.length, newText.length, fontSize, iconText.font)
    }
...
function updateRectangle (iconRect, iconDim, insDim, insTopLeft) {
  ...
  var newStrokeWidth = iconRect.strokeWidth * ratio.x
  return Object.assign(
    {'x': newX},
    {'y': newY},
    {'width': newWidth},
...
function lineJoints (linesWithBackground) {
  ...
  if (lines.length > 1) {
    ...
    for (var j = 1; j < linePoints.length; j++) {
      for (var k = 0; k < j; k++) {
        // search if any two lines have the same starting point

        if (linePoints[j].x[0] === linePoints[k].x[0] && linePoints[j].y[0] === linePoints[k].y[0]) {
          // find out the lines that have less number of points
          var lessPointLine = Math.min(linePoints[j].x.length, linePoints[k].x.length)
          // search points on both lines, find out at which point the two lines become seperately
          for (var l = 0; l < lessPointLine - 1; l++) {
            if (linePoints[j].x[l] !== linePoints[k].x[l] || linePoints[j].y[l] !== linePoints[k].y[l]) {
              break
            }
          }
          var temp
          ...
          if (linePoints[j].x[l] === linePoints[k].x[l]) {
            temp = [linePoints[j].y[l - 1], linePoints[j].y[l], linePoints[k].y[l]]
            temp.sort(function (a, b) { return a - b })
            eclipseCx = linePoints[j].x[l]
            eclipseCy = temp[1]
          } else if (linePoints[j].y[l] === linePoints[k].y[l]) {
            temp = [linePoints[j].x[l - 1], linePoints[j].x[l], linePoints[k].x[l]]
            temp.sort(function (a, b) { return a - b })
            eclipseCx = temp[1]
            eclipseCy = linePoints[j].y[l]
          } else {
            eclipseCx = linePoints[j].x[l - 1]
            eclipseCy = linePoints[j].y[l - 1]
          }
