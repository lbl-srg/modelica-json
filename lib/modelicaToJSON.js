...
function toJSON (modelicaFile) {
  ...
  var fileContents = fs.readFileSync(modelicaFile, 'utf8');
  ...
  var op = visitor.visitStored_definition(parseTree);
  var jsonContent = JSON.stringify(op);
  logger.debug('Parsing output to json.')
  var res = JSON.parse(jsonContent)
