          ...
          instanceDict.annotation.forEach(eleModRep => {
            if ('element_modification_or_replaceable' in eleModRep) {
              if ('element_modification' in eleModRep.element_modification_or_replaceable) {
                ...
                if (!('name' in graphicalObj) && ('Line' in graphicalObj || 'Text' in graphicalObj ||
                    'Rectangle' in graphicalObj || 'Polygon' in graphicalObj || 'Ellipse' in graphicalObj ||
                    'Bitmap' in graphicalObj || 'Placement' in graphicalObj || 'coordinateSystem' in graphicalObj)) {
                  const graphicalObjStr = json2moGraphic.parse(graphicalObj)
