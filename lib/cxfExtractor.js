  ...
  instancesList.forEach(instance => {
    ...
    if (instanceType === 'long_class_specifier') {
      ...
      if (blockDocumentation !== "") {
        graph.add(instanceNode, s231Ns('description'), blockDocumentation)
      }
...
function getCxfSingleHtmlContent(graph, cxfBlockName, s231Ns, rdfNs, blockNode, imagePathUrlPrefix) {
  const imagePath = imagePathUrlPrefix + cxfBlockName + "I.png"
  ...
  if (descriptions.length === 1) {
    htmlDescription = descriptions[0].value.split('<html>')[1].split('</html>')[0]
  }
  ...
  var content = {'blockName': cxfBlockName, 'imagePath': imagePath, 'inputs': htmlInputs, 'outputs': htmlOutputs, 
                    'description': htmlDescription, 'parameters': htmlParameters, 'constants': htmlConstants}
  return content
}
...
function getCxfHtml(cdlFolder, directory) {
      ...
      if (stat && stat.isDirectory()) {
        ...
        if (!newPath.endsWith('package.jsonld')) {
          ...
          cxfParsePromises.push(processGraph(cxfGraphJsonld, prefix, mimeType, newPath, cxfBlockName, s231Ns, rdfNs, imagePathUrlPrefix))
            // .then((htmlContent) => {
            //     htmlContents = htmlContents.concat(htmlContent)
          ...
          //   }
          //   const blockNodeRes = graph.each(undefined, rdfNs('type'), s231Ns('ElementaryBlock'))
          //   if (blockNodeRes.length == 0) {
          //     const enumerationRes = graph.each(undefined, rdfNs('type'), s231Ns('EnumerationType'))
          //     if (enumerationRes.length > 0) {
          //       // TODO: handle enumeration
          //       console.log("enumeration. not hanlded at the moment.")                
          //     } else {
          //       throw Error("No ElementaryBlocks defined in graph of "+newPath+". Error.")  
          //     }
          //   }
          //   else if (blockNodeRes.length > 1) {
          //     throw Error("More than one ElementaryBlock defined in graph of "+newPath+". Error.")
          //   } 
          //   else {
          //     const blockNode = blockNodeRes[0]
            
          //     const blockContent = getCxfSingleHtmlContent(graph, cxfBlockName, s231Ns, rdfNs, blockNode, imagePathUrlPrefix)
          //     htmlContents.push(blockContent)
          //   }
          // console.log(htmlContents)
          // })
        }
  ...
  // const cxfRelPathFilename = ut.getRelativePath(cdlFile).split('.mo')[0]
  // const cxfBlockName = cxfRelPathFilename.replaceAll(path.sep, '.')
  // const cxfRelPath = path.join(directory, 'cxf',  cxfRelPathFilename+'.jsonld')
  // const cxfGraphJsonld = fs.readFileSync(cxfRelPath, 'utf-8')
  // const imagePathUrlPrefix = "https://simulationresearch.lbl.gov/modelica/releases/v11.0.0/help/"
  ...
  // rdflib.parse(cxfGraphJsonld, graph, "https://example.org/", mimeType, (err) => {
  //   if (err)  {
  //     console.error("Parsing error:", err);
  //   }
  //   const blockContent = getCxfSingleHtmlContent(s231Ns, blockNode, imagePath)

    
  //   var res = nj.render('lib/docHtmlTemplate.html', context);
  //   ut.writeFile(path.join(directory, 'cxf', cxfBlockName+'.html'), res)
  //   const filePath = path.join(directory, 'cxf', cxfBlockName+'.docx')
