...
/**
 * Checks if the given name is a CDL elementary block.
 * @param {string} name - The name to check.
 * @returns {boolean} - True if the name is a CDL elementary block, false otherwise.
 */
...
/**
 * Checks if the given instances and required references represent a valid CDL.
 * @param {Object} instances - The list of instantiated objects with a CDL file.
 * @param {Object} requiredReferences - The connect statements and extends clause of the CDL file.
 * @returns {boolean} - Returns true if the CDL is valid, false otherwise.
 */
...
/**
 * Retrieves the CXF graph based on the provided instances, required references, block name, and generation options.
 *
 * @param {Object} instances - The list of instantiated objects with a CDL file.
 * @param {Array} requiredReferences - The connect statements and extends clause of the CDL file.
 * @param {string} blockName - The CDL block name.
 * @param {boolean} generateElementary - Indicates whether to generate elementary blocks.
 * @param {boolean} generateCxfCore - Indicates whether to generate CXF-Core.jsonld.
 * @returns {Object|null} - The CXF graph or null if the instances are not valid.
 */
function getCxfGraph (instances, requiredReferences, blockName, generateElementary, generateCxfCore) {
  ...
  instancesList.forEach(instance => {
    ...
    if (instanceType === 'long_class_specifier') {
      ...
      const blockDocumentation = getBlockDocumentationAnnotation(instanceDict, instance)
      if (blockDocumentation !== "") {
        graph.add(instanceNode, s231Ns('documentation'), blockDocumentation)
      }
      if ('description_string' in instanceDict && instanceDict.description_string !== undefined && instanceDict.description_string !== null) {
        graph.add(instanceNode, s231Ns('description'), instanceDict.description_string)
      }
      ...
      if ((isElementaryBlock && generateElementary && instanceAccessSpecifier === 'public') || (isElementaryBlock && generateCxfCore && instanceAccessSpecifier === 'public') || !isElementaryBlock) {
        ...
        if (instanceDict.type_prefix === 'parameter') {
          ...
          } else if (typeSpecifier.includes('CDL.Types')) { 
            graph.add(instanceNode, s231Ns('isOfDataType'), s231Ns(typeSpecifier))
            graph.add(instanceNode, rdfNs('type'), s231Ns('Parameter'))
          }
...
function getBlockDocumentationAnnotation(instanceDict, instance) {
  // console.log(instanceDict['annotation'][1]['element_modification_or_replaceable']['element_modification']['modification']['class_modification'][0]['element_modification_or_replaceable'])
  var found = false
  if (instanceDict !== undefined && instanceDict !== null && 'annotation' in instanceDict) {
    const annotations = instanceDict['annotation']
    if (annotations !== undefined && annotations !== null && annotations.length > 0) {
      let i = 0
      for (i=0; i<annotations.length; i++) {
        const annotation = annotations[i]
        if (annotation !== undefined && annotation !== null && 'element_modification_or_replaceable' in annotation) {
          const eleModRep = annotation['element_modification_or_replaceable']
          if (eleModRep !== undefined && eleModRep !== null && 'element_modification' in eleModRep) {
            const eleMod = eleModRep['element_modification']
            if (eleMod !== undefined && eleMod !== null && 'name' in eleMod) {
              const name = eleMod['name']
              if (name !== undefined && name !== null && name.length > 0 && name !== 'Documentation') {
                continue
              }
              if ('modification' in eleMod) {
                const modification = eleMod['modification']
                if (modification !== undefined && modification !== null && 'class_modification' in modification) {
                  const classModifications = modification['class_modification']
                  const infosRevisions = parseClassModification(classModifications, instance, ['info', 'revisions'])
                  if (infosRevisions !== undefined && infosRevisions !== null && 'info' in infosRevisions) {
                    const info = infosRevisions['info']
                    if (info !== undefined && info != null && 'value' in info) {
                      found = true
                      return info['value']
                    }
                  }
...
function modifyHtml(htmlString) {
  const htmlContent = cheerio.load(htmlString, {
    normalizeWhitespace: false,
    xmlMode: false,
    decodeEntities: false });

  htmlContent('pre').each((_, preElement) => {
    const newStyle = "background-color: white; font-size: 11px; padding-top: 4px; padding-bottom: 4px; color: maroon; font-family: \'Courier New\';"
    htmlContent(preElement).attr('style', newStyle);
  });
  return htmlContent('body').html();
}

function getCxfSingleHtmlContent(graph, cxfBlockName, s231Ns, rdfNs, blockNode, imagePathUrlPrefix) {

  const imagePath = fs.readFileSync(path.join(__dirname, 'cdl_icons', cxfBlockName+'.png'), 'base64')
  let descriptions = [], htmlDescription = ""
  descriptions = graph.each(s=blockNode, p=s231Ns('description'), o=undefined)
  if (descriptions.length > 1) {
    console.log("more than one description string found for node.")
    htmlDescription = ""
  }
  if (descriptions.length === 1) {
    htmlDescription = descriptions[0].value
  }

  let documentations = [], htmlDocumentation = ""
  documentations = graph.each(s=blockNode, p=s231Ns('documentation'), o=undefined)
  if (documentations.length > 1) {
    console.log("more than one documentation string found for node.")
    htmlDocumentation = ""
  }
  if (documentations.length === 1) {
    htmlDocumentation = documentations[0].value.split('<html>')[1].split('</html>')[0]
    // TODO: modify content using cheerio instead of manually
    // htmlDocumentation = modifyHtml(htmlDocumentation)   

    //replace image sources within the description
    if (htmlDocumentation.includes('src=\\modelica://Buildings')) {
      htmlDocumentation = htmlDocumentation.replaceAll('src=\\modelica://Buildings', 'src=https://raw.githubusercontent.com/lbl-srg/modelica-buildings/refs/tags/v11.0.0/Buildings')
      htmlDocumentation = htmlDocumentation.replaceAll('.png\\\n', '.png\n')
    }
    if (htmlDocumentation.includes('href=\\modelica://Buildings')) {
      // TODO: handle references to self
    }

    if (htmlDocumentation.includes("<pre>")) {
      htmlDocumentation = htmlDocumentation.replaceAll('<pre>', '<pre style="background-color:white; font-size: 11px; padding-top: 4px; padding-bottom: 4px; color: maroon; font-family: \'Courier New\';">')
    }
    if (htmlDocumentation.includes("<code>")) {
      htmlDocumentation = htmlDocumentation.replaceAll('<code>', '<code style="background-color:white; font-size: 11px; padding-top: 4px; padding-bottom: 4px; color: maroon; font-family: \'Courier New\';">')
    }
    if (htmlDocumentation.includes('<a href=')) {
      htmlDocumentation = htmlDocumentation.replaceAll(/<a [^>]+>([\s\S]*?)<\/a>/g, (match, p1) => {
        if (p1.includes('Buildings.Controls.OBC.CDL')) {
          return 'CDL.'+p1.split('CDL.')[1]
        } else {
          return match
        }
      })
  ...
  var inputs = [], htmlInputs = []
  inputs = graph.each(s=blockNode, p=s231Ns('hasInput'), o=undefined)
  inputs.forEach(input => {
    const type = graph.each(s=input, p=rdfNs('type'), o=undefined)[0]['value'].split('#')[1].split('Input')[0]
    const description = graph.each(s=input, p=s231Ns('description'), o=undefined)[0]['value']
    htmlInputs = htmlInputs.concat([{
      'type': type,
      'name': input.value.split(blockNode.value + '.')[1],
      'description': description
    }])
  })

  var outputs = [], htmlOutputs = []
  outputs = graph.each(s=blockNode, p=s231Ns('hasOutput'), o=undefined)
  outputs.forEach(output => {
    const type = graph.each(s=output, p=rdfNs('type'), o=undefined)[0]['value'].split('#')[1].split('Output')[0]
    const description = graph.each(s=output, p=s231Ns('description'), o=undefined)[0]['value']
    htmlOutputs = htmlOutputs.concat([{
      'type': type,
      'name': output.value.split(blockNode.value + '.')[1],
      'description': description
    }])
  })

  var parameters = [], htmlParameters = []
  parameters = graph.each(s=blockNode, p=s231Ns('hasParameter'), o=undefined)
  parameters.forEach(parameter => {
    const type = graph.each(s=parameter, p=s231Ns('isOfDataType'), o=undefined)[0]['value'].split('#')[1]
    const description = graph.each(s=parameter, p=s231Ns('description'), o=undefined)[0]['value']
    htmlParameters = htmlParameters.concat([{
      'type': type,
      'name': parameter.value.split(blockNode.value + '.')[1],
      'description': description
    }])
  })
  
  var constants = [], htmlConstants = []
  constants = graph.each(s=blockNode, p=s231Ns('hasConstant'), o=undefined)
  constants.forEach(constant => {
    const type = graph.each(s=constant, p=s231Ns('isOfDataType'), o=undefined)[0]['value'].split('#')[1]
    const description = graph.each(s=constant, p=s231Ns('description'), o=undefined)[0]['value']
    htmlConstants = htmlConstants.concat([{
      'type': type,
      'name': constant.value.split(blockNode.value + '.')[1],
      'description': description
    }])
  })
  
  const htmlBlockName = 'CDL.'+cxfBlockName.split('CDL.')[1]
  var content = {'blockName': htmlBlockName, 'imagePath': imagePath, 'inputs': htmlInputs, 'outputs': htmlOutputs, 
                    'description': htmlDescription, 'documentation': htmlDocumentation, 'parameters': htmlParameters, 'constants': htmlConstants}
  return content
}

const parseGraph = (cxfGraphJsonld, graph, prefix, mimeType) => {
  return new Promise((resolve, reject) => {
    rdflib.parse(cxfGraphJsonld, graph, prefix, mimeType, (err) => {
      if (err) {
        return reject(new Error("Parsing error: " + err));
      }
      resolve(graph);
    });
  });
...
const processGraph = async (cxfGraphJsonld, prefix, mimeType, newPath, cxfBlockName, s231Ns, rdfNs, imagePathUrlPrefix) => {
  try {
    let graph = rdflib.graph()
    const parsedGraph = await parseGraph(cxfGraphJsonld, graph, prefix, mimeType);
    const blockNodeRes = parsedGraph.each(undefined, rdfNs("type"), s231Ns("ElementaryBlock"));

    if (blockNodeRes.length === 0) {
      const enumerationRes = parsedGraph.each(undefined, rdfNs("type"), s231Ns("EnumerationType"));
      if (enumerationRes.length > 0) {
        console.log("Enumeration. Not handled at the moment.");
        return [];
      } else {
        throw new Error(`No ElementaryBlocks defined in graph of ${newPath}.`);
      }
    } else if (blockNodeRes.length > 1) {
      throw new Error(`More than one ElementaryBlock defined in graph of ${newPath}.`);
    }

    const blockNode = blockNodeRes[0];
    const blockContent = getCxfSingleHtmlContent(parsedGraph, cxfBlockName, s231Ns, rdfNs, blockNode, imagePathUrlPrefix);
...
function getFunctionForEleBlock(blockOrdering, blockName) {
  let found = false
  let blockFunction = null
  for (let key in blockOrdering) {
    let blocks = blockOrdering[key]
...
function getCxfHtml(cdlFolder, directory) {
  if (directory === 'current') {
    directory = ''
  }
  const cxfFolder = ut.getOutputFile(cdlFolder, 'cxf', directory)
  const blockOrdering = JSON.parse(fs.readFileSync(path.join(__dirname, 'cdlEleBlocksOrdering.json'), 'utf-8'))
  const orderedHtmlPromises = []

  for (let key in blockOrdering) {
    orderedHtmlPromises.push([])
  }
  // one more empty list for all null elements -> 1st element for all nulls
  orderedHtmlPromises.push([])

  var cxfParsePromises = []
  
  function parseDirectory (folder, cxfParsePromises) {
    ...
    for(i=0; i<filesAndFolders.length; i++) {
      const fileAndFolder = filesAndFolders[i]
      const newPath = path.join(folder, fileAndFolder)
      const stat = fs.statSync(newPath)
      if (stat && stat.isDirectory()) {
        if (!newPath.endsWith('CDL'+path.sep+'Interfaces')) {
          cxfParsePromises = parseDirectory(newPath, cxfParsePromises)
        }
      } else {
        if (!newPath.endsWith('package.jsonld')) {
          //TODO: handle package.jsonld
          const cxfBlockName = newPath.split(directory+path.sep+'cxf'+path.sep)[1].split('.jsonld')[0].replaceAll(path.sep, '.')
          const imagePathUrlPrefix = "https://simulationresearch.lbl.gov/modelica/releases/v11.0.0/help/"

          const cxfGraphJsonld = fs.readFileSync(newPath, 'utf-8')
          const prefix = "https://example.org#"
          const mimeType = 'application/ld+json'
          const cdlBlockName = 'CDL.'+cxfBlockName.split('CDL.')[1]
          const cdlFunctionName = getFunctionForEleBlock(blockOrdering, cdlBlockName)

          if (cdlFunctionName !== null) {
            const functionIdx = Object.keys(blockOrdering).indexOf(cdlFunctionName)
            // functionIdx+1 as first element is for all nulls
            orderedHtmlPromises[functionIdx+1].push(processGraph(cxfGraphJsonld, prefix, mimeType, newPath, cxfBlockName, s231Ns, rdfNs, imagePathUrlPrefix))
          } else {
            // TODO: handle nulls
            orderedHtmlPromises[0].push(processGraph(cxfGraphJsonld, prefix, mimeType, newPath, cxfBlockName, s231Ns, rdfNs, imagePathUrlPrefix))
          }          
          // cxfParsePromises.push(processGraph(cxfGraphJsonld, prefix, mimeType, newPath, cxfBlockName, s231Ns, rdfNs, imagePathUrlPrefix))
        }
      }
    }
    return orderedHtmlPromises
    // return cxfParsePromises

  }
  cxfParsePromises = parseDirectory(cxfFolder, cxfParsePromises)
  Promise.all([].concat(...cxfParsePromises))
    .then((results) => {
      let htmlContents = [].concat(...results);
      var res = nj.render('lib/docHtmlTemplate.html', {'blocks': htmlContents});
      ut.writeFile(path.join(directory, 'cxf', 'cxfDoc.html'), res)
      const filePath = path.join(directory, 'cxf', 'cxfDoc.docx')

      var fileBuffer

      const promise = new Promise((resolve, reject) => {
        (async () => {
          const fileBuffer = await HTMLtoDOCX(res, null, {
    ...
    .catch((err) => {
      // console.error("Error in processing graphs:", err);
    })
}
...
module.exports.getCxfHtml = getCxfHtml
