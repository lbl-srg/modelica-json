    ...
    if (instanceType === 'long_class_specifier') {
      ...
      if (checkIfCdlElementaryBlock(within)) {
        ...
        if (generateCxfCore || generateElementary) {
          graph.add(s231Ns(blockNameWithPackage), rdfNs('type'), s231Ns('ElementaryBlock'))
          newInstanceDict.cxfNode = s231Ns(blockNameWithPackage)
        } else {
          newInstanceDict.cxfNode = s231Ns(blockNameWithPackage)
        }
      } else {
        // TODO: how to separate composite block v/s extension block
        graph.add(cxfPrefix(blockNameWithPackage), rdfNs('type'), s231Ns('Block'))
        newInstanceDict.cxfNode = cxfPrefix(blockNameWithPackage)
      }
...
function getInstanceNode (graph, element, newInstances, s231Ns, cxfPrefix) {
  ...
  if (element.includes('.')) {
    ...
    if (instance in newInstances) {
      ...
      const instancePackage = instanceNode.value.split('.').slice(0, -1).join('.')
      const prefix = instanceNode.prefix
      if (prefix === 's231') {
        node = s231Ns(`${instancePackage}.${instance}.${subInstance}`)
      } else {
        node = cxfPrefix(`${instancePackage}.${instance}.${subInstance}`)
      }
