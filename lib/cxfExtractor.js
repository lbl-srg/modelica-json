...
function getCxfSingleHtmlContent(graph, cxfBlockName, s231Ns, rdfNs, blockNode, imagePathUrlPrefix) {
  ...
  if (documentations.length === 1) {
    ...
    if (htmlDocumentation.includes("<pre>")) {
      htmlDocumentation = htmlDocumentation.replaceAll('<pre>', '<pre style="background-color:lightgrey; font-size: 16px; padding-top: 4px; padding-bottom: 4px; color: maroon; font-family: \'Courier New\';">')
    }
    if (htmlDocumentation.includes("<code>")) {
      htmlDocumentation = htmlDocumentation.replaceAll('<code>', '<code style="background-color:lightgrey; font-size: 16px; padding-top: 4px; padding-bottom: 4px; color: maroon; font-family: \'Courier New\';">')
    }
  }
  ...
  inputs.forEach(input => {
    const type = graph.each(s=input, p=rdfNs('type'), o=undefined)[0]['value'].split('#')[1]
  ...
  outputs.forEach(output => {
    const type = graph.each(s=output, p=rdfNs('type'), o=undefined)[0]['value'].split('#')[1]
  ...
  constants.forEach(constant => {
    const type = graph.each(s=constant, p=rdfNs('type'), o=undefined)[0]['value'].split('#')[1]
  ...
  var content = {'blockName': cxfBlockName, 'imagePath': imagePath, 'inputs': htmlInputs, 'outputs': htmlOutputs, 
