...
function parseModification (mod, instance, attributeList, typeSpecifier = '') {
  ...
  if (mod !== undefined && 'equal' in mod && mod.equal) {
    if ('simple_expression' in mod.expression) {
      ...
      // TODO: handle
      logger.warn('if expressions not well handled right now in cxf')
...
function getQudtUnit (value, unitNs, s231Ns) {
  ...
  if (value === undefined || value === null || value === '') {
    ...
    logger.error('Unit ' + value + ' not found in qudt. Using S231:(' + value + ')')
    return s231Ns(value)
  }
}
...
function getQudtQuanityKind (value, qudtQkNs) {
  if (value === undefined || value === null || value === '') {
    ...
    logger.error('Quantity kind ' + value + ' not allowed in CDL. ')
    return null
  }
}
