...
function checkIfCdlElementaryBlock (name) {
  console.log(name, name.split('.').slice(-2, -1)[0].toUpperCase())
  if (name !== undefined && name !== null && name.length > 0 && (name.split('.').slice(-2, -1)[0].toUpperCase() === 'CDL' 
        || (name.split('.').slice(-3, -2)[0].toUpperCase() === 'CDL') && (name.split('.').slice(-2, -1)[0].toUpperCase() === 'SOURCES'))) {
    return true
  }
  return false
}
...
function getCxfGraph (instances, requiredReferences, blockName, generateElementary, generateCxfCore) {
  ...
  console.log(instances, requiredReferences.connections, isElementaryBlock)
