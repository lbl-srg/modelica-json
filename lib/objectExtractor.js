  ...
  for (let i = 0; i < elementList.length; i++) {
    ...
    if ('component_clause' in element && element.component_clause !== undefined) {
      ...
      for (let i = 0; i < componentList.length; i++) {
        ...
        if ('array_subscripts' in singleComponentList.declaration) {
          isVector = true
          singleComponentList.declaration.array_subscripts.forEach(subscript => {
            if ('expression' in subscript) {
              if ('simple_expression' in subscript.expression) {
                singleComponentArraySubscripts += subscript.expression.simple_expression
              }
            }
          })
          if (singleComponentArraySubscripts !== '') {
            singleComponentArraySubscripts = '[' + singleComponentArraySubscripts + ']'
          }
        }
        instances[identifier] = {
 ...
 * @param {boolean} [stringifyExpression=false] - Whether to convert expression objects into Modelica expressions (strings).
 * @returns {Object} The parsed modifications as a dictionary.
 */
function parseModifications (modifications, stringifyExpression = false) {
  let outputDict = {}
        ...
        classModifications.forEach(classModification => {
          ...
          if (mod.expression !== undefined) {
            if (stringifyExpression) {
              eleDict[eleKey] = expressionToString.parse(mod.expression)
            } else {
