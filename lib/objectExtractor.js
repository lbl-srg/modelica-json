...
function getCxfGraph(instances, blockName, generateCxfCore=false) { 
  ...
  instancesList.forEach(instance => {
    ...
    if (instanceType === 'long_class_specifier') {
      ...
      if (within !== undefined && within.split('.').slice(-2,-1)[0] !== undefined && within.split('.').slice(-2,-1)[0].toUpperCase() === 'CDL') {
        if (generateCxfCore) {
      ...
      var className = instanceDict.long_class_specifier_identifier === undefined? blockName: instanceDict.long_class_specifier_identifier
      var instanceNode = cxfPrefix(`${className}#${instance}`)
      var blockNode = cxfPrefix(`${className}`)
      // console.log(instance, instanceDict)
      if (instanceDict.type_prefix === 'parameter') { 
        ...
        var typeSpecifier = instanceDict.type_specifier
        if (typeSpecifier.endsWith('IntegerInput') || typeSpecifier.endsWith('BooleanInput') || typeSpecifier.endsWith('RealInput')) {
          ...
          var inputConnectorType = typeSpecifier.split('.').slice(-1)[0]
          ...
          var outputConnectorType = typeSpecifier.split('.').slice(-1)[0]
          ...
          var cxfElementaryBlockName = ['CXF'].concat(typeSpecifier.split('.').slice(-2)).join('.')
