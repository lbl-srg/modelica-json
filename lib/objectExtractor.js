...
function extractFromElementList (elementList, longClassSpecifierIdentifier, fullMoFilePath, within, compositionSpecifier) {
  ...
  for (let i = 0; i < elementList.length; i++) {
    ...
    if ('class_definition' in element && element.class_definition !== undefined) {
      var classDefinitionInstance = {}
      var classSpecifier = element.class_definition.class_specifier
      var classPrefixes = element.class_definition.class_prefixes
      var identifier2 = null
      var dict = {
        'within': within,
        'fullMoFilePath': fullMoFilePath,
        'classPrefixes': classPrefixes,
        'compositionSpecifier': compositionSpecifier
      }
      if ('long_class_specifier' in classSpecifier) {
...
function getCxfGraph(instances, blockName) { 
  var instanceCategoryDict = {}
  var instancesList = []
  ...
  var CxfNs = rdflib.Namespace('seq:'+blockName+'#')
  var S231Ns = rdflib.Namespace('https://data.ashrae.org/S231P#')
  var RDFNs = rdflib.Namespace("http://www.w3.org/1999/02/22-rdf-syntax-ns#")

  const uri = blockName + '.jsonld'

  graph.add(CxfNs(blockName), RDFNs('type'), S231Ns('Block'))

  instancesList.forEach(instance => {
    var instanceDict = instances[instance]
    ...
    if (instanceType === 'long_class_specifier') {
      ...
      if (instanceDict.type_prefix === 'parameter') { 
        ...
        graph.add(CxfNs(blockName), S231Ns('hasParameter'), CxfNs(`${instance}`))
      }
      else if (instanceDict.type_prefix === '' || instanceDict.type_prefix === undefined) {
        if (instanceDict.type_specifier.endsWith('IntegerInput') || instanceDict.type_specifier.endsWith('BooleanInput') || instanceDict.type_specifier.endsWith('RealInput')) {
          graph.add(CxfNs(blockName), S231Ns('hasInput'), CxfNs(`${instance}`))
          graph.add(CxfNs(`${instance}`), RDFNs('type'), S231Ns('Input'))
        } else if (instanceDict.type_specifier.endsWith('IntegerOutput') || instanceDict.type_specifier.endsWith('BooleanOutput') || instanceDict.type_specifier.endsWith('RealOutput')) {
          graph.add(CxfNs(blockName), S231Ns('hasOutput'), CxfNs(`${instance}`))
          graph.add(CxfNs(`${instance}`), RDFNs('type'), S231Ns('Output'))
        }
      }
    } 
  })
  ...
  // TODO: fix below: save as jsonld
  try { 
    rdflib.parse(null, graph, uri, 'application/ld+json')
  }  catch (err) {
          console.log(err)
        }
