...
function parseComponentClause1 (componentClause1, isFinal) {
  const res = {}
  let typeSpecifier
  let componentDeclaration1
  let identifier
  if ('type_specifier' in componentClause1 && componentClause1.type_specifier !== undefined) {
    typeSpecifier = componentClause1.typeSpecifier
    res[typeSpecifier] = typeSpecifier
  }
  if ('component_declaration1' in componentClause1 && componentClause1.componen_declaration1 !== undefined) {
    componentDeclaration1 = componentClause1.component_declaration1
    if ('declaration' in componentDeclaration1 && componentDeclaration1.declaration !== undefined) {
      const declaration = componentDeclaration1.declaration
      if ('identifier' in declaration && declaration.identifier !== undefined) {
...
function parseExtendsClauseClassModifications (classModifications) {
  const res = {}
  const eleModifications = []
  const eleRedclarations = []

  if (classModifications !== null && classModifications !== undefined && classModifications.length > 0) {
    classModifications.forEach(classMod => {
      if (classMod !== undefined) {
        if ('element_redeclaration' in classMod && classMod.element_redeclaration !== undefined) {
          const eleRed = classMod.element_redeclaration

          let isFinal = false
          if ('final' in eleRed && eleRed.final !== undefined) {
            isFinal = eleRed.final
          }

          if ('component_clause1' in eleRed && eleRed.component_clause1 !== undefined) {
            const componentClause1 = eleRed.component_clause1

            const compClause1Object = parseComponentClause1(componentClause1)
            compClause1Object.isFinal = isFinal
            eleRedclarations.push(compClause1Object)
          }

          if ('element_replaceable' in eleRed && eleRed.element_replaceable !== undefined) {
            const eleRep = eleRed.element_replaceable
          }

          if ('element_modification_or_replaceable' in classMod && classMod.element_modification_or_replaceable !== undefined) {
            const eleModRep = classMod.element_modification_or_replaceable
          }
        } else if ('element_modification_or_replaceable' in classMod && classMod.element_modification_or_replaceable !== undefined) {
          const eleModRep = classMod.element_modification_or_replaceable
        }
      }
    })
...
function extractFromElementList (elementList, longClassSpecifierIdentifier, fullMoFilePath, within, compositionSpecifier) {
  ...
  for (let i = 0; i < elementList.length; i++) {
    ...
    if ('extends_clause' in element && element.extends_clause !== undefined) {
      ...
      if ('class_modification' in extendsClause && extendsClause.class_modification !== undefined) {
        parseExtendsClauseClassModifications(extendsClause.class_modification)
      }
