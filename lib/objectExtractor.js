 ...
 */
function parseModifications (modifications, parseExpression = false) {
  let outputDict = {}
      ...
      if (classModifications?.length > 0) {
        classModifications.forEach(classModification => {
          ...
          if (mod.expression !== undefined) {
            if (parseExpression) {
              eleDict[eleKey] = expressionParser.parse(mod.expression)
            } else {
              eleDict[eleKey] = mod.expression.simple_expression || mod.expression.if_expression
            }
          } else {
            eleDict[eleKey] = parseModifications(mod, parseExpression)
          }
          if (eleKey !== 'propagate') {
