...
const functions = [ // https://obc.lbl.gov/specification/cdl.html#functions
  ...
  'div', 'rem',
  'fill', 'size',
  'Array', 'parseInt', 'divide' // JS functions
]
...
function stringifyExpression (obj) {
  ...
  if (obj.if_expression) {
    ...
    return expressionToString.parse(JSON.parse(JSON.stringify(obj, (key, val) => { // We use the recursive algorithm of JSON.stringify
      if (val.not) return Object.assign(val,
        {
    ...
    for (const [idx, term] of Object.entries(exprToEvaluate)) {
      ...
      if (Array.isArray(termEvalSplit) && termEvalSplit.some(
        x => Object.keys(context).includes(x))) {
          exprToEvaluate[idx] = evalExpression(exprToEvaluate[idx], context, undefined, true)
