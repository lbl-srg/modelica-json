'use strict'
const expressionToString = require('../json2mo/expression')
 ...
 * @param {boolean} [stringifyExpression=true] - Whether to convert expression objects into Modelica expressions (strings).
 * @returns {Array} An array of components extracted from the class object.
 */
function getComponents (classObject, stringifyExpression = true) {
  const components = [];
  let annotation_cdl;
  let modification;
  let assignment;
    ...
    subset.forEach((element) => {
      ...
      element?.component_clause?.component_list.forEach((component) => {
        ...
        assignment = modification?.equal == null
          ? null
          : stringifyExpression ? expressionToString.parse(modification.expression) : modification.expression
 ...
 * @param {Object=} classObject - The class object or class definition containing the short class definition – only required for short class specifiers.
...
function getClassDefinition (jsons, type_specifier, classObject) {
  ...
  // FIXME: logger
  console.log(`Class definition not found for: ${type_specifier}`)
  return null
}
 ...
 * @param {{name: string, protected: boolean, condition: boolean|string, cdlAnnotation: Object}} [_instance=] - Instance data — used for recursive calls only.
 * @param {Object} [_bindings=] - The bindings object — used for recursive calls only.
 * @returns {{parameters: Object, documentation: Array<Object>}} - The parameters and documentation of the class object.
 *
