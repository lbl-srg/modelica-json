...
function propagateValue (classSet, className, insName, data, valueList) {
  ...
  for (var i = 0; i < classSet.length; i++) {
    ...
    if (tempSet.instantiate) {
      ...
      tempList1['name'] = enableParName
      tempList1['value'] = tempSet.instantiate
      tempList1['type'] = 'Boolean'
      valueList.push(tempList1)
    ...
    if (newPar) {
      newPar.forEach(function (obj) {
        ...
        tempList2['name'] = proParNam
        tempList2['value'] = obj.value
        tempList2['type'] = obj.type ? obj.type : obj.className
        valueList.push(tempList2)
      })
    }
    // evaluate class instantiations
