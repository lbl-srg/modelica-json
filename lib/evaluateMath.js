...
function evaluateParExp (data, evaExp) {
  ...
  var valueList = []
  valueList = propagateValue(newCla, data[0].topClassName, undefined, data, valueList)
  valueList = propagateValue(newInt, data[0].topClassName, undefined, data, valueList)
  data[0] = workDataSet[0]
...
function propagateValue (classSet, className, insName, data, valueList) {
  ...
  for (var i = 0; i < classSet.length; i++) {
    ...
    if (tempSet.instantiate) {
      ...
      // valueList[enableParName] = tempSet.instantiate
      tempList['name'] = enableParName
      tempList['value'] = tempSet.instantiate
      tempList['type'] = 'Boolean'
    ...
    if (newPar) {
      newPar.forEach(function (obj) {
        ...
        tempList['name'] = proParNam
        tempList['value'] = obj.value
        tempList['type'] = obj.type
        valueList.push(tempList)
      })
    }
    // evaluate class instantiations
