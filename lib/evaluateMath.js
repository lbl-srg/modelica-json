...
function evaluateParExp (data, evaExp) {
  ...
  // evaluate parameter sections (filtering out all falsy values from the array)
  ...
  // evaluate class instantiations (filtering out all falsy values from the array)
  ...
  // evaluate interface classes (filtering out all falsy values from the array)
...
function propagateValue (classSet, className, insName, data, valueList) {
  ...
  for (var i = 0; i < classSet.length; i++) {
    ...
    if (tempSet.instantiate) {
      ...
      addBooParameter(valueList, temBoo, null)
    ...
    if (newPar) {
      newPar.forEach(function (obj) {
        ...
        if (parType === 'Real' || parType === 'Integer') {
          addRealParameter(valueList, newObj, parType, newPar)
        } else if (parType === 'Boolean') {
          addBooParameter(valueList, newObj, newPar)
        } else {
    ...
    // all interfaces
    var inpCla = (workSet.public && workSet.public.inputs) ? workSet.public.inputs : null
    var outCla = (workSet.public && workSet.public.outputs) ? workSet.public.outputs : null
    // evaluate interface classes
    var allInt = [].concat(inpCla).concat(outCla).filter(function (obj) { return obj })
    if (allInt.length > 0) {
      var newInt = evaluateCla(allInt, newPar, workDataSet, true)
      propagateValue(newInt, tempSet.className, newInsNam, data, valueList)
    }
  }
}
 ...
 * @param newPar list of parameter object in which the value has been calculated
 */
function addBooParameter (valueList, temBoo, newPar) {
  var sortBoo = {}
  sortBoo['name'] = temBoo['name']
  sortBoo['value'] = temBoo['value']
  var attVal = evaluateAttributes(temBoo, 'Boolean', newPar)
  var sortData = Object.assign(sortBoo, attVal)
  if (valueList.Boolean) {
    valueList.Boolean.push(sortData)
  } else {
    var temp = []
    temp.push(sortData)
 ...
 *
 * @param valueList value propagation list
 * @param tempData object of parameter
 * @param parType parameter type, 'Real' or 'Integer'
 * @param newPar parameter object list
 */
function addRealParameter (valueList, tempData, parType, newPar) {
  var sortData = {}
  sortData['name'] = tempData['name']
  var patt = /((([a-zA-Z0-9]+).([a-zA-Z0-9]+)).)+/g
  var value = tempData['value']
  sortData['value'] = (parType === 'Integer' && value.match(patt)) ? value : Number(value)
  sortData['quantity'] = tempData['quantity']
  sortData['unit'] = tempData['unit']
  sortData['displayUnit'] = tempData['displayUnit']
  sortData['fixed'] = tempData['fixed']
  var attVal = evaluateAttributes(tempData, parType, newPar)
  var sortData1 = Object.assign(sortData, attVal)
  if (parType === 'Real') {
    if (valueList.Real) {
      valueList.Real.push(sortData1)
    } else {
      var temp1 = []
      temp1.push(sortData1)
      valueList['Real'] = temp1
    }
  } else {
    if (valueList.Integer) {
      valueList.Integer.push(sortData1)
    } else {
      var temp = []
      temp.push(sortData1)
      valueList['Integer'] = temp
    }
  }
}

/**
 * Evaluete the attribute value
 *
 * @param dataObj data object that includes all the attributes
 * @param dataType data type, 'Real', 'Integer', 'Boolean'
 * @param allPar parameters that have been evaluated
 */
function evaluateAttributes (dataObj, dataType, allPar) {
  // evaluate expressions for the interface class connector
  var startObj = dataObj.start
  var minObj = dataObj.min
  var maxObj = dataObj.max
  var nominalObj = dataObj.nominal
  var attributes = {}
  if (startObj) {
    var startValue = expressionValue(dataType, startObj.value, allPar, true)
    attributes['start'] = dataType === 'Boolean' ? startValue : Number(startValue)
  }
  if (minObj) {
    attributes['min'] = Number(expressionValue(dataType, minObj.value, allPar, true))
  }
  if (maxObj) {
    attributes['max'] = Number(expressionValue(dataType, maxObj.value, allPar, true))
  }
  if (nominalObj) {
    attributes['nominal'] = Number(expressionValue(dataType, nominalObj.value, allPar, true))
  }
  return attributes
}
...
function addEnumParameter (valueList, parType, tempEnum) {
  ...
  if (parType === 'String') {
    if (valueList.String) {
      valueList.String.push(sortEnum)
    } else {
      var temp1 = []
      temp1.push(sortEnum)
      valueList['String'] = temp1
    }
  } else {
...
function evaluate (expression) {
  ...
  return expression.includes('max(') ? math.evaluate(expression) : func()
}
...
function evaluateCla (allMod, allPar, data, evaExp) {
  ...
  filMod.forEach(function (obj) {
    ...
    if (!claNam.includes('CDL.Interfaces')) {
      // evaluate the parameters in class instantiate
...
function evaluatePar (allPar) {
  ...
  allPar.forEach(function (obj) {
    ...
    var type = obj.type ? obj.type : obj.className
    if (!obj.value) {
      const msg = 'No value is assigned to the parameter ' + obj.name
      logger.error(msg)
      throw new Error(msg)
    }
    if (!checkType(type, obj.value)) {
...
/**
 * Return the Modelica constant
 *
 * @param constantStr constant string
 */
function modelicaConstants (constantStr) {
  if (constantStr.includes('Constants.eps')) {
    return 1E-15
  } else if (constantStr.includes('Constants.small')) {
    return 1E-60
  } else {
