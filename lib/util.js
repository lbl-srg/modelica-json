...
// Get the output json path from modelica file full path
function getJsonPathFromMoPath (moFilPat, outDir, outputFormat) {
  const MODELICAPATH = getMODELICAPATH()
  var moPath
  for (var i = 0; i < MODELICAPATH.length; i++) {
    moPath = MODELICAPATH[i]
    if (moFilPat.includes(moPath)) {
      break
    }
  }
  const parPath = path.join(outDir, outputFormat)
  const replacedPath = moFilPat.replace(moPath, parPath)
  const len = replacedPath.length
  const withoutExt = replacedPath.substr(0,len-3)
...
module.exports.getJsonPathFromMoPath = getJsonPathFromMoPath
