...
function searchPath (claLis, within, sourceMoFile) {
  var MODELICAPATH = getMODELICAPATH()
  ...
  for (var i = 0; i < claLis.length; i++) {
    ...
    if (fulPat) {
      ...
      if (sourceMoFile !== undefined && sourceMoFile !== null) {
        var folders = sourceMoFile.split('.mo')[0].split(path.sep)
        let j = folders.length - 1
        while (j > 0) {
          var possiblePath = path.join(folders.slice(0, j).join(path.sep), ithCla + '.mo')
