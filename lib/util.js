...
// Convert svg or png in base64png and store in a txt file
function convertbase64 (listofimg) {
  listofimg.forEach(function (element) {
    format = element.substr(element.length - 3)
    var base64FileName = element.replace(format, 'txt')
    if (format==='svg'){
      fs.readFile(element, 'utf-8', function (err, svgfile) {
        (async () => {
          const image = await svgToImg.from(svgfile).toPng({
            encoding: "base64"
          });
          base64graphics = 'data:image/png;base64,' + image
          fs.writeFileAsync(base64FileName, base64graphics).then(
            function (res) {
              logger.debug('Wrote ', base64FileName)
              return true
            })
        })();
      })
      ...
      var base64graphics = base64Img.base64Sync(element)
      fs.writeFile(base64FileName, base64graphics, (err) => {
...
function getbase64data (listofimg) {
  console.log('getbase');
  var images = []
  listofimg.forEach(function (element) {
    var format = element.substr(element.length - 3)
    var srcelement = element.substring(5)
    re = new RegExp(/(png|svg)/gm)
    if (format==='svg'| format==='png') {
      base64file = element.replace(re, 'txt')
      base64read = readbase64(base64file)
      images.push(srcelement)
      images.push(base64read)
    }
  })
  return Promise.all(images)
}

  function readbase64 (imgpath) {
    return fs.readFileAsync(imgpath, 'utf8')
  }
...
module.exports.getbase64data = getbase64data
