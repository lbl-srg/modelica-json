...
function getMODELICAPATH () {
  const spe = process.platform === 'win32' ? ';' : ':'
  ...
  if (files.endsWith('.mo')) {
    ...
    for (let j = 0; j < MODELICAPATH.length; j++) {
      ...
      for (let k = 0; k < filesInMoPath.length; k++) {
        ...
        if (fileInMoPath.endsWith(files)) {
          found = true
    ...
    if (!fse.existsSync(tempFolder)) {
      for (let i = 0; i < MODELICAPATH.length; i++) {
        ...
        for (let k = 0; k < foldersInMoPath.length; k++) {
          ...
          if (folderInMoPath.endsWith(files)) {
            found = true
...
/**
 * Get list of files and folders in folder dir
 *
 * @param dir directory path
 * @param filesFoldersList already retrieved list of files and folders
 */
function getFilesAndFolders (dir, filesFoldersList) {
  filesFoldersList = filesFoldersList || []
  const filesAndFolders = fs.readdirSync(dir)
  filesAndFolders.forEach(fileAndFolder => {
    const newPath = path.join(dir, fileAndFolder)
    const stat = fs.statSync(newPath)
    if (stat && stat.isDirectory()) {
      filesFoldersList.push(newPath)
      filesFoldersList = getFilesAndFolders(newPath, filesFoldersList)
    } else {
      filesFoldersList.push(newPath)
    }
  })
