...
function valuestoreplace (listofimg) {
  var valuestoreplace = {}
    listofimg.forEach(function (element) {
      format = element.substr(element.length - 3)
      re = new RegExp(/(png|svg)/gm)
      if (format==='svg'|format==='png'){
        srcelement = element.substring(5)
        base64file = element.replace('svg', 'txt')
        base64data = fs.readFileSync(base64file)
        valuestoreplace[srcelement] = base64data.toString('base64')
    }
})
...
module.exports.valuestoreplace = valuestoreplace
