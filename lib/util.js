...
/** Get input mo file array.
  ...
  */
function getMoFiles (parseMode, files, outputFormat) {
  var moFiles = []
  const MODELICAPATH = getMODELICAPATH()
  if (parseMode === 'modelica') {
    ...
    if (!fse.existsSync(tempFolder)) {
      ...
      moFiles = fileList.filter(function (obj) {
        if (files.includes(cdlPath) && (outputFormat === 'docx' || outputFormat === 'html') && (obj.includes('Validation') || obj.includes('Example'))) {
          return false
        } else {
