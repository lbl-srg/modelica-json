...
function searchPath (claLis, within) {
  ...
  for (var i = 0; i < claLis.length; i++) {
    ...
    const claRelPath = ithCla.replaceAll('.', pathSep)
...
function searchFullPath (claRelPath, MODELICAPATH, within) {
  const withEle = within.split('.')
  var fullPath
  MODELICAPATH.forEach(function (moPath) {
    const tempPathes = joinWithinPath(moPath, withEle)
    var tempPath
    for (var i = 0; i < tempPathes.length; i++) {
      tempPath = path.join(tempPathes[i], claRelPath)
      if (fse.existsSync(tempPath)) {
        fullPath = tempPath
        break
      }
...
module.exports.removeTempDir = removeTempDir
