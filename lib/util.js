  ...
  if (parseMode === 'cdl') {
    ...
    if (outputFormat === 'raw-json') {
      if (jsonDataSet[0].within === undefined) {
        ...
        var topClaNam = (jsonDataSet[0].modelicaFile).split('/')[(jsonDataSet[0].modelicaFile).split('/').length - 2]
        if (topClaNam === '.') {
          var newTopClaNam = dirCur.split('/')[dirCur.split('/').length - 1]
        } else {
          var newTopClaNam = topClaNam
        }
        if (moName === 'package') {
