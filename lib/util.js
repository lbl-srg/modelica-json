 ...
 * element. Elements are separated by ':' for unix-based platforms and ; for windows
 */
function getMODELICAPATH () {
  const sep = process.platform.startsWith('win') ? ';' : ':'
  const moPath = process.env.MODELICAPATH
...
function searchPath (claLis, within, sourceMoFile) {
  ...
  for (let i = 0; i < claLis.length; i++) {
    const ithCla = claLis[i].replaceAll('.', os.sep)
