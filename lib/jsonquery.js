...
/** Get the simplified json (not raw) representation for the model
  ...
  */
function simplifyModelicaJSON (model, parseMode, cdlDoc) {
  ...
  const proParameters = !cdlDoc
    ? this.getParameters(model, 'protected', parseMode)
    : null
  ...
  const proInputs = !cdlDoc
    ? (isElementaryCDL(className)
  ...
  const proModels = !cdlDoc ? this.getModels(model, 'protected') : null
  ...
  // Get connections
  var connections = !cdlDoc ? this.getConnections(model) : null
