...
function simplifyModelicaJSON (model) {
  ...
  var info
  if (patToAnn && patToAnn.documentation && patToAnn.documentation.info) {
    info = patToAnn.documentation.info
    // Cut quotes
    info = info.slice(1, -1)
...
function getComment (model) {
  if (model.class_definition) {
    return model.class_definition[0].class_specifier.long_class_specifier.comment
  }
  return undefined
}
/**
...
function getClassName (model) {
  if (model && model.within && model.within.length === 1) {
    return model.within[0] + '.' + model.class_definition[0].class_specifier.long_class_specifier.name
  } else if (model && model.class_definition) {
    return model.class_definition[0].class_specifier.long_class_specifier.name
  } else {
...
function updateHtml (data) {
  if (data.comment) {
    data['html'] = { 'comment': data.comment.replace(/"/g, '') }
  }
  if (data.inputs) {
