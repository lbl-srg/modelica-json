...
/** Get the simplified json representation for the model
  */
function simplifyModelicaJSON (model, parseType) {
  ...
  if (proInputs.length !== 0 && !isElementaryCDL(className) && parseType === 'cdl') {
  ...
  if (proOutputs.length !== 0 && parseType === 'cdl') {
  ...
  if (parseType === 'cdl' && extModels.length !== 0) {
  ...
  if (connections) {
    addConnectorTargets(connections, allModels, parseType)
  }
  var data = {
...
/** Return 'connectors' with the element `linkTarget` added to all
    ports
*/
function addConnectorTargets (connectors, models, parseType) {
