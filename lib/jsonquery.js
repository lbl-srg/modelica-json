...
function simplifyModelicaJSON (model, parseMode) {
  ...
  // console.log(extModels)
  // var extendedMoFiles = []
  // for (var extModelIndex in extModels){
  //   var extModel = extModels[extModelIndex]
  //   var name = extModel.className
  //   if (name.startsWith("Buildings")){
  //     name = name.replace(/\./g,'/')
  //     name = name.substring(10)
      
  //     name = name.substring(0, name.lastIndexOf('/'))
  //     extendedMoFiles = extendedMoFiles.concat(ut.getMoFiles(parseMode, name))
  //   }  
  // }
  // console.log(extendedMoFiles)
  // for (var extendedMoFile in extendedMoFiles) {
  //   const jsonContent = pa.getRawJSON(extendedMoFile, parseMode)
  //   const 
  // }
...
function getItemName(dataElement, connector) {
  var allModels = null
  const havePublic = dataElement.public && dataElement.public.models
  const haveProtected = dataElement.protected && dataElement.protected.models
  const haveExtends = dataElement.extends
  if (havePublic && haveProtected && haveExtends) {
    allModels = dataElement.public.models.concat(dataElement.protected.models)
      .concat(dataElement.extends)
  } else if (havePublic && haveProtected && !haveExtends) {
    allModels = dataElement.public.models.concat(dataElement.protected.models)
  } else if (havePublic && haveExtends && !haveProtected) {
    allModels = dataElement.public.models.concat(dataElement.extends)
  } else if (haveProtected && haveExtends && !havePublic) {
    allModels = dataElement.protected.models.concat(dataElement.extends)
  } else if (havePublic && !haveProtected && !haveExtends) {
    allModels = dataElement.public.models
  } else if (haveProtected && !havePublic && !haveExtends) {
    allModels = dataElement.protected.models
  } else if (haveExtends && !havePublic && !haveProtected) {
    allModels = dataElement.extends
  }
...
function orderConnections (data) {
  ...
  for (var dataElementIndex in data) {
    var dataElement = data[dataElementIndex]

    console.log("***********************************************************************")
    if ('connections' in dataElement) {
      var connections = dataElement.connections
      for (var connectionIndex in connections) {
        var connection = connections[connectionIndex]
        const connector = connection[0]
        var foundOutput = false
        // ////////////////////////////////////////////////////////////
        if (connector.connector === undefined) {
          // This is an input or an output of this class, rather than of
          // an instance of this class.
          if (dataElement.outputs) {
            // Instance has outputs
            foundOutput = dataElement.outputs.some(hasName, connector.class)
          }
        } else {
          var instance
          for (var dataElementIndex2 in data) {
            var dataElement2 = data[dataElementIndex2]
            instance = getItemName(dataElement2, connector)
            if (typeof instance === 'undefined') {
              console.log("case 2 ********************************************************************************************instance == "+instance+ " connector === " + connector.connector)
            }
            else {
              break
            }
          ...
          console.log("case 1********************************************************************************************instance == "+instance+ " connector === " + connector.connector)
        }
      }
    }
    console.log("***********************************************************************")
