...
function elementModification (eleMod) {
  ...
  const modObj = mod ? this.modification(mod) : undefined
  const desStr = this.trimDesString(eleMod.string_comment)
  if (isGraAnn) {
    const graMod = mod ? graPri.graphicAnnotationObj(name, modObj) : undefined
    return Object.assign(
      { [name]: graMod }
    )
  } else {
...
function ifExpressionObj (pri) {
  ...
  for (let i = 0; i < expLis.length; i++) {
    ...
    for (let j = 0; j < ifEls.length; j++) {
      ...
      const condition = this.expressionString(jthEle.condition, true)
      const then = this.expressionString(jthEle.then, true)
    ...
    expArray.push(Object.assign(
      ...
      { else: this.expressionString(elsExp, true) }
    ))
  }
...
function forIndObj (forIndObject) {
  ...
  for (let i = 0; i < ind.length; i++) {
    ...
    const expString = exp ? this.expressionString(exp, true) : undefined
...
function arrSubString (arrSub) {
  ...
  for (let i = 0; i < sub.length; i++) {
    ...
    const ithSubString = colOp ? ':' : this.expressionString(exp, true)
...
function outExpLisString (outExpLis) {
  ...
  try {
    for (let i = 0; i < expList.length; i++) {
      arr.push(this.expressionString(expList[i], true))
    }
    return arr.join(',')
  } catch (e) {
...
function expressionListString (expList) {
  ...
  if (ithListEle) {
    for (let j = 0; j < ithListEle.length; j++) {
      arr.push(this.expressionString(ithListEle[j], true))
    }
...
function forIndString (forInd) {
  ...
  for (let i = 0; i < ind.length; i++) {
    ...
    const ident = ithInd.identifier
    const exp = ithInd.expression
    const ithIndString = exp ? (ident + ' in ' + this.expressionString(exp, true)) : ident
...
function ifExpString (ifExp) {
  ...
  let ifExpString = 'if ' + this.expressionString(ifElse[0].condition, true) + ' then ' + this.expressionString(ifElse[0].then, true)
  if (ifElse.length > 1) {
    for (let i = 1; i < ifElse.length; i++) {
      const ithEle = ifElse[i]
      const elseifString = ' elseif ' + this.expressionString(ithEle.condition, true) + ' then ' + this.expressionString(ithEle.then, true)
      ifExpString = ifExpString + elseifString
    }
  }
  return (ifExpString + ' else ' + this.expressionString(elsExp, true))
}
