...
function addPointlist (jsonDataSet) {
  ...
  Object.assign(entryData, pointList)
  ...
  allCla.forEach(ele => {
    ...
    if (insCDL) {
      ...
      if (insCDL.generatePointlist && insCDL.generatePointlist === 'true') {
        ...
        Object.assign(eleObj, innPointList)
      }
    }
  })
}
...
function generatePointlist (classPath, instanceName, jsonDataSet) {
  ...
  if (!instanceName.includes('.')) {
    ...
    if (!innClassObj.pointList) {
      ...
      // Add point list to the entry object
      Object.assign(innClassObj, pointList)
    }
  } else {
...
function propagateConnector (claInstance, connectorSpe, jsonDataSet) {
  ...
  if (!instanceName.includes('.')) {
    ...
    // Add point list to the entry object
    Object.assign(classObj, pointList)
  } else {
    var names = instanceName.split('.')
...
function createPointlist (classObj) {
  ...
  pubInt.forEach(ele => {
    ...
    var connectionSpe = (ele.__cdl && ele.__cdl.connection) ? ele.__cdl.connection : null
    point['hardwired'] = connectionSpe ? connectionSpe[0].hardwired : 'false'
    var trendSpe = (ele.__cdl && ele.__cdl.trend) ? ele.__cdl.trend : null
