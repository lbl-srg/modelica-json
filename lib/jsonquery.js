  ...
  if (argLis) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      claModArr.push(Object.assign(
        { element_modification_or_replaceable: eleModRep ? this.elementModificationReplaceable(eleModRep, isGraAnn) : undefined },
        { element_redeclaration: eleRed ? this.elementRedeclaration(eleRed, isGraAnn) : undefined }
      ))
    }
    return claModArr
  } else {
 ...
 */
function arithmeticExpression (ariExp, isGraAnn=false) {
  const termList = ariExp.arithmetic_term_list
  const termArray = []
  for (let i = 0; i < termList.length; i++) {
    const addOp = termList[i].add_op
    const term = this.termString(termList[i].term, isGraAnn)
 ...
 */
function termString (ter, isGraAnn=false) {
  const factor = ter.factors
  const mulOps = ter.mul_ops
  let temp = this.factorString(factor[0], isGraAnn)
  if (mulOps) {
    for (let i = 0; i < mulOps.length; i++) {
      temp = temp + mulOps[i] + this.factorString(factor[i + 1], isGraAnn)
    }
  }
  return temp
}
...
function term (ter) {
  ...
  if (hasObject) {
    return Object.assign({
      operators: mulOps.length > 0 ? mulOps : undefined,
      factors: factorEles
    })
  } else {
 ...
 */
function factorString (fac, isGraAnn = false) {
  const pri1Str = this.primary(fac.primary1, isGraAnn)
  const op = fac.op
  if (op && op !== '') {
    return (pri1Str + op + this.primary(fac.primary2, isGraAnn))
  } else {
    return pri1Str
  }
}
