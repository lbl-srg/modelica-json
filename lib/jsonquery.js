    ...
    if (model.class_definition[0].class_specifier.short_class_specifier) {
      ...
      const jsonClassNames = elements.filter(function (obj) {
        if (prefix === 'extends') {
          ...
          if (obj.replaceable) {
            if (obj.replaceable_component_clause) {
              if (obj.replaceable_component_clause.type_prefix === 'parameter') {
                return false
              } else {
                return isModel(obj.replaceable_component_clause.type_specifier)
              } 
            } else {
              return false
            }
      ...
      jsonClassNames.forEach(function (obj) {
        if (obj.replaceable) {
          const declarations = (prefix === 'extends')
            ...
            : parseComponentDeclaration(obj.replaceable_component_clause.component_list.component_declaration[0], currentClass)
          const condition = (prefix === 'extends')
            ? null
            : parseComponentCondition(obj.replaceable_component_clause.component_list.component_declaration[0])
          const annotation = (prefix === 'extends')
            ? null
            : parseComponentAnnotation(obj.replaceable_component_clause.component_list.component_declaration[0], currentClass)
          const entry = Object.assign(
            { 'prefix': 'replaceable', 
              'className': (prefix === 'extends') ? obj.extends_clause.name
              : obj.replaceable_component_clause.type_specifier}, declarations, condition, annotation)
          classes.push(entry)
        } else {
          const declarations = (prefix === 'extends')
            ? parseExtendsClause(obj)
    ...
    if (model.class_definition[0].class_specifier.short_class_specifier) {
      ...
      const jsonPars = elements.filter(function (obj) {
        ...
        if (obj.extends_clause) {
          return false
        } else if (obj.replaceable){
          return obj.replaceable_component_clause &&
                 obj.replaceable_component_clause.type_prefix === 'parameter'
        } else {
      ...
      jsonPars.forEach(function (obj) {
        ...
        let className;
        if (obj.replaceable) {
          declarations = parseComponentDeclaration(obj.replaceable_component_clause.component_list.component_declaration[0], currentClass)
          annotation = parseParameterAnnotation(obj.replaceable_component_clause.component_list.component_declaration[0], currentClass)
          className = obj.replaceable_component_clause.type_specifier
        } else {
          declarations = parseComponentDeclaration(obj.component_clause.component_list.component_declaration[0], currentClass)
          annotation = parseParameterAnnotation(obj.component_clause.component_list.component_declaration[0], currentClass)
