...
function composition (com) {
  ...
  const eleSec = eleSecObj !== undefined && (eleSecObj.length > 0) ? this.elementSections(com.element_sections) : undefined
...
function elementList (eleLis) {
  ...
  const eleArr = []
  if (ele === undefined) { return eleArr }
...
function arraySubscripts (arrSub) {
  ...
  if (subscripts === undefined) { return eleArr }
 ...
 */
function expression (expObj, isGraAnn = false) {
  ...
  return Object.assign(
    { simple_expression: simExp ? this.simpleExpression(simExp, false, isGraAnn) : undefined },
...
function componentList (comLis) {
  ...
  const decLisArr = []
  if (compList === undefined) { return decLisArr }
 ...
 */
function modification (mod, isGraAnn = false) {
  const claMod = mod.class_modification
  const equ = mod.equal
  const colEqu = mod.colon_equal
  const exp = mod.expression
  return Object.assign(
    { class_modification: claMod ? this.classModification(claMod, isGraAnn) : undefined },
    { equal: equ || undefined },
    { colon_equal: colEqu || undefined },
    { expression: exp ? this.expression(exp, isGraAnn) : undefined }
  )
}
 ...
 */
function classModification (claMod, isGraAnn = false) {
 ...
 */
function elementModificationReplaceable (eleModRep, isGraAnn = false) {
  ...
  return Object.assign(
    { each: each || undefined },
    { final: final || undefined },
    { element_modification: eleMod ? this.elementModification(eleMod, isGraAnn) : undefined },
 ...
 */
function elementModification (eleMod, graAnn = false) {
  const name = eleMod.name ? this.nameString(eleMod.name) : undefined
  const checkGraAnn = name ? graPri.isGraphicAnnotation(name) : undefined
  const isGraAnn = checkGraAnn || graAnn
  ...
  if (checkGraAnn) {
    const graMod = mod ? graPri.graphicAnnotationObj(name, this.modification(mod, isGraAnn)) : undefined
    return Object.assign(
      { [name]: graMod }
    )
  } else {
    ...
    return Object.assign(
      { name },
      { modification: mod ? this.modification(mod, isGraAnn) : undefined },
      { description_string: desStr }
    )
  }
}
...
function elementSections (eleSec) {
  const secArr = []
  if (eleSec === undefined) { return secArr }
...
function componentReference (comRef) {
  ...
  const parArr = []
  if (comPar === undefined) { return parArr }
  for (let i = 0; i < comPar.length; i++) {
    const obj = comPar[i]
    const arrSub = obj.array_subscripts
    parArr.push(Object.assign(
      { dot_op: obj.dot_op },
      { identifier: obj.identifier != null ? obj.identifier : undefined },
      { array_subscripts: arrSub ? this.arraySubscripts(arrSub) : undefined }
    ))
  }
  return parArr
...
function whenEquation (wheEqus) {
  ...
  const wheEquArr = []
  if (wheEqu === undefined) { return wheEquArr }
...
function namedArguments (namArg) {
  ...
  const namArgArr = []
  if (args === undefined) { return namArgArr }
  for (let i = 0; i < args.length; i++) {
    const obj = args[i]
...
function forIndices (forInd) {
  ...
  const indArr = []
  if (indLis === undefined) { return indArr }
...
function whenStatement (wheStas) {
  ...
  const wheStaArr = []
  if (wheSta === undefined) { return wheStaArr }
 ...
 */
function simpleExpression (simExp, outStr = false, isGraAnn = false) {
  ...
  if (!(logExp2 || logExp3)) {
    ...
    logExp = this.logicalExpressionObj(simExp.logical_expression1, isGraAnn)
  }
  if ((funCal || forLoo || logExp || ifExp) && !outStr) {
    if (funCal || forLoo || ifExp) {
      return Object.assign(
    ...
    if (logExp && !(funCal || forLoo || ifExp)) {
      ...
      if (logOr.length === 1 && logOr[0].logical_and.length === 1 &&
        ...
        if (isGraAnn) {
          ...
          const ariExp = logOr[0].logical_and[0].arithmetic_expressions[0]
          if (typeof ariExp !== 'object') {
            return ariExp
          }
...
function ifExpressionObj (pri) {
  ...
  const expArray = []
  if (expLis === undefined) { return undefined }
  for (let i = 0; i < expLis.length; i++) {
    ...
    for (let j = 0; j < ifEls.length; j++) {
      ...
      const condition = this.expression(jthEle.condition)
 ...
 */
function logicalExpressionObj (logExp, isGraAnn = false) {
  ...
  for (let i = 0; i < termList.length; i++) {
    ...
    for (let j = 0; j < factorList.length; j++) {
      ...
      const jthFactorObj = this.logicalFactorObj(jthFactor, isGraAnn)
      if (jthFactorObj) {
 ...
 */
function logicalFactorObj (fac, isGraAnn = false) {
  const notOpe = fac.not
  const rel = fac.relation
  const relOp = rel.rel_op
  const ariExpArray = []
  if (isGraAnn) {
    ariExpArray.push(Object.assign({ name: this.arithmeticExpression(rel.arithmetic_expression1, isGraAnn) }))
    if (relOp) {
      ariExpArray.push(Object.assign({ name: this.arithmeticExpression(rel.arithmetic_expression2, isGraAnn) }))
    }
  } else {
    const ariExp1 = this.arithmeticExpressionObj(rel.arithmetic_expression1)
    ariExpArray.push(ariExp1)
    if (relOp) {
      const ariExp2 = this.arithmeticExpressionObj(rel.arithmetic_expression2)
      ariExpArray.push(ariExp2)
    }
  }
  return Object.assign(
...
/**
 * Get the string or object of arithmetic_expression
 *
 * @param ariExp arithmetic_expression object
 */
function arithmeticExpressionObj (ariExp) {
  const termList = ariExp.arithmetic_term_list
  const terms = []
  const addOps = []
  for (let i = 0; i < termList.length; i++) {
    ...
    terms.push(this.term(termList[i].term))
  }
  /* Check if there is any term returns as an object. */
  const hasObject = terms.some(ele => typeof ele === 'object')
  if (hasObject) {
    /* return object */
    return Object.assign({
      addOps: addOps.length > 0 ? addOps : undefined,
      terms
    })
  } else {
    /* return string */
    ...
    if (addOps.length === terms.length) {
      /* terms array has same number of elements as addOps. */
      for (let j = 0; j < addOps.length; j++) {
        termArray.push(addOps[j] + terms[j])
      }
    } else {
      /* terms array has one more element than addOps. */
      termArray.push(terms[0])
      for (let k = 0; k < addOps.length; k++) {
        termArray.push(addOps[k] + terms[k + 1])
      }
    }
    return termArray.join(' ')
  }
...
function termString (ter) {
  ...
  let temp = this.factorString(factor[0])
...
/**
 * Get the string or object of term
 *
 * @param ter term object
 */
function term (ter) {
  const factors = ter.factors
  const mulOps = ter.mul_ops
  const factorEles = []
  factorEles.push(this.factor(factors[0]))
  if (mulOps && mulOps.length > 0) {
    for (let i = 0; i < mulOps.length; i++) {
      factorEles.push(this.factor(factors[i + 1]))
    }
  }
  const hasObject = factorEles.some(ele => typeof ele === 'object')
  if (hasObject) {
    return Object.assign({
      operators: mulOps && mulOps.length > 0 ? mulOps : undefined,
      factors: factorEles
    })
  } else {
    let temp = factorEles[0]
    if (mulOps && mulOps.length > 0) {
      for (let i = 0; i < mulOps.length; i++) {
        temp = temp + mulOps[i] + factorEles[i + 1]
      }
    }
    return temp
  }
...
function factorString (fac) {
  const pri1Str = this.primary(fac.primary1, true)
  const op = fac.op
  if (op && op !== '') {
    return (pri1Str + op + this.primary(fac.primary2, true))
  } else {
    return pri1Str
  }
}
...
/**
 * Get the string or object of factor
 *
 * @param fac factor object
 */
function factor (fac) {
  const pri1 = this.primary(fac.primary1)
  const op = fac.op
  if (op && op !== '') {
    const pri2 = this.primary(fac.primary2)
    if (typeof pri1 === 'object' || typeof pri2 === 'object') {
      return Object.assign({
        operator: op,
        primary1: pri1,
        primary2: pri2
 ...
 * @param pri primary object
 */
function primary (pri, isGraAnn = false) {
  const unsNum = pri.unsigned_number
  const priStr = pri.primary_string
  const isFalse = pri.is_false
  const isTrue = pri.is_true
  ...
  if (unsNum === 0) {
    ...
    if (isGraAnn) {
      ...
      return this.outExpLisObj(outExpLis)
    }
...
function nameString (name) {
  ...
  let nameString = ''
  if (nameList === undefined) { return nameString }
...
function comRefString (comRef) {
  ...
  let comString = ''
  if (comRefPar === undefined) { return comString }
...
/**
 * Get the object of output_expression_list
 *
 * @param outExpLis output_expression_list object
 */
function outExpLisObj (outExpLis) {
  const expList = outExpLis.output_expressions
  const arr = []
  try {
    for (let i = 0; i < expList.length; i++) {
      arr.push(this.expression(expList[i]))
    }
    return arr
  } catch (e) {
    throw new Error('missing out_exp_lis')
...
function expressionString (exp, outStr = false) {
  ...
  return simExp ? this.simpleExpression(simExp, true, true) : this.ifExpString(ifExp)
}
...
module.exports.arithmeticExpressionObj = arithmeticExpressionObj
...
module.exports.outExpLisObj = outExpLisObj
