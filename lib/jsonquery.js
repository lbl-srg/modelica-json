...
function simplifyModelicaJSON (model, parseMode) {
  ...
  var data = {
    ...
    'comment': getComment(model, className),
 ...
 */
function getComment (model, className) {
  var classComment
  if (model.class_definition[0].class_specifier.short_class_specifier) {
    classComment = getProperty(['class_definition', 0, 'class_specifier', 'short_class_specifier', 'comment', 'string_comment'], model)
    return getProperty(['class_definition', 0, 'class_specifier', 'short_class_specifier', 'comment', 'string_comment'], model)
  } else {
    classComment = getProperty(['class_definition', 0, 'class_specifier', 'long_class_specifier', 'comment'], model)
  }
  if (classComment === null) {
    logger.warn(className + ' does not have class comment.')
    pa.warnCounter = pa.warnCounter + 1
    return null
  }
  return classComment.replace(/"/g, '')
}
...
function parseComponentDeclaration (declaration, currentClass, insType) {
  ...
  var ret = (declaration.className)
    ...
    : {
      ...
      'modifications': (insType === 'class') ? parseParameterAssignments(declaration.declaration.class_modification) : null
    }
