...
function simplifyModelicaJSON (model, parseMode) {
  ...
  if ((parseMode === 'cdl' || className.includes('Buildings.Controls.OBC.')) &&
    ...
    if (!defaultName && !className.includes('UsersGuide')) {
...
function getModels (model, prefix) {
  ...
  if (elements) {
    ...
    if (model.class_definition[0].class_specifier.short_class_specifier) {
      if (prefix === 'public') {
        const declaration = parseComponentDeclaration(elements, currentClass, 'class')
      ...
      jsonClassNames.forEach(function (obj) {
        const declarations = (prefix === 'extends')
          ...
          : parseComponentDeclaration(obj.component_clause.component_list.component_declaration[0], currentClass, 'class')
  ...
  if (elements) {
    if (model.class_definition[0].class_specifier.short_class_specifier) {
      ...
      jsonPars.forEach(function (obj) {
        const declarations = parseComponentDeclaration(obj.component_clause.component_list.component_declaration[0], currentClass, 'parameter')
    ...
    if (data[index].connections) {
      ...
      data[index].connections.forEach(function (connectStatement, idx) {
        ...
        if (connector.connector === undefined) {
          ...
          const instance = allModels ? allModels.find(item => {
...
function parseClassModification (classModification, name) {
  ...
  for (var i = 0; i < mod.length; i++) {
    if (mod[i].name === name) {
      ...
      val['isFinal'] = false
      if (val['prefix'] && val['prefix'] === 'final') {
        val['isFinal'] = true
        delete val['prefix']
      }
      delete val['name']
...
function parseComponentCondition (componentDeclaration) {
  ...
  const enableCondition = (componentDeclaration.condition_attribute)
                          ...
                          : null
  return Object.assign({'instantiate': enableCondition})
}

function parseVendorAnnotation (vendorAnnotation) {
  var annotationEntry = {}
  var inAnnotationEntry = {}
  const vendorName = vendorAnnotation.name
  if (vendorAnnotation.intAnn) {
    var intAnnObj = vendorAnnotation.intAnn
    var innAnoNam = []
    intAnnObj.forEach(ele => { innAnoNam.push(ele.name) })
    // Find unique names
    var uniNam = innAnoNam.filter((item, i, ar) => ar.indexOf(item) === i)
    uniNam.forEach(obj => {
      var tempEnt = []
      for (var i = 0; i < intAnnObj.length; i++) {
        if (obj === intAnnObj[i].name) {
          tempEnt.push(parseVendorAnnotation(intAnnObj[i])[obj])
        }
      }
      inAnnotationEntry[obj] = tempEnt
    })
  } else {
    inAnnotationEntry = null
  }
  if (vendorAnnotation.annotation) {
    vendorAnnotation.annotation.forEach(obj => {
      if (obj.name === 'haystack') {
        annotationEntry[obj.name] = haystackAnnotation(obj)
      } else if ((obj.name === 'brick')) {
        annotationEntry[obj.name] = obj.value.slice(1, -1)
      } else {
        annotationEntry[obj.name] = obj.value
      }
    })
  } else {
    annotationEntry = null
  }
  var combined = Object.assign({}, annotationEntry, inAnnotationEntry)
  return { [vendorName]: combined }
}
...
function parseComponentDeclaration (declaration, currentClass, insType) {
  ...
  var ret = (declaration.className)
    ...
    : {
      ...
      'fixed': parseClassModification(declaration.declaration.class_modification, 'fixed'),
      'stateSelect': parseClassModification(declaration.declaration.class_modification, 'stateSelect'),
...
function getComponentsOfClass (model, parseMode, className, prefix) {
  ...
  if (elements) {
    if (model.class_definition[0].class_specifier.short_class_specifier) {
      ...
      jsonClassNames.forEach(function (obj) {
        const declarations = parseComponentDeclaration(obj.component_clause.component_list.component_declaration[0], currentClass, 'class')
...
/**
 * Add point list to the json objects
 *
 * @param jsonDataSet entire json data set
 */
function addPointlist (jsonDataSet) {
  jsonDataSet.forEach(ele => Object.assign(ele, {'hasPointList': false}))
  const entryData = jsonDataSet[0]
  var pointList = createPointlist(entryData)
  var hasPointList = !!pointList
  // Add point list to the entry object
  Object.assign(entryData, {'hasPointList': hasPointList}, pointList)
  var pubCla = (entryData.public && entryData.public.models) ? entryData.public.models : null
  var proCla = (entryData.protected && entryData.protected.models) ? entryData.protected.models : null
  var allCla = [].concat(pubCla).concat(proCla).filter(function (obj) { return obj })
  allCla.forEach(ele => {
    // Check if the instance should have point list
    const insCDL = ele.__cdl
    if (insCDL) {
      const propagateList = insCDL.propagate
      if (propagateList) {
        propagateList.forEach(obj => {
          var tempName = (obj.instance).trim()
          var instanceName = tempName.substring(1, tempName.length - 1)
          // Check it is propagating connector, if so, then the model that has the connector should generate
          // point list
          if (obj.trend || obj.connection) {
            // Propagate the connector specification and generate the point list
            propagateConnector(ele.className, obj, jsonDataSet)
          }
          if (obj.generatePointlist) {
            var elePath = ele.className
            generatePointlist(elePath, instanceName, jsonDataSet)
          }
        })
      }
      if (insCDL.generatePointlist && insCDL.generatePointlist === 'true') {
        var eleObj = findJsonObject(ele.className, jsonDataSet)
        var innPointList = createPointlist(eleObj)
        var hasInPointList = !!innPointList
        // Add point list to the sub object
        Object.assign(eleObj, {'hasPointList': hasInPointList}, innPointList)
      }
    }
  })
}

function generatePointlist (classPath, instanceName, jsonDataSet) {
  var classObj = findJsonObject(classPath, jsonDataSet)
  if (!instanceName.includes('.')) {
    var innClassPath = findClassPath(classObj, instanceName)
    var innClassObj = findJsonObject(innClassPath, jsonDataSet)
    if (!innClassObj.pointList) {
      var pointList = createPointlist(innClassObj)
      var hasPointList = !!pointList
      // Add point list to the entry object
      Object.assign(innClassObj, {'hasPointList': hasPointList}, pointList)
    }
  } else {
    var names = instanceName.split('.')
    var newClassName = findClassPath(classObj, names[0])
    var truNam = names.slice(1, -1)
    var newInstanceName = truNam.join('.')
    generatePointlist(newClassName, newInstanceName, jsonDataSet)
  }
}

function findJsonObject (className, jsonDataSet) {
  var classObj
  for (var i = 0; i < jsonDataSet.length; i++) {
    var ithItem = jsonDataSet[i]
    if (ithItem.topClassName.endsWith(className)) {
      classObj = ithItem
    }
  }
  return classObj
...
function propagateConnector (claInstance, connectorSpe, jsonDataSet) {
  var workDataSet = JSON.parse(JSON.stringify(connectorSpe))
  var tempName = (workDataSet.instance).trim()
  var instanceName = tempName.substring(1, tempName.length - 1)
  // Find the object
  var classObj = findJsonObject(claInstance, jsonDataSet)
  if (!instanceName.includes('.')) {
    var pubInp = (classObj.public && classObj.public.inputs) ? classObj.public.inputs : null
    var pubOutp = (classObj.public && classObj.public.outputs) ? classObj.public.outputs : null
    var pubInt = [].concat(pubInp).concat(pubOutp).filter(function (obj) { return obj })
    // Find the connector and replace the specification with the propagated one
    for (var j = 0; j < pubInt.length; j++) {
      var jthInt = pubInt[j]
      if (jthInt.name === instanceName) {
        // If the connector was not specified
        if (!jthInt.__cdl) {
          delete workDataSet.instance
          jthInt['__cdl'] = workDataSet
        } else {
          jthInt['__cdl']['connection'] = workDataSet.connection
          jthInt['__cdl']['trend'] = workDataSet.trend
        }
      }
    }
    var pointList = createPointlist(classObj)
    var hasPointList = !!pointList
    // Add point list to the entry object
    Object.assign(classObj, {'hasPointList': hasPointList}, pointList)
  } else {
    var names = instanceName.split('.')
    var className = findClassPath(classObj, names[0])
    var truNam = names.slice(1)
    var newInstanceName = truNam.join('.')
    workDataSet['instance'] = newInstanceName
    propagateConnector(className, workDataSet, jsonDataSet)
  }
}

function createPointlist (classObj) {
  var conDev = classObj.vendorAnnotation ? classObj.vendorAnnotation.__cdl.controlledDevice : null
  var trimName = 'Not_specified'
  if (conDev) {
    var temp = conDev.trim()
    trimName = temp.substring(1, temp.length - 1)
  }
  var pubInp = (classObj.public && classObj.public.inputs) ? classObj.public.inputs : null
  var pubOutp = (classObj.public && classObj.public.outputs) ? classObj.public.outputs : null
  var pubInt = [].concat(pubInp).concat(pubOutp).filter(function (obj) { return obj })
  ...
  if (pubInt.length > 0) {
    pubInt.forEach(ele => {
      const point = {}
      point['name'] = ele.name
      point['type'] = pointType(ele.className)
      const connectionSpe = (ele.__cdl && ele.__cdl.connection) ? ele.__cdl.connection : null
      const conSpeFla = connectionSpe ? connectionSpe[0].hardwired : 'false'
      point['hardwired'] = (conSpeFla === 'true') ? 'Yes' : 'No'
      const trendSpe = (ele.__cdl && ele.__cdl.trend) ? ele.__cdl.trend : null
      point['trend'] = trendSpe ? (((!trendSpe[0].enable) || (trendSpe[0].enable === 'true')) ? trendSpe[0].interval : 'n/a') : 'n/a'
      point['description'] = ele.comment
      point['controlledDevice'] = trimName
      points.push(point)
    })
    return Object.assign({'pointList': points})
  } else {
    return null
  }
}

function findClassPath (classObj, insName) {
  var pubCla = (classObj.public && classObj.public.models) ? classObj.public.models : null
  var proCla = (classObj.protected && classObj.protected.models) ? classObj.protected.models : null
  var allCla = [].concat(pubCla).concat(proCla).filter(function (obj) { return obj })
  var className
  for (var i = 0; i < allCla.length; i++) {
    var ithCla = allCla[i]
    if (ithCla.name === insName) {
      className = ithCla.className
    }
  }
  return className
}

function pointType (className) {
  var type = null
  if (className.endsWith('Interfaces.RealInput') || className.endsWith('Interfaces.IntegerInput')) {
    type = 'AI'
  } else if (className.endsWith('Interfaces.RealOutput') || className.endsWith('Interfaces.IntegerOutput')) {
    type = 'AO'
  } else if (className.endsWith('Interfaces.BooleanInput')) {
    type = 'BI'
  } else if (className.endsWith('Interfaces.BooleanOutput')) {
    type = 'BO'
  }
  return type
}
...
module.exports.addPointlist = addPointlist
