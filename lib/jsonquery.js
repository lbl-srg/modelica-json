...
function getModels (model, prefix) {
  ...
  if (elements) {
    ...
    jsonClassNames.forEach(function (obj) {
      const declarations = obj.extends_clause ? parseextendsClause(obj.extends_clause)
                                              : parsecomponentDeclaration(obj.component_clause.component_list.component_declaration[0])
      const entry = Object.assign(
        {'className': obj.extends_clause ? obj.extends_clause.name
...
function parseextendsClause (extendsClause) {
  var ret = {
    ...
    'modifications': parseExtendsAssignments(extendsClause.class_modification)
  }
  return ret
}
...
function parseExtendsAssignments (classModification) {
  ...
  for (var k = 0; k < mod.length; k++) {
    if (mod[k].prefix) {
      if (mod[k].prefix === 'final') {
        assignments.push({
          'name': mod[k].name,
          'value': mod[k].value,
          'isFinal': (mod[k].prefix).indexOf('final') !== -1
        })
      } else if (mod[k].prefix.includes('redeclare')) {
        assignments.push({
          'prefix': mod[k].prefix,
          'className': mod[k].className,
          'name': mod[k].name,
          'modifications': parseParameterAssignments(mod[k].modification),
          'isFinal': (mod[k].prefix).indexOf('final') !== -1
        })
      }
    } else {
