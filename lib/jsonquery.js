...
/** Get the simplified json representation for the model
  */
function simplifyModelicaJSON (model, obcOnly) {
  ...
  if (proInputs.length !== 0 && !isElementaryCDL(className) && within.includes('Buildings.Controls.OBC.')) {
  ...
  if (proOutputs.length !== 0 && within.includes('Buildings.Controls.OBC.')) {
  ...
  if (connections) {
    addConnectorTargets(connections, allModels, obcOnly)
  }
    ...
    jsonClassNames.forEach(function (obj) {
      ...
      const entry = Object.assign(
        {'className': (prefix === 'extends') ? obj.extends_clause.name
                                             : obj.component_clause.type_specifier}, declarations)
      classes.push(entry)
    })
  }
  return classes
...
function addConnectorTargets (connectors, models) {
  function getPortLinkTarget (obj) {
    ...
    if (i === -1) {
      const msg = 'Did not find model for connector ' + cla + ' in current model, may find it in extends model'
      logger.warn(msg)
      const extModels = models.filter(function (obj) {
        return !obj.name
      })
...
function validateClass (model) {
  ...
  if (model.within.includes('Buildings.Controls.OBC')) {
