...
/** Get the JSON property `p` from the json data `o`
  * If the property does not exist, the function return null
  */
    ...
    if (data[index].connections) {
      ...
      data[index].connections.forEach(function (connectStatement, idx) {
        ...
        if (connector.connector === undefined) {
          ...
          const allModels = (data[index].public && data[index].public.models)
            ? data[index].public.models.concat(data[index].protected.models)
            : data[index].protected.models
