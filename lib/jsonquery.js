...
function classDefinition (claDef) {
  ...
  const classSpecifierObj = classSpecifier(claDef.class_specifier)
  return Object.assign(
...
function elementModification (eleMod) {
  ...
  if (isGraAnn) {
    const graMod = mod ? graPri.graphicAnnotationObj(name, modification(mod)) : undefined
    return Object.assign(
      {[name]: graMod}
    )
  } else {
 ...
 */
function simpleExpression (simExp) {
  const logExp1 = logicalExpression(simExp.logical_expression1)
  const logExp2 = simExp.logical_expression2
    ? (':' + logicalExpression(simExp.logical_expression2))
    : ''
  const logExp3 = simExp.logical_expression3
    ? (':' + logicalExpression(simExp.logical_expression3))
    : ''
  var funCal, forLoo, logExp, ifExp
  if (!(logExp2 || logExp3)) {
    const pri = checkPri (simExp.logical_expression1)
    if (pri) {
      funCal = functionCallObj(pri)
      forLoo = forLoopObj(pri)
      ifExp = ifExpressionObj(pri)
    }
    logExp = logicalExpressionObj(simExp.logical_expression1)
  }
  if (funCal || forLoo || logExp || ifExp) {
    return Object.assign(
...
function ifExpressionObj (pri) {
  ...
  for (var i = 0; i < expLis.length; i++) {
    ...
    for (var j = 0; j < ifEls.length; j++) {
      var jthEle = ifEls[j]
      var condition = expressionString(jthEle.condition)
      var then = expressionString(jthEle.then)
...
function forLoopObj (pri) {
  ...
  const exp = funArgObj(funArg)
  const forObj = Object.assign({'expression': exp}, forIndObj(forInd))
  return forObj
}
...
function logicalExpressionObj (logExp) {
  ...
  for (var i = 0; i < termList.length; i++) {
    ...
    for (var j = 0; j < factorList.length; j++) {
      ...
      var jthFactorObj = logicalFactorObj(jthFactor)
      if (jthFactorObj) {
...
function functionCallObj (pri) {
  ...
  if (unsNum || priStr || isFalse || isTrue || comRef || outExpLis || expLis || funArg || end) {
    return undefined
  }
  if (funCalPri && funCalPri.function_call_args) {
    ...
    return Object.assign(
      ...
      {'arguments': funCalArgObj(funCalArg)}
    )
  }
}
...
function funCalArgObj (funCalArg) {
  ...
  return funArg ? funArgsObj(funArg) : undefined
}
...
function funArgsObj (argObj) {
  ...
  const argArray = []
  if (namedArg) {
    argArray.push(Object.assign({'name': namedArgsString(namedArg)}))
  } else {
    if (funArgs) {
      argArray.push(Object.assign({'name': funArgString(funArg)}))
      argArray.push(Object.assign({'name': funArgsString(funArgs)}))
    } else if (forInd) {
      const exp = funArgObj(funArg)
      const forObj = Object.assign({'expression': exp}, forIndObj(forInd))
 ...
 * 
 * @param forIndObj for_indices object
 */
function forIndObj (forIndObj) {
  const ind = forIndObj.indices
  const indices = []
  for (var i = 0; i < ind.length; i++) {
    const ithInd = ind[i]
    const ident = ithInd.identifier
    const exp = ithInd.expression
    const expString = exp ? expressionString(exp) : undefined
...
function funArgObj (funArg) {
  ...
  if (exp) {
    ...
    if (ifExp) {
      ...
      return simpleExpression(simExp)
    }
  } else {
    return undefined
  }
...
function logicalFactor (logFac) {
  const relStr = relation(logFac.relation)
