...
parameter Real uMin (max=0, unit="1")=-0.25
...
parameter Real uMax (min=0, unit="1")=+0.25
	"Upper limit of controller input when return damper is closed (see diagram)"
	annotation (Evaluate=true, Dialog(tab="Commissioning", group="Controller"))
parameter Real uOutDamMax (min=-1, max=1, unit="1")=(uMin +uMin)/2
	"Maximum loop signal for the OA damper to be fully open"
	annotation (Evaluate=true, Dialog(tab="Commissioning", group="Controller"))
parameter Real uRetDamMin (min=-1, max=1, unit="1")=(uMin +uMin)/2
...
Buildings.Controls.OBC.CDL.Interfaces.RealInput uTSup (unit="1")
	"Signal for supply air temperature control (T Sup Control Loop Signal in diagram)"
	annotation (Placement(transformation(extent={{-160,-20},{-120,20}}), iconTransformation(extent={{-120,-10},{-100,10}})))
Buildings.Controls.OBC.CDL.Interfaces.RealInput uOutDamPosMin (min=0, max=1, unit="1")
	"Minimum economizer damper position limit as returned by the damper position limits  sequence"
	annotation (Placement(transformation(extent={{-160,-120},{-120,-80}}), iconTransformation(extent={{-120,-90},{-100,-70}})))
Buildings.Controls.OBC.CDL.Interfaces.RealInput uOutDamPosMax (min=0, max=1, unit="1")
	...
	annotation (Placement(transformation(extent={{-160,-70},{-120,-30}}), iconTransformation(extent={{-120,-50},{-100,-30}})))
Buildings.Controls.OBC.CDL.Interfaces.RealInput uRetDamPosMin (min=0, max=1, unit="1")
	"Minimum return air damper position limit as returned by the economizer enable-disable sequence"
	annotation (Placement(transformation(extent={{-160,30},{-120,70}}), iconTransformation(extent={{-120,30},{-100,50}})))
Buildings.Controls.OBC.CDL.Interfaces.RealInput uRetDamPosMax (min=0, max=1, unit="1")
	"Maximum return air damper position limit as returned by the economizer enable-disable sequence"
	annotation (Placement(transformation(extent={{-160,80},{-120,120}}), iconTransformation(extent={{-120,70},{-100,90}})))
Buildings.Controls.OBC.CDL.Interfaces.RealOutput yOutDamPos (min=0, max=1, unit="1")
	"Economizer damper position"
	annotation (Placement(transformation(extent={{120,-70},{140,-50}}), iconTransformation(extent={{100,-30},{120,-10}})))
Buildings.Controls.OBC.CDL.Interfaces.RealOutput yRetDamPos (min=0, max=1, unit="1")
	"Return air damper position"
	annotation (Placement(transformation(extent={{120,50},{140,70}}), iconTransformation(extent={{100,10},{120,30}})))
protected
Buildings.Controls.OBC.CDL.Continuous.Sources.Constant outDamMinLimSig (k=uMin)
	"Minimal control loop signal for the outdoor air damper"
	annotation (Placement(transformation(extent={{-60,-32},{-40,-12}})))
Buildings.Controls.OBC.CDL.Continuous.Sources.Constant outDamMaxLimSig (k=uOutDamMax)
	"Maximum control loop signal for the outdoor air damper"
	annotation (Placement(transformation(extent={{-100,-60},{-80,-40}})))
Buildings.Controls.OBC.CDL.Continuous.Sources.Constant retDamConMinLimSig (k=uRetDamMin)
...
public
Buildings.Controls.OBC.CDL.Continuous.Sources.Constant retDamMaxLimSig (k=uMax)
	"Maximal control loop signal for the return air damper"
	annotation (Placement(transformation(extent={{-60,10},{-40,30}})))
Buildings.Controls.OBC.CDL.Continuous.Line outDamPos (limitBelow=true, limitAbove=true)
	"Damper position is linearly proportional to the control signal between signal limits"
	annotation (Placement(transformation(extent={{0,-40},{20,-20}})))
protected
Buildings.Controls.OBC.CDL.Continuous.Line retDamPos (limitBelow=true, limitAbove=true)
	"Damper position is linearly proportional to the control signal between signal limits"
...
Buildings.Controls.OBC.CDL.Discrete.FirstOrderHold firOrdHolOutDam (samplePeriod=samplePeriod)
...
Buildings.Controls.OBC.CDL.Discrete.FirstOrderHold firOrdHolRetDam (samplePeriod=samplePeriod)
	"First order hold to avoid too fast opening/closing of damper (which may cause freeze protection to be too slow to compensate)"
...
connect(outDamPos.f2, uOutDamPosMax)
	annotation (Line(points={{-2,-38},{-20,-38},{-20,-66},{-108,-66},{-108,-50},{-140,-50}}, color={0,0,127}));
...
connect(firOrdHolRetDam.y, yRetDamPos)
	...
	annotation (defaultComponentName="mod", Icon(graphics={Rectangle(extent={{-100,-100},{100,100}},lineColor={0,0,127},fillColor={255,255,255},fillPattern=FillPattern.Solid),Line(points={{20,58}},color={28,108,200}),Line(points={{-92,-84},{-50,-84},{12,70},{82,70}},color={0,0,127},thickness=0.5),Line(points={{-66,58},{12,58},{50,-76},{100,-76}},color={0,0,127},pattern=LinePattern.Dash,thickness=0.5),Text(extent={{-108,138},{102,110}},lineColor={0,0,127},textString="%name")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-120},{120,120}}), graphics={Rectangle(extent={{-26,118},{46,-118}},lineColor={0,0,0},fillColor={215,215,215},fillPattern=FillPattern.Solid),Rectangle(extent={{-118,118},{-34,-118}},lineColor={0,0,0},fillColor={215,215,215},fillPattern=FillPattern.Solid),Text(extent={{-80,128},{-36,88}},lineColor={0,0,0},fontSize=12,horizontalAlignment=TextAlignment.Left,textString="Damper position
